...
int main(void)
{
    ...
    for (struct storage_device *dev = storage_device_get_next(NULL); dev != NULL;
         dev = storage_device_get_next(dev)) {
        ...
        TLK_LOG_INFO(nv_storage_demo, "\tcapable: %c%c%c\n",
                dev->erase ? 'E' : '_',
                dev->read ? 'R' : '_',
                dev->write ? 'W' : '_');
    }
    ...
    do {
        ...
        const uintptr_t addr = UNISDK_CHIP_MEMORY_ROM_STARTADDR +
                                   UNISDK_CHIP_MEMORY_ROM_SIZE * 1024 - 5000;

        /* Now next line should be un-commented */
        TLK_LOG_INFO(nv_storage_demo, "test storage access at 0x%zx with %zu bytes\n", addr, sizeof(data));

        struct storage_device *s_dev = storage_device_find(addr, sizeof(data));
