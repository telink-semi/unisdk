...
void rf_configure(enum tlk_rf_mode mode, enum tlk_rf_bitrate bitrate)
{
    ...
    RfDriver::configure(mode, &rf_config, &rf_mode_config);
}
...
int main(void)
{
    ...
    led1.write(UNISDK_BOARD_LED_1_PIN, 1);
...
{
    TLK_LOG_INFO(rf_demo, "[%u] Received!", GET_PDU(RfDriver::instance().rx_packet)->payload[0]);
}
...
void on_pong_received(void)
{
    ...
    uint8_t* rx_payload = GET_PDU(RfDriver::instance().rx_packet)->payload;

    sent_packets++;

    if (ping >= TLK_MS_TO_US(PING_TIMEOUT_MS) || RfDriver::instance().rx_timeout_error)
    {
    ...
    else
    {
        if (tx_payload[0] == rx_payload[0])
        {
            ...
            TLK_LOG_INFO(rf_demo, "Time: %-4u us | RSSI: %i", ping, RfDriver::instance().rx_rssi);
