...
#if TLK_IS_ENABLED(CONFIG_TLK_RF_DEMO_EXCHANGE_BTX_BRX)
...
void transmitter_loop(void)
{
    ...
    while (1)
    {
        ...
        tlk_rf_start_btx(tlk_rf_get_tick(), UNISDK_RF_MAX_TIMEOUT_BTX, 0);
...
void receiver_loop(void)
{
    ...
    while (1)
    {
        do
        {
            tlk_rf_start_brx(tlk_rf_get_tick(), UNISDK_RF_MAX_TIMEOUT_BRX, RT_DELAY_US);
