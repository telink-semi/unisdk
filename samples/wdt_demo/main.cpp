#include "core/cpp_wrappers/tlk_watchdog.hpp"
...
int main(void)
{
...
#if TLK_IS_ENABLED(CONFIG_TLK_WDT_DEMO_GPIO_INDICATION)
    GPIODriver gpio(UNISDK_BOARD_LED_0_PORT);
    gpio.configure(UNISDK_BOARD_LED_0_PIN, TLK_GPIO_OUTPUT);
    gpio.write(UNISDK_BOARD_LED_0_PIN, 1);

    gpio.configure(UNISDK_BOARD_LED_1_PIN, TLK_GPIO_OUTPUT);
    gpio.write(UNISDK_BOARD_LED_1_PIN, 0);
#endif
    ...
    WDTDriver::feed();
#else
    WDTDriver::start(TLK_SEC_TO_MS(5));
#endif
...
#if TLK_IS_ENABLED(CONFIG_TLK_WDT_DEMO_GPIO_INDICATION)
    gpio.write(UNISDK_BOARD_LED_1_PIN, 1);
#endif
