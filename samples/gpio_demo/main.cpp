...
#if IS_ENABLED(CONFIG_TLK_GPIO_SAMPLE_PLIC)
...
#if IS_ENABLED(CONFIG_TLK_GPIO_SAMPLE_PM)
...
#if IS_ENABLED(CONFIG_TLK_GPIO_SAMPLE_PM)
        if (tlk_api_micros() - last_us >= TLK_USEC(4)) {
            GPIO_Port_Led_3.write(UNISDK_BOARD_LED_3_PIN, false);
            tlk_api_sleep(TLK_MSEC(4));
