...
This sample shows:
- if `CONFIG_TLK_UART_AUTO_CONFIG` is disabled:
    * Manual UART configuration using `tlk_uart_configure` and `tlk_uart_configure_pinmux`
    * Manual UART flow configuration using `tlk_uart_flow_configure` and `tlk_uart_flow_configure_pinmux` if `CONFIG_TLK_UART1_FLOW_CONTROL` is enabled
...
### Initialization
1.  **Manual Configuration:** If `CONFIG_TLK_UART_AUTO_CONFIG` is disabled, the sample configures `TLK_UART1` with set parameters.
