...
#if IS_ENABLED(CONFIG_TLK_UART_DEMO_PREVENT)
    #include "core/include/tlk_pm.h"
#endif

#define UART_TIMEOUT TLK_USEC(5)
...
int main(void)
{
...
#if IS_DISABLED(CONFIG_TLK_UART_DEMO_AUTO_CONFIG)
    ...
    IF_ENABLED(CONFIG_TLK_UART0_FLOW_CONTROL,
        ...
        tlk_uart_flow_configure(CONFIG_TLK_UART_DEMO_UART, &flow_control_config););
    ...
    while (1) {
        ...
        enum tlk_uart_status recieve_status = tlk_uart_receive_bytes(CONFIG_TLK_UART_DEMO_UART, data, CONFIG_TLK_UART_DEMO_BUFFER_SIZE, rx_handler, UART_TIMEOUT);

    #if IS_ENABLED(CONFIG_TLK_UART_DEMO_PREVENT)
        enum tlk_pm_sleep_status sleep_status = tlk_pm_sleep(TLK_PM_SLEEP_MODE_SUSPEND, 10);

        if (sleep_status == TLK_PM_SLEEP_DENIED) {
            uart_print(CONFIG_TLK_UART_DEMO_UART, "The sleep was prevented due to active receiving\n", UART_TIMEOUT);
        }
    #endif
        ...
        switch (recieve_status) {
            case TLK_UART_OK:
                ...
                tlk_uart_send_bytes(CONFIG_TLK_UART_DEMO_UART, data, rx_count, tx_handler, UART_TIMEOUT);
                while (!tx_done) {}
                tx_done = 0;
                rx_count = 0;
            ...
            case TLK_UART_TIMEOUT:
                // On timeout, data is not sent, so a '\n' is not required at the beginning of message.
