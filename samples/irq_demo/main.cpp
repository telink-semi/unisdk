...
void tlk_print(const char* __restrict format, ...)
{
    ...
    if (size > 0)
    {
        uart0.sendBytes((uint8_t*) tlk_print_buffer, size, tlk_tx_handler, TLK_SEC_TO_US(5));
        while (!tlk_tx_done)
        {
        }
        tlk_tx_done = 0;
...
static void tlk_sample_msi_handler(void)
{
    ...
    PlicDriver::setPending(TLK_SAMPLE_PLIC_SRC_LOW);

    tlk_print("\n");
    tlk_sample_log("Pending PLIC high (MEI)");
    PlicDriver::setPending(TLK_SAMPLE_PLIC_SRC_HIGH);
...
int main(void)
{
    ...
    // MTI Configuration
    tlk_mtimer_register_callback(tlk_sample_mti_handler);

    // MEI Configuration
    PlicDriver::setPriority(TLK_SAMPLE_PLIC_SRC_LOW, TLK_IRQ_PRI_LEV1);
    PlicDriver::setPriority(TLK_SAMPLE_PLIC_SRC_HIGH, TLK_IRQ_PRI_LEV2);
    PlicDriver::setPriority(UNISDK_PLIC_IRQ_NUM_UART0, TLK_IRQ_PRI_LEV3); // TODO: move to UART

    PlicDriver::enable(TLK_SAMPLE_PLIC_SRC_LOW);
    PlicDriver::enable(TLK_SAMPLE_PLIC_SRC_HIGH);

#if TLK_IS_ENABLED(CONFIG_TLK_IRQ_DEMO_IRQ_BASE_PLIC) &&                                           \
    TLK_IS_ENABLED(CONFIG_TLK_IRQ_DEMO_PLIC_PREEMPT)
    PlicDriver::preemptEnable();
#endif
