...
2. **MTI Configuration:** Registers `tlk_sample_mti_handler` as the hardware timer-driven exception callback via `tlk_mtimer_irq_register_callback`.
...
4. **Preemption & M-Mode Activation:** If both `CONFIG_TLK_IRQ_DEMO_IRQ_BASE_PLIC` and `CONFIG_TLK_IRQ_DEMO_PLIC_PREEMPT` are set, the hardware module enables nested interrupt execution via `tlk_plic_enable_preempt()`. The core enabkes interrupt flags (MSIE, MTIE, MEIE) and activates global interrupts via `tlk_core_interrupt_enable()`.
