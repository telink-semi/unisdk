...
class SocBoardConfigSetter:
    ...
    def set_soc(self, soc_name):
        ...
        if dependencies:
            ...
            for dep in dependencies:
                if dep.startswith('TLK_SOC_SERIES_'):
    ...
    def set_board(self, board_name):
        ...
        if dependencies:
            ...
            for dep in dependencies:
                if dep.startswith('TLK_SOC_'):
                    # Set SOC dependency if not already set
                    soc_name = dep.replace('TLK_SOC_', '')
        ...
        self._set_config_value('TLK_BOARD', f'"{board_string_value}"')
    ...
    def _set_config_value(self, key, value):
        ...
        try:
            ...
            if key == 'TLK_HAS_BOARD_SELECTED' or key == 'TLK_BOARD' or key.startswith('TLK_BOARD_'):
            ...
            if key in self.kconf.syms:
                ...
                board_symbols = [name for name in self.kconf.syms.keys() if 'TLK_BOARD' in name]
                if board_symbols:
    ...
    def _set_config_direct(self, key, value):
        ...
        lines = [line for line in content.split('\n') 
                if not line.startswith(f'CONFIG_{key}=') and not line.startswith(f'# CONFIG_{key} is not set')]
        
        # Add new setting with CONFIG_ prefix
    ...
    def _unset_other_socs(self, current_soc):
        ...
        for soc_name in self.parser.socs:
            if not soc_name.startswith('TLK_SOC_'):
                continue
            
            clean_soc_name = soc_name.replace('TLK_SOC_', '')
    ...
    def _unset_other_boards(self, current_board):
        ...
        for board_name in self.parser.boards:
            if not board_name.startswith('TLK_BOARD_'):
                continue
            
            clean_board_name = board_name.replace('TLK_BOARD_', '')
