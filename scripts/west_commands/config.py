...
CONFIG_USAGE = '''
west tl-config [-h] [-d BUILD_DIR] [source_dir]''' 
    ...
    def do_run(self, args, unknown_args):
        ...
        if args.list_socs:
            ...
            for soc in socs:
                ...
                self.inf(f"{soc['name'].replace('SOC_', '')}: {soc['prompt']} (dependencies: {deps})")
            return 0
        ...
        if args.validate:
            ...
            is_valid, error_msg = Build()._validate_soc_board(soc_name, board_name)
            if is_valid:
