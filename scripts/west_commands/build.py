...
west tl-build [-h] [-d BUILD_DIR]
           ...
           [-n] [-o BUILD_OPT] [-f] [--kconfig] [-b BOARD] [--soc SOC]
...
class Build(WestCommand):
    ...
    def _validate_soc_board(self, soc, board):
        ...
        try:
            ...
            self.wrn(f"Warning: Failed to import kconfig_parser: {e}")
            return True, ""  # Skip validation if parser is not available
        ...
        try:
            ...
            if not kconfig_file.exists():
                return True, ""  # Skip validation if Kconfig file doesn't exist
            ...
            self.wrn(f"Warning: SOC/BOARD validation failed: {e}")
            return True, ""  # Allow build to continue if validation fails
    ...
    def _sanity_check(self):
        ...
        if apps_mismatched and not self.auto_pristine:
        ...
        if (boards_mismatched or socs_mismatched) and not self.auto_pristine:
        ...
        if self.auto_pristine and (apps_mismatched or boards_mismatched or socs_mismatched):
            self._clean_build()
            self.cmake_cache = None
    ...
    def do_run(self, args, unknown_args):
        ...
        if "TELINK_TOOLCHAIN_PATH" not in os.environ:
            self.err("Error: TELINK_TOOLCHAIN_PATH environment variable not set")
            self.die("Please set it first")
            return 1
