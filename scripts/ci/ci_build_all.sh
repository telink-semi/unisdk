...
MEMORY_PARSE_SCRIPT="./scripts/ci/parse.sh"          # Script to collect memory usage report

collect_memory_report() {
    exit_code=$?

    # Do not let report generation hide the original build error
    set +e
    trap - EXIT

    echo
    echo "[ci_build_all.sh] Collecting memory usage report..."

    if [ -f "$MEMORY_PARSE_SCRIPT" ]; then
        bash "$MEMORY_PARSE_SCRIPT" --csv
        report_code=$?

        if [ -f "ci_logs/memory_report.log" ]; then
            echo
            cat "ci_logs/memory_report.log"
        fi
    else
        echo "[ci_build_all.sh] ERROR: Memory parser not found: $MEMORY_PARSE_SCRIPT"
        report_code=1
    fi

    # If build was successful, but report generation failed, fail the job
    if [ "$exit_code" -eq 0 ] && [ "$report_code" -ne 0 ]; then
        exit "$report_code"
    fi

    # If build failed, preserve the original build failure code
    exit "$exit_code"
}

trap collect_memory_report EXIT

SOC_SERIES=("TL321X" "TL721X" "TLSR952X")
BOARDS=("TL3218X_EVK" "TL7218X_EVK" "TLSR9528A_EVK")
SAMPLE_NAMES=("gpio" "pm" "uart" "irq")
