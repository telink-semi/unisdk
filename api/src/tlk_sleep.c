...
#define TLK_API_CONVERT_MS_DELAY_TO_TICKS(ms)                                                      \
    ((uint32_t) (((uint64_t) (ms) * UNISDK_MTIMER_CYCLES_PER_SECOND) / 1000ULL))

static uint64_t current_time = 0;

void tlk_api_time_handle_mtimer_callback(void)
{
    tlk_core_mie_disable(TLK_FLD_MIE_MTIE);
    tlk_mtimer_set_mtime_compare(tlk_mtimer_get_mtime() + 1);
}

void tlk_api_time_init_mtimer(void)
{
    tlk_mtimer_irq_register_callback(tlk_api_time_handle_mtimer_callback);
}

TLK_REGISTER_PRE_INIT(tlk_api_time_init_mtimer, TLK_INIT_LEVEL_DRIVER, TLK_INIT_PRIORITY_NORMAL);
...
void tlk_api_sleep(uint32_t duration_ms)
{
    ...
    enum tlk_pm_sleep_status pm_sleep_state = TLK_PM_SLEEP_MS(mode, duration_ms);
...
#endif
    {
        uint32_t sleep_duration_tick = TLK_API_CONVERT_MS_DELAY_TO_TICKS(duration_ms);
