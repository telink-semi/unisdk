...
/**
 * @brief   This function is designed to prevent the aging of the ADC reference voltage during deep
 * sleep.
 * @return  none
 * @note    The reference voltage value of ADC Vref decreases with time, which can be avoided by
...
_tlk_attribute_ram_code_sec_noinline_ void tlk_adc_anti_aging_mode_for_asm(void)
{
    ...
    reg_rst3 |= FLD_RST3_SARADC;
    // The following logic is equivalent to tlk_adc_en_clk()
    reg_clk_en3 |= FLD_CLK3_SARADC_EN;

    if (read_reg8(0x14083d) == TLK_CHIP_VERSION_A1)
    {
        // Since this interface is called within the .s file and requires the configuration of
        // analog registers, it is necessary to enable the analog clock first.
        reg_rst1 |= FLD_RST1_ALGM;
        reg_clk_en1 |= FLD_CLK1_ALGM_EN,
            /*
             * Turn on xtal_24M clock to analog (includes stimer), this should setup before power up
             * PLL and call tlk_pm_wait_xtal_ready. Because the stimer is necessary for the
             * tlk_pm_wait_xtal_ready. ADC operation depends on the 24M_xtl in sc_clk. In sc_clk,
             * the 24M_xtl source requires two switches to be enabled in sequence. The first level
             * enables the 24M_xtl source, and the second level is controlled by bit 0x8c[1] in
             * reg_ana. (add by bolong, confirmed by ya.yan,qiangkai20240513)
             */
    ...
    reg_adc_config0 =
        ((reg_adc_config0 & (~FLD_SCANT_MAX)) | ((1 * 2) << 4)); // scan_cnt = chn_cnt*2
    ...
    reg_adc_channel_set_state(TLK_ADC_M_CHANNEL) =
        (reg_adc_channel_set_state(TLK_ADC_M_CHANNEL) & (~FLD_SEL_VREF)) |
        (TLK_ADC_VREF_ANTI_AGING << 6);
