...
#define REG_OTP_STATUS      0x80140307
...
/* ==== EXTERN HELPERS ==== */
extern void tlk_wdt_start(uint32_t timeout);
extern void tlk_adc_anti_aging_mode_for_asm(void);
...
_tlk_attribute_ram_code_sec_noinline_ void tlk_core_init(uint8_t init_type)
{
...
#ifdef CONFIG_TLK_WDT_STARTUP_ENABLE
    /* set & start timer watchdog. */
    ...
    */
    tlk_adc_anti_aging_mode_for_asm();
    ...
    tlk_plic_reg.feat_en = 0x02;                     /* Vector mode enable bit (VECTORED) of the Feature Enable Register */
#else
    tlk_write_csr(NDS_MTVEC, tlk_trap_entry);
    tlk_clear_csr(mmisc_ctl, 0x02);              /* Disable vectored external plic interrupt */
    tlk_plic_reg.feat_en = 0x00;                     /* Vector mode disable bit (VECTORED) of the Feature Enable Register */
#endif
    ...
    if (init_type) {
        ...
        while (read_sram8(REG_MSPI_STATUS) == 0x80);  //read reg_mspi_status FLD_MSPI_BUSY(0xA3FFFF28[bit7])
        ...
        if ((read_sram8(REG_OTP_STATUS) & 0x01) != 0x00) { //0x80140307[bit0]:OTP_STATUS-busy
            for (volatile uint32_t i = 0; i <= EFUSE_LOAD_AND_FLASH_WAKEUP_LOOP_NUM; i++) {}
        }
    ...
    if (!init_type) {
        ...
        memcpy(_RETENTION_RESET_VMA_START,
            ...
            (uint8_t *)_RETENTION_RESET_VMA_END -
            (uint8_t *)_RETENTION_RESET_VMA_START);

        /* Zero .aes section in sram */
        memset(_AES_VMA_START,
            0,
            (uint8_t *)_AES_VMA_END -
            (uint8_t *)_AES_VMA_START);

        /* Move Data from flash to sram */
        memcpy(_DATA_VMA_START,
            _DATA_LMA_START,
            (uint8_t *)_DATA_VMA_END -
            (uint8_t *)_DATA_VMA_START);

        /* Zero .bss section in sram */
        memset(_BSS_VMA_START,
            0,
            (uint8_t *)_BSS_VMA_END -
            (uint8_t *)_BSS_VMA_START);
    }
}
...
_tlk_attribute_text_sec_noinline_ void tlk_core_init_flash(void)
{
    ...
    memcpy(_RAMCODE_VMA_START,
        _RAMCODE_LMA_START,
        (uint8_t *)_RAMCODE_VMA_END -
        (uint8_t *)_RAMCODE_VMA_START);
