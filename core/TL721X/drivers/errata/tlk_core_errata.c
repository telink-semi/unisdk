 ...
 * usb_set_pin(1) interface (equivalent to usb_set_pin_en) will re-enable the dp_through_swire
...
_tlk_attribute_ram_code_sec_noinline_ void tlk_core_init(uint8_t init_type)
{
    ...
    *(volatile uint8_t*) 0x80100C01 = 0x00; // 0x80100c01 -> 0x00 <7>: 0 swire_usb_dis
    ...
    /*
     * Since the adc reference voltage decreases over time, resulting in inaccurate sampling,
     * invoking this interface can avoid this problem. Since the chip experiences aging as soon as
     * it is powered on, it is recommended to place the anti-aging logic as early as possible to
     * prevent such issues. The A0/A1/A2 versions have aging issues and need to be modified as
     * necessary. the A3 versions, once confirmed to be problem free, should have the logic removed.
    ...
    if (init_type)
    {
        ...
        if ((read_sram8(REG_OTP_STATUS) & 0x01) != 0x00)
        { // 0x80140307[bit0]:OTP_STATUS-busy
            for (volatile uint32_t i = 0; i <= EFUSE_LOAD_AND_FLASH_WAKEUP_LOOP_NUM; i++)
            {
            }
        }
