...
_tlk_attribute_ram_code_sec_noinline_ void tlk_gpio_shutdown_for_asm(void)
{
    ...
    tlk_gpio_reg.port_config[0].function = 0x7F; // Enable SWS input
...
{
    tlk_analog_write_reg8(TLK_AREG_PC_IE, 0);
    tlk_analog_write_reg8(TLK_AREG_PD_IE, 0);
}

_tlk_attribute_ram_code_sec_noinline_ void tlk_gpio_analog_powerdown(void)
{
    tlk_analog_write_reg8(TLK_AREG_PX_PUPD0 + 1,
                          (tlk_analog_read_reg8(TLK_AREG_PX_PUPD0 + 1) & 0x3F) | 0x40);
}

TLK_REGISTER_PRE_INIT(tlk_gpio_analog_powerdown, TLK_INIT_LEVEL_SYSTEM, TLK_INIT_PRIORITY_HIGH)
