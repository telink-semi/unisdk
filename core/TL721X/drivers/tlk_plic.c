...
_tlk_attribute_ram_code_sec_noinline_ void tlk_plic_all_interrupt_save_and_disable(void)
{
...
_tlk_attribute_ram_code_sec_noinline_ void tlk_plic_all_interrupt_restore(void)
{
    /* restore PLIC irq. */
...
_tlk_always_inline _tlk_attribute_ram_code_sec_ void tlk_plic_set_priority(uint32_t src, enum tlk_irq_priority priority)
{
    tlk_plic_reg.irq_priority[src] = priority;
}

_tlk_always_inline _tlk_attribute_ram_code_sec_ void tlk_plic_set_threshold(enum tlk_irq_priority threshold)
{
    ...
    when the mcu sends an interrupt completion message to the PLIC (tlk_plic_interrupt_complete()), the PLIC will restore(Hardware automatic operation) the highest priority number in the preempted priority stack back to the priority threshold register.
...
_tlk_always_inline _tlk_attribute_ram_code_sec_ uint32_t tlk_plic_interrupt_claim(void)
{
    return tlk_plic_reg.irq_claim_compl;
}

_tlk_always_inline _tlk_attribute_ram_code_sec_ void tlk_plic_interrupt_complete(uint32_t src)
{
