...
#define TLK_I2C_CHECK_TIMEOUT(start_tick, timeout_us)                                              \
    if ((tlk_api_micros() - (start_tick)) > (timeout_us))                                          \
...
static inline void tlk_i2c0_clr_irq(enum tlk_i2c0_irq status)
{
...
static inline bool tlk_i2c0_get_irq(enum tlk_i2c0_irq status)
{
    if (status & TLK_I2C0_STRETCH_IRQ)
    {
...
static inline void tlk_i2c_irq_enable(enum tlk_i2c0_irq mask)
{
...
static inline void tlk_i2c_irq_disable(enum tlk_i2c0_irq mask)
{
    if (mask & TLK_I2C0_STRETCH_IRQ)
    {
        tlk_i2c0_reg.control2_bit.slave_stretch_irq_en = 0;
    }
    tlk_i2c0_reg.irq_mask &= ~mask;
}
...
{
    tlk_i2c0_clr_irq(TLK_I2C0_MASTER_NAK_IRQ);
    ...
    tlk_i2c0_clr_irq(TLK_I2C0_TX_BUF_IRQ);
...
static enum tlk_i2c_status tlk_i2c0_master_write(tlk_i2c_address_t id, uint8_t* buffer,
                                                 uint32_t size)
{
    uint32_t start_tick = tlk_api_micros();
    ...
    tlk_dma_start_transfer(tlk_i2c0_dma_config.tx_channel,
                           (uint32_t) buffer,
                           (uint32_t) tlk_i2c0_reg.data_buff,
                           size,
                           TLK_DMA_WORD_WIDTH);
    ...
    if (tlk_i2c0_get_irq(TLK_I2C0_MASTER_NAK_IRQ))
    {
                                                ...
                                                uint32_t size)
{
    ...
    tlk_dma_start_transfer(tlk_i2c0_dma_config.rx_channel,
                           (uint32_t) tlk_i2c0_reg.data_buff,
                           (uint32_t) buffer,
                           size,
                           TLK_DMA_WORD_WIDTH);
    ...
    tlk_i2c0_clr_irq(TLK_I2C0_RX_BUF_IRQ);
    tlk_i2c0_reg.id                         = (id << 1) | TLK_I2C_WRITE_READ_BIT;
    tlk_i2c0_reg.control3_bit.master_nak_en = 1;
...
enum tlk_i2c_status tlk_i2c_configure_pinmux(struct tlk_i2c_device*        dev,
                                             ...
                                             struct tlk_i2c_master_config* config)
{
    ...
    if (dev == &tlk_i2c0)
    {
        ...
        {
            tlk_i2c0_dma_config.tx_channel = tlk_dma_request_chn();
            tlk_dma_configure(tlk_i2c0_dma_config.tx_channel, &tlk_i2c0_dma_config.tx_config);
        }
        if (tlk_i2c0_dma_config.rx_channel == TLK_DMA_NONE)
        {
            tlk_i2c0_dma_config.rx_channel = tlk_dma_request_chn();
            tlk_dma_configure(tlk_i2c0_dma_config.rx_channel, &tlk_i2c0_dma_config.rx_config);
        }

        tlk_i2c_irq_enable(TLK_I2C0_MASTER_NAK_IRQ);
                                            ...
                                            __unused struct tlk_i2c_slave_config* config)
{
    ...
    if (dev == &tlk_i2c0)
    {
        ...
        tlk_i2c_irq_enable(TLK_I2C0_RX_BUF_IRQ | TLK_I2C0_RX_END_IRQ | TLK_I2C0_TX_END_IRQ |
...
_tlk_attribute_ram_code_sec_noinline_ void tlk_i2c_irq_handler(void)
{
#if TLK_IS_ENABLED(CONFIG_TLK_I2C0_MASTER_DMA)
    if (tlk_i2c0_get_irq(TLK_I2C0_MASTER_NAK_IRQ))
    {
        ...
        tlk_dma_abort(tlk_i2c0_dma_config.tx_channel);
    ...
    if (tlk_i2c0_get_irq(TLK_I2C0_SLAVE_WR_IRQ))
    {
        tlk_i2c0_clr_irq(TLK_I2C0_SLAVE_WR_IRQ);
    ...
    if (tlk_i2c0_get_irq(TLK_I2C0_RX_BUF_IRQ))
    {
    ...
    if ((tlk_i2c0_get_irq(TLK_I2C0_RX_END_IRQ)))
    {
        tlk_i2c0_clr_irq(TLK_I2C0_RX_END_IRQ);
        if (tlk_i2c0_reg.buff_count_bit.rx_buf_cnt > 0)
        {
            tlk_i2c0_slave_read(&tlk_i2c0_slave.rx_buffer[tlk_i2c0_slave.rx_count],
    ...
    if (tlk_i2c0_get_irq(TLK_I2C0_TX_END_IRQ))
    {
        tlk_i2c0_clr_irq(TLK_I2C0_TX_END_IRQ);
...
TLK_PLIC_ISR_REGISTER(tlk_i2c_irq_handler, UNISDK_PLIC_IRQ_NUM_I2C)
...
#define TLK_DECLARE_I2C_RESTORE_CONTEXT(num)                                                       \
    ...
    {                                                                                              \
        tlk_i2c_configure_pinmux(&tlk_i2c##num, &tlk_i2c##num.retention->pinmux);                  \
