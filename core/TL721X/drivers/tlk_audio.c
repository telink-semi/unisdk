...
{
    bool pd_val = (tlk_g_chip_version == TLK_CHIP_VERSION_A1); // seems that A1 has reversed pd reg
    TLK_ANALOG_MODIFY(TLK_AREG_PGA_CTRL2, (value.bit.audio_vmid_pd = pd_val;));

    TLK_ANALOG_MODIFY(TLK_AREG_PGA_CTRL0, (value.bit.td_inmute_pga = 0;));
    TLK_ANALOG_MODIFY(TLK_AREG_PGA_CTRL1, (
        value.bit.td_pd_pgabuf = 0;
        value.bit.td_pd_pgaboost = 0;
        value.bit.td_pd_inppga = 0;
        value.bit.td_pd_asdm = 0;
        value.bit.td_pd_bias = 0;
    ));
}
                                      ...
                                      enum tlk_audio_sample_rate  rate)
{
    if (source == TLK_AUDIO_INPUT_DMIC)
    {
         ...
         * problem, so reg_codec_cfg bit[5] need to be configured to adjust the direction. Moreover,
         * this bit is also recommended to be configured for scenarios with less than 32k sampling
         * rate, and in this case, the direction will not be flipped.
         */
...
{
    TLK_ANALOG_MODIFY(TLK_AREG_PGA_CTRL0, (value.bit.td_pgavol_in = pga_gain;));
}
...
static void tlk_audio_set_i2s0_clk(uint16_t div_numerator, uint16_t div_denominator)
{
...
static void tlk_audio_ascl_lerp_en(tlk_audio_ascl_select_e ascl_select)
{
    ...
    {
        TLK_BM_SET(reg_line(ascl_select), FLD_LINE_12);
    }
    else
    {
...
static void tlk_audio_set_ascl_channel(tlk_audio_ascl_select_e ascl_select, bool is_stereo)
{
    if (ascl_select != ASCL0)
    {
        if (!is_stereo)
        {
            TLK_BM_SET(reg_ascl_config(ascl_select), FLD_MONO_12);
        }
        else
        {
            TLK_BM_CLR(reg_ascl_config(ascl_select), FLD_MONO_12);
        }
    }
    else
    {
...
static void tlk_audio_set_ascl_gain(tlk_audio_ascl_select_e ascl_select, uint8_t gain)
{
    if (ascl_select != ASCL0)
    {
        reg_ascl_vol(ascl_select) =
            ((reg_ascl_vol(ascl_select) & (~FLD_VOL_12)) | TLK_MASK_VAL(FLD_VOL_12, gain));
    }
    else
    {
...
void tlk_audio_init(void)
{
    ...
    TLK_BM_SET(reg_rst2, FLD_RST2_AUD);
    TLK_BM_SET(reg_clk_en2, FLD_CLK2_AUD_EN);
...
void tlk_audio_output_configure(struct tlk_audio_output* cfg)
{
    ...
    tlk_audio_set_i2s0_clk(1, tlk_sys_clk.source_clk / 2);
