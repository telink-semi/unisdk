...
static void dp_through_swire_en(bool dp_through_swire)
{
    if (dp_through_swire)
    {
        tlk_swire_reg.control_bit.usb_en = 1;
        tlk_swire_reg.control_bit.usb_en = 1;
    }
    else
    {
        tlk_swire_reg.control_bit.usb_en = 0;
        tlk_swire_reg.control_bit.usb_en = 0;
    }
}
...
static void usb_set_pin(enum tlk_gpio_port port_dm, enum tlk_gpio_port port_dp,
                        enum tlk_gpio_pin pin_dm, enum tlk_gpio_pin pin_dp)
{
    ...
    dp_through_swire_en(1);
}

static void tlk_usb_set_ctr_ep_size(enum tlk_usb_ctrl_ep_size size)
{
    tlk_usb_reg.edp0_size_bit.r_cwptr_mux = size;
}

_tlk_attribute_ram_code_sec_ static void tlk_usb_setup_irq_handler(void)
{
    ...
    {
        TLK_LOG_ERROR(usb_log, "No callback has been set for setup event!");
        return;
    }
}

_tlk_attribute_ram_code_sec_ static void tlk_usb_data_irq_handler(void)
{
    ...
    {
        TLK_LOG_ERROR(usb_log, "No callback has been set for data event!");
        return;
    }
}

_tlk_attribute_ram_code_sec_ static void tlk_usb_eos_irq_handler(void)
{
    ...
    {
        TLK_LOG_ERROR(usb_log, "No callback has been set for endpoint data event!");
        return;
    }
}

_tlk_attribute_ram_code_sec_ static void tlk_usb_suspend_irq_handler(void)
{
    ...
    {
        TLK_LOG_ERROR(usb_log, "No callback has been set for suspend event!");
        return;
    }
}

_tlk_attribute_ram_code_sec_ static void tlk_usb_reset_irq_handler(void)
{
    ...
    {
        TLK_LOG_ERROR(usb_log, "No callback has been set for reset event!");
        return;
    }
}

_tlk_attribute_ram_code_sec_ static void tlk_usb_sof_irq_handler(void)
{
    tlk_usb_clr_irq_status(USB_IRQ_SOF_STATUS);
    uint32_t frame_num = tlk_usb_reg.tstamp;
    if (usb_irq_cb.sof_cb != NULL)
    {
        usb_irq_cb.sof_cb(frame_num);
    }
    else
    {
        TLK_LOG_ERROR(usb_log, "No callback has been set for start of frame event!");
        return;
    }
}
...
TLK_PLIC_ISR_REGISTER(tlk_usb_sof_irq_handler, UNISDK_PLIC_IRQ_NUM_USB_250US_SOF);
...
void tlk_usb_init(void)
{
    ...
    tlk_power_dig_modules_set_state(TLK_POWER_USB, true);

    tlk_reset_reg.reset0_bit.usb    = 1;
    tlk_clock_reg.clock_en0_bit.usb = 1;

    tlk_clock_reg.usb_div = tlk_sys_clk.source_clk / 48;

    tlk_usb_set_ctr_ep_size(SIZE_64_BYTE);

    usb_set_pin(PINMUX_USB_DM_PORT, PINMUX_USB_DP_PORT, PINMUX_USB_DM_PIN, PINMUX_USB_DP_PIN);
}
...
{
    tlk_power_dig_modules_set_state(TLK_POWER_USB, true);
    tlk_clock_reg.clock_en0_bit.usb = 0;
}
...
void tlk_usb_enable_manual_interrupt(int m)
{
    ...
    tlk_usb_set_irq_mask(USB_IRQ_RESET_MASK | USB_IRQ_SUSPEND_MASK | USB_IRQ_SETUP_MASK |
                         USB_IRQ_DATA_MASK | USB_IRQ_SOF_MASK);
}
...
_tlk_attribute_ram_code_sec_ void tlk_usb_clr_irq_status(enum tlk_usb_irq_status status)
{
...
{
    TLK_ANALOG_MODIFY(TLK_AREG_POWER6, (value.bit.dp_pullup_res_en_b = !(en);));
}
...
{
    if (tlk_usb_reg.usb_min1_bit.r_edps_sm_map_en)
    {
        tlk_usb_reg.edps_logic_en |= ep;
    }
    else
    {
