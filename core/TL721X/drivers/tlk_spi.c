...
/* --- config dependencies --- */
static void tlk_lspi_set_pin_mux(struct tlk_gpio_port_pin port_pin, enum tlk_gpio_mux function)
{
    if (function == TLK_GPIO_MUX_LSPI_CS)
    {
...
{
    tlk_spi_rx_set_irq_trig_cnt(module,
                                4); // default gspi & lspi burst1(N=1) ,rx_fifo trig_cnt=4*N=4.
    tlk_spi_tx_set_irq_trig_cnt(module, (module == TLK_SPI0) ? 8 : 4);
}
...
_tlk_attribute_ram_code_sec_ static void tlk_spi_irq_set_mask(enum tlk_spi_module     module,
                                                              enum tlk_spi_irq_status status)
{
    tlk_spi_reg(module).control0 |= status;
}

_tlk_attribute_ram_code_sec_ static uint8_t tlk_spi_irq_get_status(enum tlk_spi_module       module,
...
static bool tlk_spi_tx_fifo_is_full(enum tlk_spi_module module)
{
...
tlk_spi_write(struct tlk_spi_device* dev, const uint8_t* buffer, uint32_t size)
{
    ...
    {
        TLK_WAIT_FOR_TRUE_OR_TIMEOUT((!tlk_spi_tx_fifo_is_num_word(dev->module)),
                                     (TLK_SPI_TIMEOUT_US),
    ...
    {
        TLK_WAIT_FOR_TRUE_OR_TIMEOUT((!tlk_spi_tx_fifo_is_full(dev->module)),
                                     (TLK_SPI_TIMEOUT_US),
                                     (return TLK_SPI_TIMEOUT;))
...
{
    if (tlk_spi_irq_get_status(dev->module, TLK_SPI_INT_STATUS_SLAVE_CMD_INT_STUS))
    {
        tlk_spi_irq_clr_status(dev->module, TLK_SPI_INT_STATUS_SLAVE_CMD_INT_STUS);
...
enum tlk_spi_status tlk_spi_pinmux_configure(struct tlk_spi_device* dev, struct tlk_spi_pinmux* mux)
{
    ...
    {
        tlk_lspi_set_pin_mux(mux->cs, TLK_GPIO_MUX_LSPI_CS);
        tlk_lspi_set_pin_mux(mux->sck, 0);
        tlk_lspi_set_pin_mux(mux->mosi, 0);
        tlk_lspi_set_pin_mux(mux->miso, 0);
    }
...
enum tlk_spi_status tlk_spi_master_configure(struct tlk_spi_device* dev, struct tlk_spi_config* cfg)
{
    ...
    if (dev->module == TLK_SPI0)
    {
        tlk_reset_reg.reset0_bit.lspi    = 1;
        tlk_clock_reg.clock_en0_bit.lspi = 1;

        // LSPI clock source select pll_clk when div_clock <= 15,
        // select xtl 24m when div_clock > 15.
        uint16_t div_clock = tlk_sys_clk.source_clk * 1000000 / cfg->frequency;

        if (div_clock > 15)
        {
            tlk_clock_reg.lspi_mode_bit.lspi_mod = 24 * 1000000 / cfg->frequency;
            tlk_clock_reg.lspi_mode_bit.lspi_div_in_sel =
                TLK_CLOCK_LSPI_MODE_LSPI_DIV_IN_SEL_24M_XTAL;
        }
        else
        {
            tlk_clock_reg.lspi_mode_bit.lspi_mod        = div_clock;
            tlk_clock_reg.lspi_mode_bit.lspi_div_in_sel = TLK_CLOCK_LSPI_MODE_LSPI_DIV_IN_SEL_PLL;
        }
    }
    ...
    if (dev->module == TLK_SPI1)
    {
        ...
        if (div_clock > 255)
        {
            ...
            tlk_clock_reg.gspi_mode_bit.gspi_div_in_sel =
                TLK_CLOCK_GSPI_MODE_GSPI_DIV_IN_SEL_24M_XTAL;
        }
        else
        {
    ...
    tlk_spi_tx_set_irq_trig_cnt(dev->module, dev->module == TLK_SPI0 ? 16 : 4);
...
{
    // set the SPI clock to 48m by splitting the frequency of the pll.
    if (dev->module == TLK_SPI0)
    {
        tlk_reset_reg.reset0_bit.lspi    = 1;
        tlk_clock_reg.clock_en0_bit.lspi = 1;

        tlk_clock_reg.lspi_mode_bit.lspi_mod        = tlk_sys_clk.source_clk / 48;
        tlk_clock_reg.lspi_mode_bit.lspi_div_in_sel = TLK_CLOCK_LSPI_MODE_LSPI_DIV_IN_SEL_PLL;
    }
