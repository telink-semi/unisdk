...
enum tlk_spi_trans_mode
{
    ...
    TLK_SPI_MODE_DUMMY_WRITE_AND_READ, //dummy,write and read at the same.must enable CmdEn.
...
/* --- config dependencies --- */
void tlk_lspi_set_pin_mux(struct tlk_gpio_port_pin port_pin, enum tlk_spi_gpio_function function)
{
    if (function == TLK_LSPI_CS) {
...
{
    if (module == TLK_SPI0) {
        tlk_lspi_set_pin_mux(config->pins.cs_port_pin, TLK_LSPI_CS);
        tlk_lspi_set_pin_mux(config->pins.sck_port_pin, 0);
        tlk_lspi_set_pin_mux(config->pins.mosi_port_pin, 0);
        tlk_lspi_set_pin_mux(config->pins.miso_port_pin, 0);
    }
...
{
    tlk_spi_rx_irq_trig_cnt(module, 4); //default gspi & lspi burst1(N=1) ,rx_fifo trig_cnt=4*N=4.
    tlk_spi_tx_irq_trig_cnt(module, (module == TLK_SPI0) ? 4 : 8);
}
...
{
    if (mode == TLK_SPI_3_LINE_MODE) {
        TLK_BM_SET(reg_spi_ctrl3(module), FLD_SPI_3LINE); // spi_3line_mode_en
        reg_spi_ctrl1(module) = ((reg_spi_ctrl1(module) & (~FLD_SPI_DATA_LANE)));
    } else {
        reg_spi_ctrl1(module) = ((reg_spi_ctrl1(module) & (~FLD_SPI_DATA_LANE)) | (mode & 0x3));
...
{
    TLK_BM_SET(reg_spi_ctrl0(module), status);
}
...
static inline void tlk_spi_tx_fifo_clr(enum tlk_spi_module module)
{
    ...
    TLK_BM_CLR(reg_spi_ctrl3(module), FLD_SPI_DMATX_SOF_CLRTXF_EN);
...
static inline void tlk_spi_rx_fifo_clr(enum tlk_spi_module module)
{
    ...
    TLK_BM_CLR(reg_spi_ctrl3(module), FLD_SPI_DMARX_EOF_CLRRXF_EN);
...
void tlk_spi_configure(struct tlk_spi_device* device, struct tlk_spi_config* config)
{
    ...
    if (config->role == TLK_SPI_MASTER) {
        if (device->module == TLK_SPI0) {
            ...
            if (config->div_clock > 15) { //LSPI clock source select pll_clk when div_clock <= 15,select xtl 24m when div_clock > 15.
                // tlk_clock_bbpll_config(TLK_PLL_CLK); //should be already enabled 
                reg_lspi_clk_set = ((FLD_LSPI_CLK_MOD & (unsigned char)(config->div_clock * 24 / (tlk_sys_clk.pll_clk))) | (1 << 4));
            } else {
                reg_lspi_clk_set = ((FLD_LSPI_CLK_MOD & (unsigned char)config->div_clock) | (2 << 4));
            }
        }
        ...
        if (device->module == TLK_SPI1) {
            ...
            if (config->div_clock > 255) { //GSPI clock source select pll_clk when div_clock <= 255,select xtl 24m when div_clock > 255.
                // tlk_clock_bbpll_config(TLK_PLL_CLK); //should be already enabled
                reg_gspi_clk_set = ((FLD_GSPI_CLK_MOD & (unsigned char)(config->div_clock * 24 / (tlk_sys_clk.pll_clk))) | (1 << 8));
            } else {
                reg_gspi_clk_set = ((FLD_GSPI_CLK_MOD & (unsigned char)config->div_clock) | (2 << 8));
            }
        }

        reg_spi_ctrl3(device->module) |= (FLD_SPI_MASTER_MODE | FLD_SPI_AUTO_HREADY_EN); //master
    ...
    if (config->role == TLK_SPI_SLAVE) {
        if (device->module == TLK_SPI0) { //set the SPI clock to 48m by splitting the frequency of the pll.
            reg_rst0 |= FLD_RST0_LSPI;
            reg_clk_en0 |= FLD_CLK0_LSPI_EN;
            
            reg_lspi_clk_set = ((FLD_LSPI_CLK_MOD & (unsigned char)(tlk_sys_clk.pll_clk / 48)) | (2 << 4));
        }

        if (device->module == TLK_SPI1) { //set the SPI clock to 48m by splitting the frequency of the pll.
            reg_rst1 |= FLD_RST1_GSPI;
            reg_clk_en1 |= FLD_CLK1_GSPI_EN;
            
            reg_gspi_clk_set = ((FLD_GSPI_CLK_MOD & (unsigned char)(tlk_sys_clk.pll_clk / 48)) | (2 << 8));
        }

        reg_spi_ctrl3(device->module) &= (~FLD_SPI_MASTER_MODE);                                         //slave.
        reg_spi_ctrl3(device->module) |= FLD_SPI_AUTO_HREADY_EN;                                         //slave.
        reg_spi_ctrl3(device->module) = ((reg_spi_ctrl3(device->module) & (~FLD_SPI_WORK_MODE)) | (config->mode << 2)); // select SPI mode, support four modes.
...
// tlk_spi_rx_irq_trig_cnt in spi_master_init
// tlk_spi_tx_irq_trig_cnt in spi_master_init