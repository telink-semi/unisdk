...
#define TLK_AREG_PM_GPIO_WAKEUP_POLARITY(port)  (UNISDK_PM_REG_WAKEUP_POLARITY + (port))
#define TLK_AREG_PM_GPIO_WAKEUP_ENABLE(port)    (UNISDK_PM_REG_WAKEUP_ENABLE + (port))

#if IS_ENABLED(CONFIG_TLK_PM_RAM_RETENTION_ENABLE)
uint8_t tlk_pm_sleep_mode_to_reg_value[] = {
    IF_ENABLED(CONFIG_TLK_PM_SUSPEND, (UNISDK_PM_BFLD_SUSPEND_MODE,))
    IF_ENABLED(CONFIG_TLK_PM_DEEP_SLEEP, (UNISDK_PM_BFLD_DEEP_SLEEP_MODE,))
    IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_32K, (UNISDK_PM_BFLD_RETENTION_MODE_SIZE_32K,))
    IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_64K, (UNISDK_PM_BFLD_RETENTION_MODE_SIZE_64K,))
    IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_128K, (UNISDK_PM_BFLD_RETENTION_MODE_SIZE_128K,))
    IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_256K, (UNISDK_PM_BFLD_RETENTION_MODE_SIZE_256K,))
};
#endif
...
extern _tlk_attribute_ram_code_sec_noinline_  enum tlk_pm_sleep_status tlk_pm_sleep_impl(enum tlk_pm_sleep_mode mode, uint32_t duration_ms);
...
{
    return tlk_analog_read_reg8(areg_aon_0x64);
}
...
void tlk_pm_clear_wakeup_sources(void)
{
    ...
    tlk_analog_write_reg32(TLK_AREG_PM_GPIO_WAKEUP_ENABLE(0), 0);
    tlk_analog_write_reg16(TLK_AREG_PM_GPIO_WAKEUP_ENABLE(4), 0);
#endif
}

_tlk_attribute_ram_code_sec_noinline_ enum tlk_pm_sleep_status tlk_pm_sleep(enum tlk_pm_sleep_mode mode, uint32_t duration_ms)
{
    ...
    if (tlk_pm_sleep_info.wakeup_sources & TLK_PM_WAKEUP_SOURCE_TIMER) {
        ...
        switch (mode)
        {
            ...
            case TLK_PM_SLEEP_MODE_SUSPEND:
                if (duration_ms < CONFIG_TLK_PM_SUSPEND_MIN_DURATION_MS )
            ...
            case TLK_PM_SLEEP_MODE_DEEP_RETENTION:
                if (duration_ms < CONFIG_TLK_PM_DEEP_RETENTION_MIN_DURATION_MS )
                return TLK_PM_SLEEP_TOO_SHORT;
            break;
            #endif
    ...
    if (mode == TLK_PM_SLEEP_MODE_DEEP_SLEEP  
        ...
        if (tlk_prevent_sleep()) {
            #if IS_ENABLED(CONFIG_TLK_PM_SUSPEND) && IS_ENABLED(CONFIG_TLK_PM_ALLOW_FALLBACK_TO_SUSPEND)
    ...
    if (did_fallback_to_suspend) {
        return TLK_PM_SLEEP_SUSPEND_FALLBACK;
    }
...
enum tlk_pm_sleep_status tlk_pm_set_gpio_wakeup(enum tlk_gpio_port port, enum tlk_gpio_pin pin, enum tlk_pm_gpio_wakeup_level polarity)
{
    ...
    if (port > UNISDK_GPIO_PORT_COUNT) {
        return TLK_PM_SLEEP_UNSUPPORTED;
    }

    value = tlk_analog_read_reg8(TLK_AREG_PM_GPIO_WAKEUP_POLARITY(port));
    if (polarity) {
        TLK_BM_CLR(value, pin);
    } else {
        TLK_BM_SET(value, pin);
    }
    tlk_analog_write_reg8(TLK_AREG_PM_GPIO_WAKEUP_POLARITY(port), value);

    value = tlk_analog_read_reg8(TLK_AREG_PM_GPIO_WAKEUP_ENABLE(port));
    TLK_BM_SET(value, pin);
    tlk_analog_write_reg8(TLK_AREG_PM_GPIO_WAKEUP_ENABLE(port), value);
