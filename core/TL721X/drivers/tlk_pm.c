        ...
        TLK_IF_ENABLED(CONFIG_TLK_PM_DEEP_SLEEP, ({.raw = UNISDK_PM_BFLD_DEEP_SLEEP_MODE}, ))
            TLK_IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_32K,
                ...
                TLK_IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_64K,
                    ...
                    TLK_IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_128K,
                                   ({.raw = UNISDK_PM_BFLD_RETENTION_MODE_SIZE_128K}, ))
                        TLK_IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_256K,
                                       ({.raw = UNISDK_PM_BFLD_RETENTION_MODE_SIZE_256K}, ))};

/* Helper functions */
extern void tlk_context_save(void);
extern _tlk_attribute_ram_code_sec_noinline_ enum tlk_pm_sleep_status
tlk_pm_sleep_impl(enum tlk_pm_sleep_mode mode, struct tlk_pm_sleep_tick* duration_ticks);

_tlk_attribute_ram_code_sec_ enum tlk_pm_wakeup_source tlk_pm_get_desired_wakeup_source(void)
{
...
/* Shared core: works entirely in native stimer ticks so it never has to multiply a caller-supplied
 * duration by a tick rate - that multiplication (and its overflow risk) is done once, by each
 * public wrapper below, with a 64-bit intermediate. */
_tlk_attribute_ram_code_sec_noinline_ enum tlk_pm_sleep_status
tlk_pm_sleep(enum tlk_pm_sleep_mode mode, struct tlk_pm_sleep_tick duration_ticks)
{
    enum tlk_pm_sleep_status status                  = TLK_PM_SLEEP_OK;
    bool                     did_fallback_to_suspend = false;

    /* Determine whether timer wakeup is used */
    if (duration_ticks.ticks == UINT32_MAX)
    {
    ...
    if (tlk_pm_sleep_info.wakeup_sources & TLK_PM_WAKEUP_SOURCE_TIMER)
    {
        ...
        if (duration_ticks.ticks > UNISDK_PM_MAX_SLEEP_DURATION_TIMER_TICKS)
        {
        ...
        case TLK_PM_SLEEP_MODE_SUSPEND:
            if (duration_ticks.ticks <=
                (CONFIG_TLK_PM_SUSPEND_WAKEUP_COST_MS + CONFIG_TLK_PM_SUSPEND_MIN_DURATION_MS) *
                    duration_ticks.tick_base / 1000)
                return TLK_PM_SLEEP_TOO_SHORT;
            else
                duration_ticks.ticks -=
                    CONFIG_TLK_PM_SUSPEND_WAKEUP_COST_MS * (duration_ticks.tick_base / 1000);
            break;
#endif

#if TLK_IS_ENABLED(CONFIG_TLK_PM_DEEP_SLEEP)
        case TLK_PM_SLEEP_MODE_DEEP_SLEEP:
            if (duration_ticks.mode <
                CONFIG_TLK_PM_DEEP_SLEEP_MIN_DURATION_MS * (duration_ticks.tick_base / 1000))
                return TLK_PM_SLEEP_TOO_SHORT;
            break;
#endif
...
#error "Minimal deep retention time can not be less than wakeup cost"
        ...
        case TLK_PM_SLEEP_MODE_DEEP_RETENTION:
            if (duration_ticks.ticks <= (CONFIG_TLK_PM_DEEP_RETENTION_WAKEUP_COST_MS +
                                         CONFIG_TLK_PM_DEEP_RETENTION_MIN_DURATION_MS) *
                                            (duration_ticks.tick_base / 1000))
                return TLK_PM_SLEEP_TOO_SHORT;
            else
                duration_ticks.ticks -=
                    CONFIG_TLK_PM_DEEP_RETENTION_WAKEUP_COST_MS * (duration_ticks.tick_base / 1000);
            break;
#endif
...
#if TLK_IS_ENABLED(CONFIG_TLK_PM_SUSPEND)
    case TLK_PM_SLEEP_MODE_SUSPEND:
        tlk_perform_before_suspend();
        status = tlk_pm_sleep_impl(mode, &duration_ticks);
        tlk_perform_after_suspend();
        break;
#endif
