...
static void tlk_adc_power_on(void)
{
    ...
    /*
     * Perform a reset operation when the adc is powered up, which will restart the digital state
     * machine and clear the previous state. Confirm with digital circuit colleagues that performing
     * an ADC reset at power-up helps to improve the operational stability of the ADC. (updated by
     * bolong.zhang, confirmed by shiyi.wu at 202503013)
     */
    tlk_adc_reset();
...
static inline void tlk_adc_set_scale_factor(enum tlk_adc_channel  chn,
                                            enum tlk_adc_prescale pre_scale)
{
    tlk_adc_reg.channel_set_state_bit[chn].sel_ai_scale = pre_scale;
}

static inline void tlk_adc_set_state_length(enum tlk_adc_channel chn, uint16_t r_max_mc,
                                            uint8_t r_max_s)
{
