#ifndef TLK_TLK_ADC_REGISTERS_H_
#define TLK_TLK_ADC_REGISTERS_H_

#include <stdint.h>
#include <common/include/tlk_bit.h>

typedef enum tlk_adc_config1_e {
    TLK_ADC_CONFIG1_MODE = TLK_BIT(5),
} tlk_adc_config1_e;

typedef enum tlk_adc_config2_e {
    TLK_ADC_CONFIG2_M_CHANNEL_EN    = TLK_BIT(0),
    TLK_ADC_CONFIG2_L_CHANNEL_EN    = TLK_BIT(1),
    TLK_ADC_CONFIG2_R_CHANNEL_EN    = TLK_BIT(2),
    TLK_ADC_CONFIG2_RX_DMA_EN       = TLK_BIT(4),
    TLK_ADC_CONFIG2_CLK_EN          = TLK_BIT(5),
    TLK_ADC_CONFIG2_RX_INTERRUPT_EN = TLK_BIT(6),
    TLK_ADC_CONFIG2_TRIG_MODE       = TLK_BIT(7),
} tlk_adc_config2_e;

typedef enum tlk_adc_soft_control_e {
    TLK_ADC_SOFT_CONTROL_IRQ_RX_STATUS = TLK_BIT(0),
    TLK_ADC_SOFT_CONTROL_FIFO_CLR      = TLK_BIT(1),
    TLK_ADC_SOFT_CONTROL_TRIG_START    = TLK_BIT(2),
    TLK_ADC_SOFT_CONTROL_SOFT_START    = TLK_BIT(7),
} tlk_adc_soft_control_e;

typedef enum tlk_adc_sample_times_e {
    TLK_ADC_SAMPLE_TIMES_PEM_TASK_EN   = TLK_BIT(0),
    TLK_ADC_SAMPLE_TIMES_PEM_EVENT0_EN = TLK_BIT(1),
    TLK_ADC_SAMPLE_TIMES_PEM_EVENT1_EN = TLK_BIT(2),
} tlk_adc_sample_times_e;
...
typedef struct __attribute__((packed)) {
    ...
    union {
        ...
        struct {
            enum tlk_adc_config1_e __attribute__((packed)) config1 : 8;
        }; // address: 0x801401e9, offset: 0x29
    };
    union {
        struct {
        ...
        struct {
            enum tlk_adc_config2_e __attribute__((packed)) config2 : 8;
        }; // address: 0x801401ea, offset: 0x2a
    };
    union {
        struct {
    ...
    union {
        ...
        struct {
            enum tlk_adc_soft_control_e __attribute__((packed)) soft_control : 8;
    ...
    union {
        ...
        struct {
            enum tlk_adc_sample_times_e __attribute__((packed)) sample_times : 8;
        }; // address: 0x801401f1, offset: 0x31
    };
} tlk_adc_reg_t;
