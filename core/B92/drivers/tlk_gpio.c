...
void tlk_gpio_enable(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
...
{
    TLK_BM_CLR(tlk_gpio_reg.port_config[port].irq_en, pin);
}

void tlk_gpio_irq_trigger_configure(enum tlk_gpio_port port, enum tlk_gpio_pin pin, enum tlk_gpio_intr_trigger trigger)
{
    switch (trigger) {
...
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_F_ENABLED)
        if (port == TLK_GPIO_PORT_F) {
            base_ana_reg = UNISDK_GPIO_AREG_PF_PUPD + ((pin & 0xf0) ? 1 : 0);
        } else 
#endif
        {
...
void tlk_gpio_irq_configure(enum tlk_gpio_port port, enum tlk_gpio_pin pin, enum tlk_gpio_intr_trigger trigger)
{
    ...
    tlk_gpio_irq_trigger_configure(port, pin, trigger);
...
TLK_PLIC_ISR_REGISTER(tlk_gpio_irq_handler, UNISDK_PLIC_IRQ_NUM_GPIO)
...
static bool tlk_gpio_prevent_sleep(void) 
{
    for (int32_t i = 0; i < UNISDK_GPIO_PORT_COUNT; i++) { 
        if (tlk_gpio_reg.port_config[i].function & ~tlk_gpio_reg.port_config[i].output_en & tlk_gpio_reg.port_config[i].output) {
            return true;
        }
    }
...
{
    for (int32_t port = 0; port <= 5; port++) {
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_F_ENABLED)
        if (port == TLK_GPIO_PORT_F){
            tlk_gpio[port]->retention.pupd = tlk_analog_read_reg16(UNISDK_GPIO_AREG_PF_PUPD);
        } else
#endif
        {
...
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_D_ENABLED)
        if (port == TLK_GPIO_PORT_D) {
            tlk_gpio[port]->retention.input_enable = tlk_analog_read_reg8(UNISDK_GPIO_AREG_PD_IE);
        } else 
#endif
        {
...
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_F_ENABLED)
        if (port == TLK_GPIO_PORT_F) {
            tlk_analog_write_reg16(UNISDK_GPIO_AREG_PF_PUPD, tlk_gpio[port]->retention.pupd);
        } else
#endif
        {
...
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_D_ENABLED)
        if (port == TLK_GPIO_PORT_D) {
            tlk_analog_write_reg8(UNISDK_GPIO_AREG_PD_IE, tlk_gpio[port]->retention.input_enable);
        } else 
#endif
        {
