...
static void tlk_adc_set_diff_input(uint8_t p_ain, uint8_t n_ain)
{
...
static void tlk_adc_set_scale_factor(enum tlk_adc_prescale pre_scale)
{
    TLK_ANALOG_MODIFY(TLK_AREG_ADC_AIN_SCALE, (value.bit.sel_ain_scale = pre_scale;));
}

static void tlk_adc_set_state_length(uint16_t r_max_mc, uint8_t r_max_s)
{
...
static void tlk_adc_set_tsample_cycle(enum tlk_adc_sample_cycle sample_cycle)
{
    union tlk_areg_adc_tsample_m reg = {.bit.tsample_cycle_m = sample_cycle};
    TLK_ANALOG_WRITE(TLK_AREG_ADC_TSAMPLE_M, reg);
}

static void tlk_adc_set_chn_en(void)
{
...
static void tlk_audio_sel_data_fifo_input_path(tlk_audio_fifo_chn_e fifo_chn, uint8_t ain_sel)
{
    reg_audio_sel(fifo_chn) = (reg_audio_sel(fifo_chn) & (~FLD_AUDIO_AIN_SEL)) | (ain_sel);
}

static void tlk_audio_on_power(void)
{
...
static void tlk_adc_set_dma_config(enum tlk_dma_chn chn)
{
    tlk_audio_on_power();
    {
        tlk_audio_sel_data_fifo_input_path(
...
static void tlk_audio_reset_fifo(tlk_audio_fifo_chn_e fifo_chn)
{
...
{
    tlk_audio_reset_fifo(TLK_AUDIO_FIFO);
...
enum tlk_adc_status tlk_adc_read(uint16_t* buffer, uint32_t size)
{
    ...
    {
        tlk_dma_chn_transfer_configure(tlk_adc_config.dma_channel,
