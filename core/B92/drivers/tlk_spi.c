...
static inline void tlk_spi_set_transmode(enum tlk_spi_module               module,
...
{
    if (tlk_spi_get_irq_status(dev->module, TLK_SPI_INT_STATUS0_SLAVE_CMD_INT_STUS))
    {
        tlk_spi_clr_irq_status(dev->module, TLK_SPI_INT_STATUS0_SLAVE_CMD_INT_STUS);
...
TLK_PLIC_ISR_REGISTER(tlk_lspi_irq_handler, UNISDK_PLIC_IRQ_NUM_LSPI);
...
enum tlk_spi_status tlk_spi_master_configure(struct tlk_spi_device* dev, struct tlk_spi_config* cfg)
{
    ...
    tlk_spi_rx_irq_trig_cnt(dev->module, 4);
    tlk_spi_tx_irq_trig_cnt(dev->module, 4);
