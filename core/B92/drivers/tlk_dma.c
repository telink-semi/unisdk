...
#define convert_ram_addr_cpu2bus(addr)                                                             \
    (uint32_t) (addr) < 0xc0000 ? ((uint32_t) (addr) + 0xc0180000) : (uint32_t) (addr)
...
{
    tlk_dma_reg.chn[chn].control_bit.tc_irq_en  = 0;
    tlk_dma_reg.chn[chn].control_bit.err_irq_en = 0;
    tlk_dma_reg.chn[chn].control_bit.abt_irq_en = 0;
}

static void tlk_dma_irq_disable_all(enum tlk_dma_chn chn)
{
    tlk_dma_reg.chn[chn].control_bit.tc_irq_en  = 1;
    tlk_dma_reg.chn[chn].control_bit.err_irq_en = 1;
    tlk_dma_reg.chn[chn].control_bit.abt_irq_en = 1;
}
...
void tlk_dma_chn_configure(enum tlk_dma_chn chn, struct tlk_dma_config* config)
{
    tlk_dma_reg.chn[chn].control =
        (tlk_dma_reg.chn[chn].control & (~TLK_BIT_RNG(4, 31))) | ((*(uint32_t*) config) << 4);
                                    ...
                                    uint32_t size)
{
    ...
    tlk_dma_reg.chn[chn].src_addr           = (uint32_t) convert_ram_addr_cpu2bus(src_addr);
    tlk_dma_reg.chn[chn].dst_addr           = (uint32_t) convert_ram_addr_cpu2bus(dst_addr);
...
{
    tlk_dma_reg.chn[chn].control_bit.chn_en = 0;
    tlk_dma_reg.chn[chn].ll_pointer =
        (uint32_t) (head == NULL ? NULL : (void*) (convert_ram_addr_cpu2bus(head->next)));

    TLK_IF_ENABLED(CONFIG_TLK_DMA_PM_DEVICE, (tlk_dma_retention_lls[chn] = head;))

    if (head != NULL)
    {
        tlk_dma_reg.chn[chn].src_addr = (uint32_t) convert_ram_addr_cpu2bus(head->src_addr);
        tlk_dma_reg.chn[chn].dst_addr = (uint32_t) convert_ram_addr_cpu2bus(head->dst_addr);
        tlk_dma_reg.chn[chn].size =
            tlk_dma_cal_transfer_size(head->length, tlk_dma_reg.chn[chn].control_bit.src_width);
    }

    for (struct tlk_dma_ll_node* current = head; current != NULL;)
    {
        // save the original CPU pointer beforehand
        struct tlk_dma_ll_node* next_cpu_ptr = current->next;
        ...
        current->dst_addr = (void*) (convert_ram_addr_cpu2bus(current->dst_addr));
        current->src_addr = (void*) (convert_ram_addr_cpu2bus(current->src_addr));
        current->next     = (struct tlk_dma_ll_node*) (next_cpu_ptr == NULL)
                                ? NULL
                                : (void*) (convert_ram_addr_cpu2bus(next_cpu_ptr));

        if (next_cpu_ptr == head)
        {
            break;
        }

        current = next_cpu_ptr;
    }
}
