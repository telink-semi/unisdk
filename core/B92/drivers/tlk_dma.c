...
{
    tlk_dma_reg.chn[chn].control = (tlk_dma_reg.chn[chn].control & (~TLK_BIT_RNG(4, 31)) ) | ((*(uint32_t *)config) << 4); // dma config
...
void tlk_dma_start_transfer(enum tlk_dma_chn chn, uint32_t src_addr, uint32_t dst_addr, uint32_t size, enum tlk_dma_transfer_width width)
{
    ...
    tlk_dma_reg.chn[chn].src_addr = (uint32_t)convert_ram_addr_cpu2bus(src_addr); // dma_set_address
    tlk_dma_reg.chn[chn].dst_addr = (uint32_t)convert_ram_addr_cpu2bus(dst_addr); // dma_set_address
