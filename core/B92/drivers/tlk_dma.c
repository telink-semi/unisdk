...
#define TLK_DECLARE_DMA_FUNCTION(num)                                                              \
    TLK_IF_ENABLED(TLK_CONFIG_WITH_DMA(TLK_DMA_FUNCTION(num)), (TLK_DMA_FUNCTION_##num, ))

enum tlk_dma_function_channels
{
    TLK_FOR_CALL(UNISDK_DMA_FUNCTION_COUNT, TLK_DECLARE_DMA_FUNCTION) TLK_DMA_RESERVED_COUNT
};
...
#define TLK_DECLARE_DMA_CHANNEL_WITH_RESERVATION(num)                                              \
    .tlk_dma##num = TLK_IF_ENABLED_ELSE(TLK_CONFIG_WITH_DMA(TLK_DMA_RESERVATION(num)), (1), (0)),

static struct
{
    TLK_FOR_CALL(UNISDK_DMA_CHN_COUNT, TLK_DECLARE_DMA_CHANNEL_FOR_RESERVATION)
} tlk_dma_channels = {TLK_FOR_CALL(UNISDK_DMA_CHN_COUNT, TLK_DECLARE_DMA_CHANNEL_WITH_RESERVATION)};
/* ==== CHANNELS MANAGEMENT ==== */
...
{
    tlk_dma_reg.chn[chn].control = (tlk_dma_reg.chn[chn].control & (~TLK_BIT_RNG(4, 31))) |
                                   ((*(uint32_t*) config) << 4); // dma config
                            ...
                            uint32_t size, enum tlk_dma_transfer_width width)
{
    ...
    tlk_dma_reg.chn[chn].src_addr =
        (uint32_t) convert_ram_addr_cpu2bus(src_addr); // dma_set_address
    tlk_dma_reg.chn[chn].dst_addr =
        (uint32_t) convert_ram_addr_cpu2bus(dst_addr); // dma_set_address
    tlk_dma_reg.chn[chn].size_bit.tran_size =
        (size + bytes_per_transfer - 1) / bytes_per_transfer;                // dma_set_size
