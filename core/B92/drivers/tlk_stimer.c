...
#if TLK_IS_ENABLED(CONFIG_TLK_SYSTEM_TIMER_IRQ_ENABLE)
#include "core/include/tlk_plic.h"
#include "properties/tlk_plic.h"
#endif

#if TLK_IS_ENABLED(CONFIG_TLK_STIMER_PM_DEVICE)

struct tlk_system_timer_retention_data
{
    uint8_t  reg_irq_mask;
    uint32_t reg_irq_count;
};

static struct tlk_system_timer_retention_data static_ret_data;
#endif
 ...
 */
_tlk_attribute_ram_code_sec_ enum tlk_stimer_state_e tlk_stimer_get_state(void)
{
 ...
 * @return    none.
 */
_tlk_attribute_ram_code_sec_ void tlk_stimer_set_state(tlk_stimer_state_e state)
{
 ...
 * @return      none.
 */
void tlk_stimer_set_irq_mask(tlk_stimer_irq_mask_e mask)
{
 ...
 * @return      none.
 */
void tlk_stimer_clr_irq_mask(tlk_stimer_irq_mask_e mask)
{
    tlk_stimer_reg.irq_mask &= (~mask);
}
 ...
 * @return      none.
 */
void tlk_stimer_clr_irq_status(tlk_stimer_irq_status_e status)
{
 ...
 */
tlk_stimer_irq_status_e tlk_stimer_get_irq_status(tlk_stimer_irq_status_e status)
{
...
#if TLK_IS_ENABLED(CONFIG_TLK_SYSTEM_TIMER_IRQ_ENABLE)
/**
 * @brief     Default stime irq handler.
 * @return    None.
 */
void tlk_stime_default_handler(void)
{
#if TLK_IS_ENABLED(CONFIG_TLK_DEBUG_PRINT)
    tlk_api_print("Warning! Default System Timer IRQ Handler!\n");
#endif
}

void (*tlk_stimer_callback)(void) = tlk_stime_default_handler;

void tlk_stimer_register_callback(void (*cb)(void))
{
    tlk_stimer_callback = cb;
}

_tlk_attribute_ram_code_sec_ void tlk_stimer_irq_handler(void)
{
    tlk_stimer_callback();
}

TLK_PLIC_ISR_REGISTER(tlk_stimer_irq_handler, UNISDK_PLIC_IRQ_NUM_STIMER)

#endif

#if TLK_IS_ENABLED(CONFIG_TLK_STIMER_PM_DEVICE)

static void tlk_stimer_save_context(void)
{
    static_ret_data.reg_irq_mask  = tlk_stimer_reg.irq_mask;
    static_ret_data.reg_irq_count = tlk_stimer_reg.irq_time;
}

static void tlk_stimer_restore_context(void)
{
    tlk_stimer_reg.irq_mask = static_ret_data.reg_irq_mask;
    tlk_stimer_reg.irq_time = static_ret_data.reg_irq_count;
}

TLK_REGISTER_BEFORE_SLEEP(tlk_stimer_save_context, TLK_INIT_LEVEL_DRIVER, TLK_INIT_PRIORITY_NORMAL)
TLK_REGISTER_AFTER_SLEEP(tlk_stimer_restore_context, TLK_INIT_LEVEL_DRIVER,
                         TLK_INIT_PRIORITY_NORMAL)
#endif
