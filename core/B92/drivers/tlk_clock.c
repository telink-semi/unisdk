...
_tlk_attribute_ram_code_sec_ static uint8_t tlk_clock_get_pll_reg_value(uint16_t pll_clk)
{
    switch (pll_clk)
    {
    case 48:
...
_tlk_attribute_ram_code_sec_ static void tlk_clock_pll_configure(uint16_t frequency)
{
...
{
    if (cfg.use_external_cap)
    {
        TLK_ANALOG_MODIFY(TLK_AREG_XO_CTRL1, (value.bit.xo_cap_off_ana = 1;));
    }
    else
    {
        TLK_ANALOG_MODIFY(TLK_AREG_XO_CTRL1, (
            value.bit.xo_cap_off_ana = 0;
            value.bit.xo_cdac_ana = cfg.internal_cap_value;
        ));
    }
}
...
void tlk_clock_select_32k_src_at_startup(void)
{
    tlk_clock_32k_configure(TLK_CLOCK_32K_RC);
    tlk_clock_32k_rc_calibrate();
}

TLK_REGISTER_PRE_INIT(tlk_clock_select_32k_src_at_startup, TLK_INIT_LEVEL_SYSTEM,
