...
#define TLK_WAIT_FOR_TRUE_OR_TIMEOUT(condition, timeout_us, timeout_handler) \
    do { \
        ...
        while (!(__DEBRACKET condition)) { \
            if ((tlk_clock_cclk_get_tick() - start) > (tlk_sys_clk.cclk * __DEBRACKET timeout_us)) { \
                __DEBRACKET timeout_handler \
            } \
        } \
    } while (0);
 ...
 */
enum tlk_clock_status tlk_clock_sys_clk_configure(const struct tlk_clock_sys_clk_config *sys_clk_config);
