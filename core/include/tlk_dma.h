...
{
    TLK_DMA_NONE = -1,
    FOR_CALL(UNISDK_DMA_CHN_COUNT, TLK_DECLARE_DMA_CHANNEL)
    TLK_DMA_CHN_MAX
};

enum tlk_dma_addr_ctrl
{
    TLK_DMA_ADDR_INCREMENT = 0, /* Increment address,For example,if the transfer direction is: sram -> peripheral, the source address (sram) needs to be increased */
    TLK_DMA_ADDR_DECREMENT, /* Decrement address,Not usually used.*/
    TLK_DMA_ADDR_FIX,       /* Fixed address,For example,if the transfer direction is: sram -> peripheral, the destination address (peripheral fifo) needs to be fixed.*/
};

enum tlk_dma_mode
{
    TLK_DMA_NORMAL_MODE = 0,     /* The SRAM as either source or destination does not require a handshake mechanism because there is no empty-full situation. */
