...
extern void tlk_uart_configure_critical(enum tlk_uart_module uart_num, uint32_t baudrate,
                                        struct tlk_gpio_port_pin tx_port_pin);
extern void tlk_uart_send_critical(enum tlk_uart_module uart_num, const char* buf,
                                   uint32_t buff_size);
...
static void tlk_critical_print_hw_init(void)
{
    if (!tlk_critical_print_hw_inited)
    {
        ...
        tlk_uart_configure_critical(CONFIG_TLK_CRITICAL_UART,
                                    CONFIG_TLK_CRITICAL_UART_BAUDRATE,
                                    (struct tlk_gpio_port_pin){
                                        .port = PINMUX_CRITICAL_PRINT_TX_PORT,
                                        .pin  = PINMUX_CRITICAL_PRINT_TX_PIN,
                                    });
    }
}
...
TLK_REGISTER_AFTER_SLEEP(tlk_critical_print_pm_reinit, TLK_INIT_LEVEL_SYSTEM,
                         TLK_INIT_PRIORITY_HIGH);
