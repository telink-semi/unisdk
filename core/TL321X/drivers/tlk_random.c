...
#include "crypto/include/trng/trng_basic.h"
...
#define TLK_RANDOM_ADD_SOURCE_TLK_IF_ENABLED(i)                                                    \
    TLK_IF_ENABLED(CONFIG_TLK_RANDOM_TRNG_SOURCE##i, (| TLK_BIT(i)))
...
static void tlk_random_init(void)
{
    ...
    trng_ro_entropy_config(TLK_RANDOM_GET_ENABLED_SOURCES);

    // In the datasheet - source0 control. But from investigation, in this offset - source2 control.
    // In the datasheet - source1 control. But from investigation, in this offset - source3 control.
    // In the datasheet - source2 control. But from investigation, in this offset - source0 control.
    // In the datasheet - source3 control. But from investigation, in this offset - source1 control.
    trng_ro_sub_entropy_config(4, TLK_RANDOM_GET_ENABLED_SOURCE_ROS(0));
    trng_ro_sub_entropy_config(3, TLK_RANDOM_GET_ENABLED_SOURCE_ROS(1));
    trng_ro_sub_entropy_config(2, TLK_RANDOM_GET_ENABLED_SOURCE_ROS(2));
    trng_ro_sub_entropy_config(1, TLK_RANDOM_GET_ENABLED_SOURCE_ROS(3));
...
_tlk_attribute_ram_code_sec_ uint32_t tlk_random(void)
{
    ...
    if (get_rand_internal((unsigned char*) &random_number, 4) == TRNG_HT_ERROR)
    {
        ...
        // work again.
        trng_disable();
        trng_reseed(); // This API will make: { rTRNG_SR |= 0x07; // write 1 to clear }
        trng_enable();
    }
