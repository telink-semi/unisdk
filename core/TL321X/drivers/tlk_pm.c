...
uint8_t tlk_pm_sleep_mode_to_reg_value[] = {
    ...
    IF_ENABLED(CONFIG_TLK_PM_RETENTION_MEMORY_SIZE_96K, (UNISDK_PM_BFLD_RETENTION_MODE_SIZE_96K,))
};
...
void tlk_pm_clear_wakeup_sources(void)
{
    ...
    tlk_analog_write_reg8(TLK_AREG_PM_GPIO_WAKEUP_ENABLE(4), 0);
#endif
}
    ...
    if (tlk_pm_sleep_info.wakeup_sources & TLK_PM_WAKEUP_SOURCE_TIMER) {
        ...
        switch (mode)
        {
            ...
            case TLK_PM_SLEEP_MODE_SUSPEND:
                if (duration_ms <= CONFIG_TLK_PM_SUSPEND_WAKEUP_COST_MS +
                  CONFIG_TLK_PM_SUSPEND_MIN_DURATION_MS)
                return TLK_PM_SLEEP_TOO_SHORT;
                else 
            ...
            case TLK_PM_SLEEP_MODE_DEEP_RETENTION:
                if (duration_ms <= CONFIG_TLK_PM_DEEP_RETENTION_WAKEUP_COST_MS + CONFIG_TLK_PM_DEEP_RETENTION_MIN_DURATION_MS)
                return TLK_PM_SLEEP_TOO_SHORT;
                else 
