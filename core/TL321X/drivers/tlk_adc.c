...
static inline void tlk_adc_set_scan_chn_dis(void)
{
    // FLD_SCANT_MAX cannot be configured to 0, because 0 is the RNG channel, if RNG is not enabled (default is not enabled), 
    // the state machine will not stop at the RNG channel after setting 0, it will not stop.
    tlk_adc_reg.config0_bit.scant_max = 1;
}

static inline void tlk_adc_set_scan_chn_cnt(uint8_t chn_cnt)
{
    tlk_adc_reg.config0_bit.scant_max = chn_cnt * 2;
}

static inline void tlk_adc_clr_rx_index(void)
{
    tlk_adc_rx_fifo_index = 0;
}

static inline void tlk_adc_clr_rx_fifo_cnt(void)
{
    tlk_adc_reg.soft_control_bit.fifo_clr = 1;
    tlk_adc_clr_rx_index();
}

static inline void tlk_adc_all_chn_data_to_fifo_en(void)
{
    tlk_analog_write_reg8(areg_adc_data_transfer_control, tlk_analog_read_reg8(areg_adc_data_transfer_control) & (~FLD_AUTO_NOT_EN));
}

static inline void tlk_adc_all_chn_data_to_fifo_dis(void)
{
    tlk_analog_write_reg8(areg_adc_data_transfer_control, FLD_AUTO_NOT_EN);
}
...
static inline void tlk_adc_set_clk(void)
{
    ...
    tlk_analog_write_reg8(areg_adc_sample_clk_div, UNISDK_ADC_CLK_DIV); // div=5, adc analog clk = 24MHz/(1+div) = 4M.
}

static inline void tlk_adc_dig_clk_en(void)
{
...
static inline void tlk_adc_set_diff_input(enum tlk_adc_channel chn, uint8_t p_ain, uint8_t n_ain)
{
...
static inline void tlk_adc_set_scale_factor(enum tlk_adc_channel chn, enum tlk_adc_prescale pre_scale)
{
    tlk_adc_reg.channel_set_state_bit[chn].sel_ai_scale = pre_scale;
}

static inline void tlk_adc_set_state_length(enum tlk_adc_channel chn, uint16_t r_max_mc, uint8_t r_max_s)
{
    tlk_adc_reg.capture_state_bit[chn].r_max_c = r_max_mc;
    tlk_adc_reg.channel_set_state_bit[chn].r_max_s = r_max_s;
}

static inline void tlk_adc_set_tsample_cycle(enum tlk_adc_channel sample_cycle)
{
    tlk_adc_reg.tsamp_bit.m_tsamp = sample_cycle;
}

static inline void tlk_adc_set_chn_en(enum tlk_adc_channel chn)
{
...
enum tlk_adc_status tlk_adc_read(uint16_t *buffer, uint32_t size)
{
    ...
    if (tlk_adc_config.dma_en) {
        tlk_dma_start_transfer(
            tlk_adc_config.dma_channel, SAR_ADC_FIFO, (uint32_t)buffer, 
        ...
        for (uint32_t i = 0; i < size; i++) {
            // 12 bit resolution, TLK_BIT(11) is sign bit, 1 means negative voltage in differential_mode
...
#endif
    {
        ...
        for (uint32_t i = 0; i < size; i++) {
            TLK_WAIT_FOR_TRUE_OR_TIMEOUT(
                (tlk_adc_reg.rxfifo_trig_num_bit.buf_cnt), 
                (tlk_adc_config.timeout_us), 
                (return TLK_ADC_TIMEOUT;) 
            )
