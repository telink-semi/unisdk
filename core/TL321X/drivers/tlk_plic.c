...
{
    (src <= 31) ? (tlk_plic_reg.irq_en_l |= TLK_BIT(src % 32))
...
void tlk_plic_preempt_enable(void)
{
...
{
    (src <= 31) ? (tlk_plic_reg.irq_pending_l = TLK_BIT(src % 32))
...
_tlk_attribute_ram_code_sec_ __attribute__((always_inline)) inline void
tlk_plic_isr(tlk_func_isr_t func, uint32_t src)
{
    /**
