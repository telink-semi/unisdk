...
#if IS_ENABLED(CONFIG_TLK_GPIO_PM_DEVICE)
    struct {
        uint16_t pupd;
        uint8_t function;
...
#define TLK_DECLARE_GPIO_PORT_HANDLE_IF_ENABLED(num) IF_ENABLED(CONFIG_TLK_GPIO_PORT_##num##_ENABLED, (TLK_DECLARE_GPIO_PORT_HANDLE(num)))

#define TLK_PAST_GPIO_PORT_HANDLE_OR_NULL(num) IF_ENABLED_ELSE(CONFIG_TLK_GPIO_PORT_##num##_ENABLED, (&tlk_gpio##num,), (NULL,))

FOR_CALL(UNISDK_GPIO_PORT_COUNT, TLK_DECLARE_GPIO_PORT_HANDLE_IF_ENABLED)

struct tlk_gpio_port_handle* tlk_gpio[UNISDK_GPIO_PORT_COUNT] = {
    FOR_CALL(UNISDK_GPIO_PORT_COUNT, TLK_PAST_GPIO_PORT_HANDLE_OR_NULL)
};
...
static _tlk_always_inline void tlk_gpio_irq_en(enum tlk_gpio_port port, enum tlk_gpio_pin pin, int32_t irq)
{
    TLK_BM_SET(tlk_gpio_reg.port_config[port].irq_en[irq], pin);
}

_tlk_always_inline void tlk_gpio_enable(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    TLK_BM_SET(tlk_gpio_reg.port_config[port].function, pin);
}

_tlk_always_inline void tlk_gpio_set_high_level(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    tlk_gpio_reg.port_config[port].output_set = pin;
    IF_ENABLED(CONFIG_TLK_GPIO_PM_DEVICE, (TLK_BM_SET(tlk_gpio[port]->retention.output, pin);))
}

_tlk_always_inline void tlk_gpio_set_low_level(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    tlk_gpio_reg.port_config[port].output_clear = pin;
    IF_ENABLED(CONFIG_TLK_GPIO_PM_DEVICE, (TLK_BM_CLR(tlk_gpio[port]->retention.output, pin);))
}

_tlk_always_inline void tlk_gpio_output_en(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    TLK_BM_CLR(tlk_gpio_reg.port_config[port].output_en, pin);
}

_tlk_always_inline void tlk_gpio_output_dis(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    TLK_BM_SET(tlk_gpio_reg.port_config[port].output_en, pin);
}

_tlk_always_inline void tlk_gpio_input_en(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_B_ENABLED)
    if ((port == TLK_GPIO_PORT_B) && (pin >= TLK_GPIO_PIN_4) && (pin <= TLK_GPIO_PIN_7)) {
        tlk_analog_write_reg8(UNISDK_GPIO_AREG_PB_IE, tlk_analog_read_reg8(UNISDK_GPIO_AREG_PB_IE) | pin);
    } else 
#endif
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_C_ENABLED)
    if (port == TLK_GPIO_PORT_C) {
        tlk_analog_write_reg8(UNISDK_GPIO_AREG_PC_IE, tlk_analog_read_reg8(UNISDK_GPIO_AREG_PC_IE) | pin);
    } else
#endif
    {
...
_tlk_always_inline void tlk_gpio_input_dis(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_B_ENABLED)
    if ((port == TLK_GPIO_PORT_B) && (pin >= TLK_GPIO_PIN_4) && (pin <= TLK_GPIO_PIN_7)) {
        tlk_analog_write_reg8(UNISDK_GPIO_AREG_PB_IE, tlk_analog_read_reg8(UNISDK_GPIO_AREG_PB_IE) & (~pin));
...
enum tlk_gpio_pin tlk_gpio_port_read(enum tlk_gpio_port port)
{
    return tlk_gpio_reg.port_config[port].input;
}

static void tlk_gpio_set_mode(enum tlk_gpio_port port, enum tlk_gpio_pin pin, enum tlk_gpio_mode mode)
{
    uint8_t r_val = mode & 0x03;

    uint8_t base_ana_reg = 0;
    if (port < 5)
    {
        base_ana_reg = UNISDK_GPIO_AREG_PUPD(port) + ((pin & 0xf0) ? 1 : 0);
    } else {
        return;
    }
    uint8_t shift_num, mask_not;
...
void tlk_gpio_irq_clr(uint8_t status)
{
    tlk_gpio_reg.irq_status = status;
}

void tlk_gpio_irq_disable(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    TLK_BM_CLR(tlk_gpio_reg.port_config[port].irq_en[0], pin);
}

void tlk_gpio_irq_trigger_configure(enum tlk_gpio_port port, enum tlk_gpio_pin pin, enum tlk_gpio_intr_trigger trigger, uint8_t irq_num)
{
    switch (trigger) {
...
void tlk_gpio_pin_toggle(enum tlk_gpio_port port, enum tlk_gpio_pin pin)
{
    ...
    IF_ENABLED(CONFIG_TLK_GPIO_PM_DEVICE, (TLK_BM_FLIP(tlk_gpio[port]->retention.output, pin);))
}
...
void tlk_gpio_irq_configure(enum tlk_gpio_port port, enum tlk_gpio_pin pin, enum tlk_gpio_intr_trigger trigger)
{
    ...
    if (irq_num == 0) {
        tlk_gpio_reg.irq_control_bit.gpio_irq_en = 1; // Only GPIO_IRQ0 needs to enable FLD_GPIO_CORE_INTERRUPT_EN.
    }
    ...
    tlk_gpio_irq_trigger_configure(port, pin, trigger, irq_num);
...
_tlk_attribute_ram_code_sec_ void tlk_gpio_irq_handler(void)
{
    for (int32_t port = 0; port < UNISDK_GPIO_PORT_COUNT; port++) {
        ...
        uint8_t irq_triggered = both_edge_triggered | falling_edge_triggered | rising_edge_triggered;
...
TLK_PLIC_ISR_REGISTER(tlk_gpio_irq_handler, UNISDK_PLIC_IRQ_NUM_GPIO_SRC0)

#endif



#if IS_ENABLED(CONFIG_TLK_GPIO_PREVENT_SLEEP)
...
static void tlk_gpio_save_context(void)
{
    for (int32_t port = 0; port <= 4; port++) {
        tlk_gpio[port]->retention.pupd = tlk_analog_read_reg16(UNISDK_GPIO_AREG_PUPD(port));
        
        tlk_gpio[port]->retention.function = tlk_gpio_reg.port_config[port].function;

#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_B_ENABLED)
        if (port == TLK_GPIO_PORT_B) {
            tlk_gpio[port]->retention.input_enable = (0x0f & tlk_gpio_reg.port_config[port].input_en) | (0xf0 & tlk_analog_read_reg8(UNISDK_GPIO_AREG_PB_IE));
        } else 
#endif
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_C_ENABLED)
        if (port == TLK_GPIO_PORT_C) {
            tlk_gpio[port]->retention.input_enable = tlk_analog_read_reg8(UNISDK_GPIO_AREG_PC_IE);
        } else 
#endif
        {
...
{
    for (int32_t port = 0; port <= 4; port++) {
        tlk_analog_write_reg16(UNISDK_GPIO_AREG_PUPD(port), tlk_gpio[port]->retention.pupd);
        ...
        if (port == TLK_GPIO_PORT_B) {
            ...
            tlk_analog_write_reg8(UNISDK_GPIO_AREG_PB_IE, 0xf0 & tlk_gpio[port]->retention.input_enable);
        } else 
#endif
#if IS_ENABLED(CONFIG_TLK_GPIO_PORT_C_ENABLED)
        if (port == TLK_GPIO_PORT_C) {
            tlk_analog_write_reg8(UNISDK_GPIO_AREG_PC_IE, tlk_gpio[port]->retention.input_enable);
        } else 
#endif
        {
