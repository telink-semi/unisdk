...
enum tlk_uart_irq
{
    /* When the number of rxfifo reaches the set threshold(tlk_uart_rx_set_irq_trig_level), an
     ...
     * threshold(tlk_uart_tx_set_irq_trig_level), an interrupt is generated and the
     ...
     * tlk_uart_rx_set_auto_clr_fifo_ptr is enabled, the interrupt flag is automatically
     * cleared. If tlk_uart_rx_set_auto_clr_fifo_ptr is disabled, the interrupt flag
...
/* ==== IRQ HELPERS ==== */
static uint32_t tlk_uart_irq_get_status(enum tlk_uart_module uart_num, enum tlk_uart_irq status)
{
    return tlk_uart_reg(uart_num).irq_status & status;
}

static void tlk_uart_irq_clr_status(enum tlk_uart_module uart_num, enum tlk_uart_irq status)
{
    /*
...
#if TLK_IS_ENABLED(CONFIG_TLK_UART_USED_IRQ)
static void tlk_uart_irq_enable(enum tlk_uart_module uart_num, enum tlk_uart_irq irqs)
{
    tlk_uart_reg(uart_num).rx_timeout1 |= irqs;
}

static void tlk_uart_irq_disable(enum tlk_uart_module uart_num, enum tlk_uart_irq irqs)
{
    tlk_uart_reg(uart_num).rx_timeout1 &= ~irqs;
}
#endif
...
static void tlk_uart_rx_set_timeout_with_exp(enum tlk_uart_module uart_num, uint8_t bwpc,
                                             uint8_t bit_cnt, enum tlk_uart_rx_timeout_mul mul,
                                             uint8_t rxtimeout_exp)
{
...
static void tlk_uart_tx_set_irq_trig_level(enum tlk_uart_module uart_num, uint8_t tx_level)
{
    tlk_uart_reg(uart_num).control3_bit.tx_irq_trig_lvl = tx_level;
}

static void tlk_uart_rx_set_auto_clr_fifo_ptr(enum tlk_uart_module uart_num, uint8_t en)
{
    tlk_uart_reg(uart_num).control0_bit.auto_rxclr_en = en;
}

static void tlk_uart_rx_set_irq_trig_level(enum tlk_uart_module uart_num, uint8_t rx_level)
{
    tlk_uart_rx_set_auto_clr_fifo_ptr(uart_num, 0);
    tlk_uart_reg(uart_num).control3_bit.rx_irq_trig_lvl = rx_level;
}
/* ==== CONFIG HELPERS ==== */

/* ==== SEND HELPERS ==== */
static uint8_t tlk_uart_tx_get_fifo_count(enum tlk_uart_module uart_num)
{
    return tlk_uart_reg(uart_num).buf_cnt_bit.tx_buf_cnt;
}

static bool tlk_uart_tx_is_fifo_full(enum tlk_uart_module uart_num)
{
    return tlk_uart_tx_get_fifo_count(uart_num) == 8; // TODO: move to YAML
}
...
/* ==== RECEIVE HELPERS ==== */
static uint8_t tlk_uart_rx_get_fifo_count(enum tlk_uart_module uart_num)
{
                                 ...
                                 struct tlk_gpio_port_pin tx_port_pin)
{
    ...
    tlk_uart_tx_set_irq_trig_level(uart_num, 0);
}
...
void tlk_uart_configure(enum tlk_uart_module uart_num, struct tlk_uart_config* config)
{
    ...
    tlk_uart_rx_set_timeout_with_exp(uart_num, bwpc, frame_bits, config->rx_timeout_mul, 0);
    ...
    {
        tlk_uart_tx_set_irq_trig_level(uart_num, 4);
    }

    tlk_uart_rx_set_irq_trig_level(uart_num, 4);
    ...
    {
        tlk_dma_chn_configure(uart_instance->tx_dma_chn, uart_tx_dma_config);
    }
    ...
    if (uart_instance->rx_mode == TLK_UART_XFER_DMA)
    {
        ...
        tlk_dma_chn_configure(uart_instance->rx_dma_chn, uart_rx_dma_config);
    }
#endif
    ...
    tlk_uart_pinmux_configure(uart_num, config);
}

void tlk_uart_pinmux_configure(enum tlk_uart_module uart_num, struct tlk_uart_config* config)
{
    TLK_UART_GET_INSTANCE(uart_num)
                                         ...
                                         uint32_t timeout_us)
{
    ...
    if (uart_instance->tx_mode == TLK_UART_XFER_BLOCKING)
    {
        uint32_t start_us             = tlk_api_time_get_micros();
        uart_instance->waiting_for_tx = 1;

        for (uint32_t i = 0; i < buff_size; i++)
        {
            while (tlk_uart_tx_is_fifo_full(uart_num))
            {
                if ((tlk_api_time_get_micros() - start_us) >= timeout_us)
                {
    ...
    if (uart_instance->tx_mode == TLK_UART_XFER_DMA)
    {
        ...
        tlk_dma_chn_transfer_configure(uart_instance->tx_dma_chn,
                                       (uint32_t) (buff),
                                       (uint32_t) tlk_uart_reg(uart_num).data_buf,
                                       buff_size);

        tlk_dma_chn_transfer_start(uart_instance->tx_dma_chn);
    }
#endif
                                            ...
                                            uint32_t buff_size, tlk_uart_rx_handler_t rx_handler)
{
    ...
    if (uart_instance->rx_mode == TLK_UART_XFER_DMA)
    {
        ...
        {
            tlk_uart_irq_enable(uart_num, TLK_UART_IRQ_RXFIFO);
        }
        ...
        tlk_dma_chn_transfer_configure(uart_instance->rx_dma_chn,
                                       (uint32_t) tlk_uart_reg(uart_num).data_buf,
                                       (uint32_t) (buff),
                                       buff_size - 3);

        tlk_dma_chn_transfer_start(uart_instance->rx_dma_chn);
    }
#endif
...
#if TLK_IS_ENABLED(CONFIG_TLK_UART_USED_FLOW_CONTROL)
void tlk_uart_flow_control_configure(enum tlk_uart_module                 uart_num,
...
#endif
    tlk_uart_flow_control_pinmux_configure(uart_num, config);
}

void tlk_uart_flow_control_pinmux_configure(enum tlk_uart_module                 uart_num,
                                            struct tlk_uart_flow_control_config* config)
{
    TLK_UART_GET_INSTANCE(uart_num)
...
/* ==== IRQ HELPERS==== */
#if TLK_IS_ENABLED(CONFIG_TLK_UART_USED_IRQ)
_tlk_attribute_ram_code_sec_ static void tlk_uart_irq_handle(enum tlk_uart_module uart_num)
{
    TLK_UART_GET_INSTANCE(uart_num)
    ...
    if (uart_instance->tx_mode == TLK_UART_XFER_IRQ)
    {
        ...
        {
            uint8_t fifo_cnt = TLK_MIN((uint32_t) (8 - tlk_uart_tx_get_fifo_count(uart_num)),
    ...
    if (uart_instance->rx_mode == TLK_UART_XFER_IRQ)
    {
        ...
        {
            uint8_t fifo_cnt = TLK_MIN(tlk_uart_rx_get_fifo_count(uart_num),
        ...
        {
            uint8_t fifo_tail = TLK_MIN(tlk_uart_rx_get_fifo_count(uart_num),
                                        uart_instance->rx_buff_size - uart_instance->rx_cnt);
        ...
        else
        {
            if (tlk_uart_irq_get_status(uart_num, TLK_UART_IRQ_RXFIFO))
            {
                if (uart_instance->rx_cnt < uart_instance->rx_buff_size)
                {
                    ...
                    uint8_t fifo_tail =
                        TLK_MIN(tlk_uart_rx_get_fifo_count(uart_num),
                                uart_instance->rx_buff_size - uart_instance->rx_cnt);
            ...
            if (tlk_uart_irq_get_status(uart_num, TLK_UART_IRQ_RXDONE))
            {
                if (uart_instance->rx_cnt < uart_instance->rx_buff_size)
                {
                    ...
                    if (tlk_uart_rx_get_fifo_count(uart_num))
                    {
                        uint8_t fifo_tail =
                            TLK_MIN(tlk_uart_rx_get_fifo_count(uart_num),
                                    uart_instance->rx_buff_size - uart_instance->rx_cnt);
...
#define TLK_UART_DECLARE_IRQ_HANDLER(num)                                                          \
    _tlk_attribute_ram_code_sec_ static void tlk_uart##num##_irq_handle(void)                      \
    {                                                                                              \
        tlk_uart_irq_handle(TLK_UART##num);                                                        \
    }                                                                                              \
    TLK_PLIC_ISR_REGISTER(tlk_uart##num##_irq_handle, UNISDK_PLIC_IRQ_NUM_UART##num)
...
#define TLK_UART_DECLARE_RX_DMA_HANDLER(num)                                                       \
    __attribute__((used, noinline)) static void tlk_uart##num##_rx_dma_irq_handle(                 \
...
#define TLK_UART_DECLARE_REQUEST_DMA_CHN(num)                                                      \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            tlk_uart##num##_instance.tx_dma_chn = tlk_dma_chn_request();                           \
        }                                                                                          \
        if (tlk_uart##num##_instance.rx_mode == TLK_UART_XFER_DMA)                                 \
        {                                                                                          \
            tlk_uart##num##_instance.rx_dma_chn = tlk_dma_chn_request();                           \
                                                                                                   \
            TLK_IF_ENABLED(CONFIG_TLK_UART##num##_RX_DMA_WITH_TAIL, ({                             \
                               if (tlk_uart##num##_instance.rx_dma_mode ==                         \
                                   TLK_UART_RX_DMA_WITH_TAIL)                                      \
                               {                                                                   \
                                   tlk_dma_chn_add_callback(tlk_uart##num##_instance.rx_dma_chn,   \
                                                            tlk_uart##num##_rx_dma_irq_handle);    \
                               }                                                                   \
                           }));                                                                    \
        }                                                                                          \
    }                                                                                              \
...
#define TLK_UART_DECLARE_RESTORE_CONTEXT(num)                                                      \
    ...
    {                                                                                              \
        ...
        tlk_uart_pinmux_configure(TLK_UART##num, &config);                                         \
        TLK_IF_ENABLED(                                                                            \
            ...
            (struct tlk_uart_flow_control_config flow_control_config =                             \
             ...
             tlk_uart_flow_control_pinmux_configure(TLK_UART##num, &flow_control_config);))        \
    }                                                                                              \
                                                                                                   \
