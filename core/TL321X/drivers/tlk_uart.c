...
static _tlk_always_inline void tlk_uart_set_rx_timeout_with_exp(enum tlk_uart_module uart_num,
                                                                uint8_t bwpc, uint8_t bit_cnt,
...
{
    return tlk_uart_get_tx_fifo_count(uart_num) == 8; // TODO: move to YAML
}

static _tlk_always_inline void tlk_uart_send_byte(enum tlk_uart_module uart_num, uint8_t data)
{
    TLK_UART_GET_INSTANCE(uart_num)
...
static _tlk_always_inline uint8_t tlk_uart_read_byte(enum tlk_uart_module uart_num)
{
    TLK_UART_GET_INSTANCE(uart_num)
...
void tlk_uart_configure(enum tlk_uart_module uart_num, struct tlk_uart_config* config)
{
    ...
    tlk_uart_set_rx_timeout_with_exp(uart_num, bwpc, frame_bits, config->rx_timeout_mul, 0);
...
#define TLK_UART_DECLARE_AUTO_CONFIG(num)                                                          \
    ...
    TLK_REGISTER_PRE_INIT(                                                                         \
        tlk_uart##num##_auto_config, TLK_INIT_LEVEL_DRIVER, TLK_INIT_PRIORITY_NORMAL)
...
#define TLK_UART_DECLARE_RX_DMA_HANDLER(num)                                                       \
    __attribute__((used, noinline)) static void tlk_uart##num##_rx_dma_irq_handler(                \
