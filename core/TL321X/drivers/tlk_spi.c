...
void tlk_gspi_set_pins(struct tlk_spi_config* config)
{
...
static inline void tlk_spi_set_slave_irq_trigs(enum tlk_spi_module module)
{
    ...
    tlk_spi_tx_irq_trig_cnt(module, 4);                                                          //default gspi burst1(N=1) ,tx_fifo trig_cnt=rx fifo depth-4*N=8-4=4.
}   

void tlk_spi_set_transmode(enum tlk_spi_module module, enum tlk_spi_trans_mode mode)
{
...
{
    return (20 - (reg_spi_txfifo_status(module) & FLD_SPI_TXF_ENTRIES) < 4);
}
...
TLK_PLIC_ISR_REGISTER(tlk_gspi_irq_handler, CONFIG_SPI0_IRQ_NUM);
/* --- IRQ --- */

void tlk_spi_configure(struct tlk_spi_device* device, struct tlk_spi_config* config)
{
    tlk_gspi_set_pins(config);
    ...
    if (config->role == TLK_SPI_SLAVE) {
        ...
        tlk_plic_interrupt_enable(CONFIG_SPI0_IRQ_NUM);
