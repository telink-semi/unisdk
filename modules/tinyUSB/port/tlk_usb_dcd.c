...
bool dcd_edpt_xfer(uint8_t rhport, uint8_t ep_addr, uint8_t * buffer, uint16_t total_bytes)
{
    ...
    if(dir == TUSB_DIR_OUT) {
        if(ep_id == TLK_USB_EP0_IDX && total_bytes == 0) {
            ...
            dcd_event_xfer_complete(rhport, TLK_USB_EP0_IDX, total_bytes, XFER_RESULT_SUCCESS, true);
        } else {
        ...
        if(ep_id == TLK_USB_EP0_IDX) {
            ...
            if(total_bytes == 0) {
                dcd_event_xfer_complete(rhport, (TUSB_DIR_IN_MASK | TLK_USB_EP0_IDX), result,
                                XFER_RESULT_SUCCESS, false);
            }
        } else {
            result = tlk_usb_ep_write(ep_id, buffer, total_bytes);
            dcd_event_xfer_complete(rhport, (TUSB_DIR_IN_MASK | ep_id), result, XFER_RESULT_SUCCESS, false);
        }
...
void dcd_int_enable(uint8_t rhport)
{
    ...
    struct tlk_usb_cb cb = {
        .setup_cb   = tlk_usb_dc_setup_cb_handler,
