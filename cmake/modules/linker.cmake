...
# toolchain variant) after the JSON-based options, so they override the ABI
# implied by -mcpu there. The linker driver invocation needs the same flags as
# the compiler to select the matching libgcc/libc multilib.
if(TLK_TOOLCHAIN_EXTRA_FLAGS)
    list(APPEND LINKER_OPTS_LIST ${TLK_TOOLCHAIN_EXTRA_FLAGS})
endif()
...
if(CONFIG_TLK_CORE)
    if(NOT IS_DIRECTORY "${TELINK_BASE}/libraries")
        set(CHIP_H ${GENERATED_HEADERS_BASE_DIR}/core/${CONFIG_TLK_CORE}/properties/tlk_chip.h)
    else()
...
list(APPEND LINKER_OPTS_LIST "-Wl,-Map=${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.map")
...
function(telink_apply_linker_options target)
    ...
    if(OBJ_COPY)
        ...
        add_custom_command(TARGET ${target} POST_BUILD COMMAND ${CMAKE_OBJCOPY} ${OBJ_COPY_LIST} ${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.elf ${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.bin)
    endif()

    string(JSON OBJ_DUMP GET ${TARGET_JSON} obj_dump)
    if(OBJ_DUMP)
        json_to_list(${OBJ_DUMP} OBJ_DUMP_LIST)
        add_custom_command(TARGET ${target} POST_BUILD COMMAND ${CMAKE_OBJDUMP} ${OBJ_DUMP_LIST} ${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.elf > ${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.lst)
    endif()

    string(JSON PRINT_SIZE GET ${TARGET_JSON} print_size)
    if(PRINT_SIZE)
        json_to_list(${PRINT_SIZE} PRINT_SIZE_LIST)
        add_custom_command(TARGET ${target} POST_BUILD COMMAND ${CMAKE_PRINTSIZE} ${PRINT_SIZE_LIST} ${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.elf)
    endif()

    # Add firmware check script
    set(CHECK_FW_SCRIPT_PATH "${TELINK_BASE}/scripts/tlk_check_fw.py")
    add_custom_command(TARGET ${target} POST_BUILD COMMAND ${PYTHON_EXECUTABLE} ${CHECK_FW_SCRIPT_PATH} ${CMAKE_BINARY_DIR}/${TLK_ARTIFACT_NAME}.bin)
endfunction()
