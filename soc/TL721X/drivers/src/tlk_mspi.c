                                                      ...
                                                      tlk_mspi_slave_size_e slave3_size)
{
    ...
    // The middle device can also not allocate the address space, because when the chip actually
    // accesses the address, it will judge according to the following logic, if the following logic
    // is not met, it will not access.
    ...
    reg_mspi_xip_size_set =
        (((slave0_size + slave1_size + slave2_size + slave3_size - 1) << 6) &
         FLD_MSPI_XIP3_END_ADDR) |
        (((slave0_size + slave1_size + slave2_size - 1) << 4) & FLD_MSPI_XIP2_END_ADDR) |
        (((slave0_size + slave1_size - 1) << 2) & FLD_MSPI_XIP1_END_ADDR) |
        ((slave0_size - 1) & FLD_MSPI_XIP0_END_ADDR);

    if (slave1_size == TLK_SLAVE_SIZE_NONE)
    {
        tlk_g_slave_base_addr[1] = 0x00000000;
    }
    else
    {
        tlk_g_slave_base_addr[1] = slave0_size << 24;
    }
    if (slave2_size == TLK_SLAVE_SIZE_NONE)
    {
        tlk_g_slave_base_addr[2] = 0x00000000;
    }
    else
    {
        tlk_g_slave_base_addr[2] = (slave0_size + slave1_size) << 24;
    }
    if (slave3_size == TLK_SLAVE_SIZE_NONE)
    {
        tlk_g_slave_base_addr[3] = 0x00000000;
    }
    else
    {
        tlk_g_slave_base_addr[3] = (slave0_size + slave1_size + slave2_size) << 24;
    }
