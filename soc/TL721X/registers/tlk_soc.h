 ...
 *           the address parameter should  be multiple of 2. For
 * example:REG_ADDR16(0x2),REG_ADDR16(0x4).
 *
 *           for all the 4-byte address operation (like REG_ADDR32,write_reg32,read_reg32) ,
 *           the address parameter should  be multiple of 4. For
 * example:write_reg32(0x4),write_reg32(0x8).
 */
...
enum
{
    ...
    TLK_CHIP_VERSION_A1 = 0x80,
    TLK_CHIP_VERSION_A2 = 0xc0,
    TLK_CHIP_VERSION_A3 = 0x01,
};

enum
{
    FLD_MSPI_CLK_MOD    = TLK_BIT_RNG(0, 3),
    FLD_MSPI_DIV_IN_SEL = TLK_BIT_RNG(4, 5), // 0:rc24m 1:xtl48m 2:pll0 3:pll1
};
...
enum
{
    FLD_LSPI_CLK_MOD    = TLK_BIT_RNG(0, 3),
    FLD_LSPI_DIV_IN_SEL = TLK_BIT_RNG(4, 5), // 0:rc24m   1:xtl 24m  2:pll
};
...
enum
{
    ...
    FLD_GSPI_DIV_IN_SEL = TLK_BIT_RNG(8, 9), // 0:rc24m   1:xtl 24m  2:pll
};
...
enum
{
    FLD_I2S0_STEP     = TLK_BIT_RNG(0, 14),
    FLD_I2S0_CLK_EN_0 = TLK_BIT(15),
};
...
enum
{
    FLD_I2S1_STEP     = TLK_BIT_RNG(0, 14),
    FLD_I2S1_CLK_EN_0 = TLK_BIT(15),
};
...
enum
{
    FLD_I2S2_STEP     = TLK_BIT_RNG(0, 14),
    FLD_I2S2_CLK_EN_0 = TLK_BIT(15),
};
...
enum
{
    FLD_AUDIO_REG_EMA  = TLK_BIT_RNG(0, 2),
    FLD_AUDIO_REG_EMAW = TLK_BIT_RNG(3, 4),
    FLD_AUDIO_REG_EMAS = TLK_BIT(5),
};
...
enum
{
    FLD_AUDIO_REG_RAWL  = TLK_BIT(0),
    FLD_AUDIO_REG_RAWLM = TLK_BIT_RNG(1, 2),
    FLD_AUDIO_REG_WABL  = TLK_BIT(3),
    FLD_AUDIO_REG_WABLM = TLK_BIT_RNG(4, 5),
};
...
enum
{
    FLD_USB_REG_EMA  = TLK_BIT_RNG(0, 2),
    FLD_USB_REG_EMAW = TLK_BIT_RNG(3, 4),
    FLD_USB_REG_EMAS = TLK_BIT(5),
};
...
enum
{
    FLD_USB_REG_RAWL  = TLK_BIT(0),
    FLD_USB_REG_RAWLM = TLK_BIT_RNG(1, 2),
    FLD_USB_REG_WABL  = TLK_BIT(3),
    FLD_USB_REG_WABLM = TLK_BIT_RNG(4, 5),
};
...
enum
{
    FLD_SRAM_EMA  = TLK_BIT_RNG(0, 2),
    FLD_SRAM_EMAW = TLK_BIT_RNG(3, 4),
    FLD_SRAM_EMAS = TLK_BIT(5),
};
...
enum
{
    FLD_SRAM_RAML  = TLK_BIT(0),
    FLD_SRAM_RAWLM = TLK_BIT_RNG(1, 2),
    FLD_SRAM_WABL  = TLK_BIT(3),
    FLD_SRAM_WABLM = TLK_BIT_RNG(4, 6),
    FLD_SRAM_STOV  = TLK_BIT(7),
};
...
enum
{
    FLD_REG_EMA  = TLK_BIT_RNG(0, 2),
    FLD_REG_EMAW = TLK_BIT_RNG(3, 4),
    FLD_REG_EMAS = TLK_BIT(5),
};
...
enum
{
    FLD_REG_RAWL  = TLK_BIT(0),
    FLD_REG_RAWLM = TLK_BIT_RNG(1, 2),
    FLD_REG_WABL  = TLK_BIT(3),
    FLD_REG_WABLM = TLK_BIT_RNG(4, 5),
};
...
enum
{
    ...
    FLD_RST0_ZB_PON = TLK_BIT(6), // Restores all RF-related registers to their default value.
                                  // RF related registers include: baseband, linklayer, modem,
                                  // radio, bb_dma, bb_timer, pdzb.
    FLD_RST0_DMA_BB = TLK_BIT(7), // Resets the BB DMA status, and related registers will be
                                  // cleared. After configuration, BB DMA needs to be reconfigured.
};
...
{
    FLD_RST1_RSTL_BB =
        TLK_BIT(2), // Clears RF state machine and some internal states of the link layer.
    FLD_RST1_RST_MDM = TLK_BIT(3), // Clears all digital logic states related to mdm, the related
                                   // configuration will not be lost after reset.
    FLD_RST1_RSTL_STIMER =
        TLK_BIT(4), // Resets BB STIMER status. RW registers remain, read-only registers will be
                    // cleared, and BB timer tick will be restarted.
};
