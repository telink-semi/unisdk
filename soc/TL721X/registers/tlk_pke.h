#ifndef TLK_PKE_REGISTERS_H_
#define TLK_PKE_REGISTERS_H_
...
typedef struct __attribute__((packed)) {
    union {
        ...
        uint8_t control; // address: 0x80110000, offset: 0x0
    };
    ...
    union {
        ...
        uint32_t mc_pointer; // address: 0x80110008, offset: 0x8
    };
    union {
        struct {
            unsigned core_isr : 1;
        ...
        uint32_t risr; // address: 0x8011000c, offset: 0xc
    };
    union {
        struct {
        ...
        uint32_t imcr; // address: 0x80110010, offset: 0x10
    };
    union {
        struct {
            unsigned core_mi : 1;
        ...
        uint32_t misr; // address: 0x80110014, offset: 0x14
    };
    uint8_t reserved1[0xc]; // address: 0x80110018, offset: 0x18
    union {
        struct {
            unsigned stop_log : 4;
    ...
    uint8_t reserved2[0x28]; // address: 0x80110028, offset: 0x28
    union {
        struct {
            unsigned iaff_r0 : 1;
            unsigned imon_r0 : 1;
        ...
        uint32_t exe_config; // address: 0x80110050, offset: 0x50
    };
    uint8_t reserved3[0xa8]; // address: 0x80110054, offset: 0x54
    union {
        struct {
            unsigned mir : 4;
            unsigned mar : 4;
