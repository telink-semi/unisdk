...
{
    FLD_OTP_AUTO_INC =
        TLK_BIT(4), /**< 1:when the latch is finished PDOUT data, PA automatically adds 4 */
...
enum
{
    ...
    FLD_OTP_KEYLOCK        = TLK_BIT(
        2), /**< Can only write 1 from 0. After setting 1, OTP specific address cannot be read */
};
...
enum
{
    FLD_OTP_CAP_EDGE = TLK_BIT_RNG(
        0, 4), /**< Indicates how many PCLK edges to latch PDOUT data after the PCLK rising edge. */
};
...
#define reg_otp_auto_load_data(i)                                                                  \
    REG_ADDR32(OTP_BASE_ADDR + 0x14 + (((i) & 0x03) << 2)) /* i[0-2] */
