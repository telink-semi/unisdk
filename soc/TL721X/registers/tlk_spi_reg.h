...
/**
 * When only Telink RISC-V MCU spi slave is used as slave, the command read from SPI Command is
 * parsed by bit.
 */
enum
{
    FLD_SPI_CMD_RESERVED           = TLK_BIT(0),
    FLD_SPI_CMD_TRANS_HWORD        = TLK_BIT(1), // 1  hword transfer.
    FLD_SPI_CMD_TRANS_WORD         = TLK_BIT(2), // 1  word transfer.
    FLD_SPI_CMD_RD_DUMMY_4CYCLE    = TLK_BIT(3), // 0 8cycle 1 4cycle.
    FLD_SPI_CMD_ADDR_AUTO_INCREASE = TLK_BIT(4), // 0 AUTO incease.
    FLD_SPI_CMD_DATA_DUAL          = TLK_BIT(5), // 0 Single 1 DuaL.
    FLD_SPI_CMD_ADDR_DUAL          = TLK_BIT(6), // 0 Single 1 DuaL.
    FLD_SPI_CMD_RD_EN              = TLK_BIT(7), // 0  write 1 read.
};
 ...
 * BIT[5] enable slvCmdint.The slave command interrupt is triggered each byte command received
 * (starting 8 bit) .slave only. BIT[6] enable RX DMA. BIT[7] enable TX DMA.
 */
#define reg_spi_ctrl0(i) REG_ADDR8(LSPI_BASE_ADDR + 0x05 + (i) * BASE_ADDR_DIFF)

enum
{
    FLD_SPI_RXFIFO_OR_INT_EN = TLK_BIT(0),
    FLD_SPI_TXFIFO_UR_INT_EN = TLK_BIT(1),
    FLD_SPI_RXFIFO_INT_EN    = TLK_BIT(2),
    FLD_SPI_TXFIFO_INT_EN    = TLK_BIT(3),
    FLD_SPI_END_INT_EN       = TLK_BIT(4),
    FLD_SPI_SLV_CMD_EN       = TLK_BIT(5),
    FLD_SPI_RX_DMA_EN        = TLK_BIT(6),
    FLD_SPI_TX_DMA_EN        = TLK_BIT(7),
};

/**
 * BIT[0:7] The second cmd of SPI. It will only be used when the slave supports two cmd modes. The
 * cmd must be configured first, and then the first cmd must be configured.
 */
#define reg_spi_cmd1(i) REG_ADDR8(LSPI_BASE_ADDR + 0x06 + (i) * BASE_ADDR_DIFF)

/**
 * BIT[0:2]  the minimum time between the edge of SPI_CS and  the edges of SPI_CLK.the actual
 * duration is (SPI_CLK period*(cs2sclk+1)),default=1, MASTER ONLY. BIT[3:7]  the minimum time that
 * SPI CS should stay HIGH.the actual duration is (SPI_CLK period*(csht+1)),default=1,MASTER ONLY.
...
enum
{
    FLD_SPI_CS2SCLK = TLK_BIT_RNG(0, 2),
    FLD_SPI_CSHT    = TLK_BIT_RNG(3, 7),
 ...
 * BIT[4]  set address format 0:single mode  1:the format of addr phase is the same as the data
 * phase(Dual/Quad),MASTER ONLY. BIT[5]  1:enable addr phase, MASTER ONLY. BIT[6]  set cmd format 0:
 * single mode  1: the format of the cmd phase is the same as the data phase(Dual/Quad),MASTER ONLY.
...
enum
{
    FLD_SPI_DATA_LANE = TLK_BIT_RNG(0, 1),
    FLD_SPI_ADDR_LEN  = TLK_BIT_RNG(2, 3),
    FLD_SPI_ADDR_FMT  = TLK_BIT(4),
    FLD_SPI_ADDR_EN   = TLK_BIT(5),
    FLD_SPI_CMD_FMT   = TLK_BIT(6),
    FLD_SPI_CMD_EN    = TLK_BIT(7),
};

/**
 * BIT[0:3]  dummy data cnt, dummy is always single wire mode, dummy number = {dummy_cnt_add,
 * dummy_cnt} + 1.default 7,master/slave. BIT[4:7]  the transfer mode.master only. the transfer
 * sequence could be: 0x0:write and read at the same time(must enable CmdEn). 0x1:write only.
...
enum
{
    FLD_SPI_DUMMY_CNT = TLK_BIT_RNG(0, 3),
    FLD_SPI_TRANSMODE = TLK_BIT_RNG(4, 7),
};
 ...
 * BIT[4:5]     the minimum time that SPI CS should stay HIGH.the actual duration is (SPI_CLK
 * period*(csht+1)),default=1,MASTER ONLY. actual_csht[6:0] = {FLD_SPI_CSHT_HIGH, FLD_SPI_CSHT}.
 */
#define reg_spi_reg_ctrl0(i) REG_ADDR8(LSPI_BASE_ADDR + 0x0a + (i) * BASE_ADDR_DIFF)

enum
{
    FLD_SPI_CMD1_EN       = TLK_BIT(0),
    FLD_SPI_TOKEN_VAL_SEL = TLK_BIT(1),
    FLD_SPI_TOKEN_EN      = TLK_BIT(2),
    FLD_SPI_CSHT_HIGH     = TLK_BIT_RNG(4, 5),
};
...
/**
 * BIT[0:7]   transfer count1 for write data,
 * byte1/{lcd_line_per_frame[5:0]lcd_pixel_per_line[9:8]}.
 */
...
enum
{
    FLD_SPI_LSB                 = TLK_BIT(0),
    FLD_SPI_3LINE               = TLK_BIT(1),
    FLD_SPI_WORK_MODE           = TLK_BIT_RNG(2, 3),
    FLD_SPI_MASTER_MODE         = TLK_BIT(4),
    FLD_SPI_DMARX_EOF_CLRRXF_EN = TLK_BIT(5),
    FLD_SPI_DMATX_SOF_CLRTXF_EN = TLK_BIT(6),
    FLD_SPI_AUTO_HREADY_EN      = TLK_BIT(7),
};
...
enum
{
    FLD_SPI_TXF_THRES = TLK_BIT_RNG(0, 5),
};
...
enum
{
    FLD_SPI_RXF_THRES = TLK_BIT_RNG(0, 5),
};
...
enum
{
    FLD_EVENT_EN      = TLK_BIT(0),
    FLD_TASK_EN       = TLK_BIT(1),
    FLD_PEM_EVENT_SEL = TLK_BIT(2),
};
 ...
 * BIT[2]  stop xip.
 * BIT[3]  enable xip.
 * BIT[4:7]  dummy_cnt_add, dummy is always single wire mode, dummy number = {dummy_cnt_add,
 * dummy_cnt} + 1.master/slave.
 */
#define reg_spi_ctrl4(i) REG_ADDR8(LSPI_BASE_ADDR + 0x1c + (i) * BASE_ADDR_DIFF)

enum
{
    FLD_SPI_DMA_TRIG_SPI_EN = TLK_BIT(0),
    FLD_TXDMA_REQ_AF_CMD    = TLK_BIT(1),
    FLD_SPI_XIP_STOP        = TLK_BIT(2),
    FLD_SPI_XIP_ENABLE      = TLK_BIT(3),
    FLD_SPI_DUMMY_CNT_ADD   = TLK_BIT_RNG(4, 7),
};

/**
 * BIT[0:7]  use this combined with xip_mode being xip page mode.default page boundary size is 0,
 * 2^page_size.
 */
...
enum
{
    FLD_SPI_TXF_ENTRIES = TLK_BIT_RNG(0, 6),
    FLD_SPI_TXF_FULL    = TLK_BIT(7),
};
...
enum
{
    FLD_SPI_RXF_ENTRIES = TLK_BIT_RNG(0, 6),
    FLD_SPI_RXF_EMPTY   = TLK_BIT(7),
};
...
enum
{
    FLD_SPI_SET_SLAVE_READY = TLK_BIT(0),
    FLD_SPI_CLR_SLAVE_READY = TLK_BIT(1),
    FLD_SPI_SOFT_RESET      = TLK_BIT(2),
    FLD_SPI_REG_ARB_ERR     = TLK_BIT(3),
    FLD_SPI_RXF_CLR_LEVEL   = TLK_BIT(4),
    FLD_SPI_TXF_CLR_LEVEL   = TLK_BIT(5),
    FLD_SPI_OSD_AHBMST_BUSY = TLK_BIT(6),
    FLD_SPI_BUSY            = TLK_BIT(7),
};
...
enum
{
    FLD_SPI_RXF_OR_INT  = TLK_BIT(0),
    FLD_SPI_TXF_UR_INT  = TLK_BIT(1),
    FLD_SPI_RXF_INT     = TLK_BIT(2),
    FLD_SPI_TXF_INT     = TLK_BIT(3),
    FLD_SPI_END_INT     = TLK_BIT(4),
    FLD_SPI_SLV_CMD_INT = TLK_BIT(5),
};
 ...
 * BIT[4]  set xip read address format 0:single mode  1:the format of addr phase is the same as the
 * data phase(Dual/Quad),XIP ONLY. BIT[5]  1:enable xip read addr phase, XIP ONLY. BIT[6]  xip read
 * cmd format 0: single mode  1: the format of the cmd phase is the same as the data
 * phase(Dual/Quad),XIP ONLY. BIT[7]  xip read spi command phase enable,XIP ONLY BIT[8:11]   xip
 * read dummy data cnt, dummy is always single wire mode, dummy number = (dummy_cnt_add,dummy_cnt)
 * + 1.XIP ONLY. BIT[12:15]   xip read transmode/lcd display transmode. the transfer sequence could
 * be: 0x0:write and read at the same time(must enable CmdEn). 0x1:write only. 0x2:read only(must
 * enable CmdEn). 0x3:write,read. 0x4:read,write. 0x5:write,dummy,read. 0x6:read,dummy,write(must
 * enable CmdEn). 0x7:None Data(must enable CmdEn). 0x8:Dummy,write. 0x9:Dummy,read. 0xa:Dummy,write
 * and read. 0xb~0xf:reserved. BIT[16]      enable spi second cmd. BIT[17]      select token
 * val.0:0x00,1:0x69. BIT[18]      enable token mode. BIT[19]      bit[4] dummy_cnt_add. BIT[21]
 * enable page mode. BIT[22]      enable timeout mode(after the set time, the slice selection is
 * pulled up). BIT[23]      enable tcem mode(the chip selection pin is pulled low for more than a
 * set amount of time and then pulled up). BIT[24:31]   read command used for xip.
 */
...
typedef enum
{
    FLD_SPI_XIP_RD_DATA_LANE_MODE = TLK_BIT_RNG(0, 1),
    FLD_SPI_XIP_RD_ADDR_LEN       = TLK_BIT_RNG(2, 3),
    FLD_SPI_XIP_RD_ADDR_FMT       = TLK_BIT(4),
    FLD_SPI_XIP_RD_ADDR_EN        = TLK_BIT(5),
    FLD_SPI_XIP_RD_CMD_FMT        = TLK_BIT(6),
    FLD_SPI_XIP_RD_CMD_EN         = TLK_BIT(7),
    FLD_SPI_XIP_RD_DUMMY_CNT      = TLK_BIT_RNG(8, 11),
    FLD_SPI_XIP_RD_TRANSMODE      = TLK_BIT_RNG(12, 15),
    FLD_SPI_XIP_RD_CMD1_EN        = TLK_BIT(16),
    FLD_SPI_XIP_RD_TOKEN_VAL_SEL  = TLK_BIT(17),
    FLD_SPI_XIP_RD_TOKEN_EN       = TLK_BIT(18),
    FLD_SPI_XIP_RD_DUMMY_CNT_ADD  = TLK_BIT(19),
    FLD_SPI_XIP_RD_PAGE_MODE      = TLK_BIT(21),
    FLD_SPI_XIP_RD_TIMEOUT_MODE   = TLK_BIT(22),
    FLD_SPI_XIP_RD_TCEM_MODE      = TLK_BIT(23),
    FLD_SPI_XIP_RD_CMD            = TLK_BIT_RNG(24, 31),
} spi_xip_rd_fmt_e;

/**
 * BIT[0:1]  spi xip write I/O mode, 2'b00 single,2'b01 dual, 2'b10 quad. XIP ONLY
 * BIT[2:3]  xip write address length.2'b00:1byte  2'b01:2bytes  2'b10:3bytes  2'b11:4bytes.XIP
 * ONLY. BIT[4]  set xip write address format 0:single mode  1:the format of addr phase is the same
 * as the data phase(Dual/Quad),XIP ONLY. BIT[5]  1:enable xip write addr phase, XIP ONLY. BIT[6]
 * xip write cmd format 0: single mode  1: the format of the cmd phase is the same as the data
 * phase(Dual/Quad),XIP ONLY. BIT[7]  xip write spi command phase enable,XIP ONLY. BIT[8:11]   xip
 * read dummy data cnt, dummy is always single wire mode, dummy number = (dummy_cnt_add,dummy_cnt)
 * + 1.XIP ONLY. BIT[12:15]   xip write transmode/lcd display transmode. the transfer sequence could
 * be: 0x0:write and read at the same time(must enable CmdEn). 0x1:write only. 0x2:read only(must
 * enable CmdEn). 0x3:write,read. 0x4:read,write. 0x5:write,dummy,read. 0x6:read,dummy,write(must
 * enable CmdEn). 0x7:None Data(must enable CmdEn). 0x8:Dummy,write. 0x9:Dummy,read. 0xa:Dummy,write
 * and read. 0xb~0xf:reserved. BIT[16]      enable spi second cmd. BIT[24:31]   write command used
 * for xip.
 */
...
typedef enum
{
    FLD_SPI_XIP_WR_DATA_LANE_MODE = TLK_BIT_RNG(0, 1),
    FLD_SPI_XIP_WR_ADDR_LEN       = TLK_BIT_RNG(2, 3),
    FLD_SPI_XIP_WR_ADDR_FMT       = TLK_BIT(4),
    FLD_SPI_XIP_WR_ADDR_EN        = TLK_BIT(5),
    FLD_SPI_XIP_WR_CMD_FMT        = TLK_BIT(6),
    FLD_SPI_XIP_WR_CMD_EN         = TLK_BIT(7),
    FLD_SPI_XIP_WR_DUMMY_CNT      = TLK_BIT_RNG(8, 11),
    FLD_SPI_XIP_WR_TRANSMODE      = TLK_BIT_RNG(12, 15),
    FLD_SPI_XIP_WR_CMD1_EN        = TLK_BIT(16),
    FLD_SPI_XIP_WR_CMD            = TLK_BIT_RNG(24, 31),
} spi_xip_wr_fmt_e;
...
typedef enum
{
    FLD_GSPI_XIP0_END_ADDR = TLK_BIT_RNG(0, 1),
    FLD_GSPI_XIP1_END_ADDR = TLK_BIT_RNG(2, 3),
    FLD_GSPI_XIP2_END_ADDR = TLK_BIT_RNG(4, 5),
    FLD_GSPI_XIP3_END_ADDR = TLK_BIT_RNG(6, 7),
} gspi_xip_size_set_e;
...
enum
{
    FLD_LSPI_LCD_PIXEL_PER_LINE = TLK_BIT_RNG(0, 9),
    FLD_LSPI_LCD_LINE_PER_FRAME = TLK_BIT_RNG(10, 19),
};
...
typedef enum
{
    FLD_LSPI_LCD_LINE_INT_STA  = TLK_BIT(0),
    FLD_LSPI_LCD_LVL_INT_STA   = TLK_BIT(1),
    FLD_LSPI_LCD_FRAME_INT_STA = TLK_BIT(2),
 ...
 * BIT[4] lcd_line_lvl_irq_mask 1: The number of send line reached trigger level would trigger
 * interrupt. BIT[5] lcd_frame_irq_mask 1: every frame send complete would trigger interrupt.
 */
#define reg_lspi_lcd_ctrl2 REG_ADDR8(LSPI_BASE_ADDR + 0x2f)

enum
{
    FLD_LSPI_LCD_SINGLE_COLOR_MODE   = TLK_BIT(0),
    FLD_LSPI_LCD_RGB_BIG_ENDIAN_MODE = TLK_BIT(1),
    FLD_LSPI_LCD_RAM_4BIT_MODE       = TLK_BIT(2),
};

typedef enum
{
    FLD_LSPI_LCD_LINE_INT_EN  = TLK_BIT(3),
    FLD_LSPI_LCD_LVL_INT_EN   = TLK_BIT(4),
    FLD_LSPI_LCD_FRAME_INT_EN = TLK_BIT(5),
} lspi_lcd_irq_mask;
...
enum
{
    FLD_LSPI_LCD_SCAN_EN      = TLK_BIT(0),
    FLD_LSPI_LCD_RGB_MODE     = TLK_BIT_RNG(1, 2),
    FLD_LSPI_LCD_2LANE_EN     = TLK_BIT(3),
    FLD_LSPI_LCD_3LINE_DCX_EN = TLK_BIT(6),
    FLD_LSPI_LCD_DCX          = TLK_BIT(7),
};
...
enum
{
    FLD_LSPI_LCD_SEND_LINE_COUNT = TLK_BIT_RNG(0, 9),
};
...
enum
{
    FLD_LSPI_LCD_DISPLAY_TRANSMODE = TLK_BIT_RNG(4, 7),
};
...
enum
{
    FLD_LSPI_LCD_PORCH_TRANSMODE = TLK_BIT_RNG(4, 7),
};
