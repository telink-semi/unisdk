 ...
 * @param[in]   sp_key_idx       - index of secure port key, (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX],
 *                                 if the MSB(sp_key_idx) is 1, that means using low 128bit of the
 256bit key.
 ...
 * @note
  @verbatim
      ...
      -# 3.if key is from user input, please make sure key is not NULL(now sp_key_idx is useless),
          otherwise, key is from secure port, and (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX].
      -# 4.iv must be 12 bytes here.
      -# 5.aad_bytes and c_bytes could not be zero at the same time.
  @endverbatim
 */
    unsigned int ske_lp_gcm_init(SKE_GCM_CTX* ctx, SKE_ALG alg, SKE_CRYPTO crypto,
                                 unsigned char* key, unsigned short sp_key_idx, unsigned char* iv,
                                 unsigned int iv_bytes, unsigned int aad_bytes,
                                 unsigned int c_bytes, unsigned int mac_bytes);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int ske_lp_gcm_update_blocks(SKE_GCM_CTX* ctx, unsigned char* in, unsigned char* out,
                                          unsigned int bytes);
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
  ...
  @endverbatim
 */
    unsigned int ske_lp_gcm_crypto(SKE_ALG alg, SKE_CRYPTO crypto, unsigned char* key,
                                   unsigned short sp_key_idx, unsigned char* iv,
                                   unsigned int iv_bytes, unsigned char* aad,
                                   unsigned int aad_bytes, unsigned char* in, unsigned char* out,
                                   unsigned int c_bytes, unsigned char* mac,
                                   unsigned int mac_bytes);

#ifdef CONFIG_TLK_CRYPTO_DMA_SUPPORT
    /**
 * @brief       ske_lp gcm mode init config(CPU style)
 * @param[in]   ctx              - SKE_GCM_CTX context pointer.
 * @param[in]   alg              - ske_lp algorithm.
 ...
 *        otherwise, key is from secure port, and (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX].
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_gcm_init(SKE_GCM_CTX* ctx, SKE_ALG alg, SKE_CRYPTO crypto,
                                     unsigned char* key, unsigned short sp_key_idx,
                                     unsigned char* iv, unsigned int iv_bytes,
 ...
 * @param[in]   aad              - aad.
 * @param[in]   bytes            - real byte length of aad.
 * @param[in]   callback         - callback function pointer, this could be NULL, means doing
 nothing.
 * @return      0:success     other:error
 * @note
  @verbatim
      ...
      -# 3.if the whole aad is too long, you could divide it by block(16 bytes), and if the whole
 aad byte length is not a multiple of 16, please make sure the last section contains the tail, then
 call this function to input the sections respectively. for example, if the whole aad byte length is
 65, it could be divided into 3 sections with byte length 48,16,1 respectively.
  @endverbatim
 */
    unsigned int ske_lp_dma_gcm_update_aad_blocks(SKE_GCM_CTX* ctx, unsigned int* aad,
                                                  unsigned int bytes, SKE_CALLBACK callback);
 ...
 * @note
 ...
 and if the whole plaintext/ciphertext byte length is not a multiple of 16, please make sure the
 last section contains the tail, then call this function to input the sections respectively. for
        example, if the whole plaintext/ciphertext byte length is 65, it could be divided into 3
 sections with byte length 48,16,1 respectively.
      -# 4.the output will be some blocks too, it has the same length as the input, and with padding
 0 if necessary.
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_gcm_update_blocks(SKE_GCM_CTX* ctx, unsigned int* in, unsigned int* out,
                                              unsigned int in_bytes, SKE_CALLBACK callback);
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
      ...
      -# 8.the output ciphertext/plaintext has the same number of blocks as the input
 plaintext/ciphertext, and followed by one block, it is mac with padding 0 if necessary, so is the
 second last block if necessary(ciphertext/plaintext).
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_gcm_crypto(SKE_ALG alg, SKE_CRYPTO crypto, unsigned char* key,
                                       unsigned short sp_key_idx, unsigned char* iv,
                                       unsigned int iv_bytes, unsigned int* aad,
                                       unsigned int aad_bytes, unsigned int* in, unsigned int* out,
                                       unsigned int c_bytes, unsigned int mac_bytes,
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
  ...
  @endverbatim
 */
    unsigned int ske_lp_gmac_init(SKE_GMAC_CTX* ctx, SKE_ALG alg, SKE_MAC mac_action,
                                  unsigned char* key, unsigned short sp_key_idx, unsigned char* iv,
                                  unsigned int iv_bytes, unsigned int aad_bytes,
                                  unsigned int c_bytes, unsigned int mac_bytes);
 ...
 * @note
  @verbatim
      ...
      -# 3.if the whole message is too long, you could divide it into some sections by byte, then
 call this function to input the sections respectively. for example, if the whole message byte
 ...
 * @note
  @verbatim
      ...
      -# 3.if ctx->mac_action is SKE_GENERATE_MAC, mac is output. and if ctx->mac_action is
 SKE_VERIFY_MAC, mac is input, return value SKE_SUCCESS means the mac is valid, otherwise mac is
 invalid.
  @endverbatim
 */
 ...
 *        mac is input, return value SKE_SUCCESS means the mac is valid, otherwise mac is invalid.
  @endverbatim
 */
    unsigned int ske_lp_gmac_crypto(SKE_ALG alg, SKE_MAC mac_action, unsigned char* key,
                                    unsigned short sp_key_idx, unsigned char* iv,
                                    unsigned int iv_bytes, unsigned char* aad,
                                    unsigned int aad_bytes, unsigned char* in, unsigned int c_bytes,
