 ...
 * @param[in]   nonce            - index of secure port key, (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX].
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int ske_lp_ccm_init(SKE_CCM_CTX* ctx, SKE_ALG alg, SKE_CRYPTO crypto,
                                 unsigned char* key, unsigned short sp_key_idx,
                                 unsigned char* nonce, unsigned char M, unsigned char L,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int ske_lp_ccm_update_blocks(SKE_CCM_CTX* ctx, unsigned char* in, unsigned char* out,
                                          unsigned int bytes);
 ...
 *        that means certification passed, otherwise not.
  @endverbatim
 */
    unsigned int ske_lp_ccm_crypto(SKE_ALG alg, SKE_CRYPTO crypto, unsigned char* key,
                                   unsigned short sp_key_idx, unsigned char* nonce, unsigned char M,
                                   unsigned char L, unsigned char* aad, unsigned int aad_bytes,
                                   unsigned char* in, unsigned char* out, unsigned int c_bytes,
                                   unsigned char* mac);
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_ccm_init(SKE_CCM_CTX* ctx, SKE_ALG alg, SKE_CRYPTO crypto,
                                     unsigned char* key, unsigned short sp_key_idx,
                                     unsigned char* nonce, unsigned char M, unsigned char L,
                                     unsigned int aad_bytes, unsigned int c_bytes);
     ...
     * @return      0:success     other:error
     */
    void ske_lp_ccm_get_B0(unsigned char* nonce, unsigned char M, unsigned char L,
                           unsigned int aad_bytes, unsigned int c_bytes, unsigned char out[16]);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_ccm_update_B0_block(SKE_CCM_CTX* ctx, unsigned int B0[4],
 ...
 * @note
  ...
  @endverbatim
 */
    void ske_lp_ccm_get_B1(unsigned char* aad, unsigned int aad_bytes, unsigned int* aad_offset,
 ...
 * @note
  @verbatim
      ...
      -# 4.the whole aad here must be with prefix(in B1), and padding the tail with 0 if
 necessary(to make it occupies a multiple of 16 bytes). and now whole aad byte length is prefix byte
 length (ctx->b1_aad_start_offset) + original aad byte length.
      -# 5.if the whole aad with prefix is too long, you could divide it by block(16 bytes), and if
 the whole aad byte length is not a multiple of 16, please make sure the last section contains the
         tail, then call this function to input the sections respectively. for example, if the whole
         aad byte length is 65, it could be divided into 3 sections with byte length 48,16,1
 respectively.
  @endverbatim
 */
    unsigned int ske_lp_dma_ccm_update_aad_blocks(SKE_CCM_CTX* ctx, unsigned int* aad,
 ...
 * @note
  @verbatim
      ...
      -# 3.f the whole plaintext/ciphertext is too long, you could divide it by block(16 bytes), and
 if the whole plaintext/ciphertext byte length is not a multiple of 16, please make sure the last
 ...
 0 if necessary.
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_ccm_update_blocks(SKE_CCM_CTX* ctx, unsigned int* in, unsigned int* out,
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
      ...
      -# 6.if aad exists, it must be some blocks, if not, please pad it with 0
        cover the input.
      -# 7.the output ciphertext/plaintext has the same number of blocks as the input
 ...
 second last block if necessary(ciphertext/plaintext).
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_ccm_crypto(SKE_ALG alg, SKE_CRYPTO crypto, unsigned char* key,
                                       unsigned short sp_key_idx, unsigned char* nonce,
                                       unsigned char M, unsigned char L, unsigned int aad_bytes,
                                       unsigned int* in, unsigned int* out, unsigned int c_bytes,
                                       SKE_CALLBACK callback);
#endif
