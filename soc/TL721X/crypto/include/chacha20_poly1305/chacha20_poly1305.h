 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int chacha20_poly1305_init_internal(CHACHA20_POLY1305_CRYPTO crypto,
                                                 unsigned char key[32], unsigned int constant,
                                                 unsigned char               iv[8],
                                                 CHACHA20_POLY1305_SEC_STAGE stage,
                                                 unsigned int                dma_en);
     ...
     * @return      0:success     other:error
     */
    unsigned int chacha20_poly1305_init(CHACHA20_POLY1305_CTX* ctx, CHACHA20_POLY1305_CRYPTO crypto,
                                        unsigned char key[32], unsigned int constant,
                                        unsigned char iv[8], unsigned char* aad,
                                        unsigned long long aad_bytes);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int chacha20_poly1305_update_excluding_last_data(CHACHA20_POLY1305_CTX* ctx,
                                                              unsigned char*         payload_in,
                                                              unsigned char*         payload_out,
                                                              unsigned long long     payload_bytes);

    /**
 * @brief       chacha20_poly1305 update message including the last data (CPU mode), and get the
 tag.
 * @param[in]   ctx                - CHACHA20_POLY1305_CTX context pointer.
 ...
 *        payload_out could set NULL and payload_bytes should be 0.
  @endverbatim
 */
    unsigned int chacha20_poly1305_update_including_last_data(CHACHA20_POLY1305_CTX* ctx,
                                                              unsigned char*         payload_in,
                                                              unsigned char*         payload_out,
                                                              unsigned long long     payload_bytes,
 ...
 * @note
  @verbatim
      -# 1. aad_bytes and payload_bytes could be any value. if length bytes is 0, could set pointer
 is NULL.
  @endverbatim
 */
    unsigned int chacha20_poly1305(CHACHA20_POLY1305_CRYPTO crypto, unsigned char key[32],
                                   unsigned int constant, unsigned char iv[8], unsigned char* aad,
                                   unsigned long long aad_bytes, unsigned char* payload_in,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int chacha20_poly1305_dma_init(CHACHA20_POLY1305_DMA_CTX* ctx,
                                            CHACHA20_POLY1305_CRYPTO crypto, unsigned char key[32],
                                            unsigned int constant, unsigned char iv[8],
                                            unsigned int* aad, unsigned int aad_bytes);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int chacha20_poly1305_dma_update_excluding_last_data(CHACHA20_POLY1305_DMA_CTX* ctx,
                                                                  unsigned int* payload_in,
                                                                  unsigned int* payload_out,
                                                                  unsigned int  payload_bytes);

    /**
 * @brief       chacha20_poly1305 update message including the last data (DMA mode), and get the
 tag.
 * @param[in]   ctx                - CHACHA20_POLY1305_DMA_CTX context pointer.
 * @param[in]   payload_in         - payload, including last block(or message tail).
 * @param[out]  payload_out        - ciphertext or plaintext.
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int chacha20_poly1305_dma_update_including_last_data(CHACHA20_POLY1305_DMA_CTX* ctx,
                                                                  unsigned int* payload_in,
                                                                  unsigned int* payload_out,
                                                                  unsigned int  payload_bytes,
                                                                  unsigned char tag[16]);
 ...
 * @param[in]   aad                   - key in bytes.
 * @param[in]   aad_bytes             - byte length of aad.
 * @param[in]   payload_in            - payload of some blocks, excluding last block(or message
 tail).
 * @param[out]  payload_out           - ciphertext or plaintext.
 * @param[in]   payload_bytes         - byte length of payload, could be 0.
 ...
 * @note
 ...
 is NULL.
  ...
  @endverbatim
 */
    unsigned int chacha20_poly1305_dma(CHACHA20_POLY1305_CRYPTO crypto, unsigned char key[32],
                                       unsigned int constant, unsigned char iv[8],
                                       unsigned int* aad, unsigned int aad_bytes,
                                       unsigned int* payload_in, unsigned int* payload_out,
                                       unsigned int payload_bytes, unsigned char tag[16]);
     ...
     * @return None.
     */
    void telink_chacha20_dma_init(enum tlk_dma_chn tx_chn, enum tlk_dma_chn rx_chn,
                                  enum tlk_dma_burst_size burst_size);
