...
#define ROSR_CTRL4 (*(volatile unsigned int*) (0x80100804))
 ...
 * @param[in]   cfg       - RO entropy config, only the low 4 bits are valid, every bit,indicates
 one RO entropy, the MSB is RO 0, and LSB is RO 3.
 ...
 */
     ...
     *                                  0: 1/4 of input frequency, the lower the sample frequency,
     * the better the randomization performance, 1: 1/8 ..., 2: 1/16 ..., 3: 1/32 ...,
     * @return      TRNG_SUCCESS(success), other(error)
     */
 ...
 * @note
  @verbatim
      -# 1. Directly from the entropy source to take the random number,
            ...
            if you get the true random number and the current mode is configured as a
 post-processing, you can use trng_disable, trng_set_mode, trng_enable interface function to set to
 the true random mode, and then call the function, if you get the pseudo-random please use get_rand.
  @endverbatim
 */
 ...
 * * @note
  @verbatim
      -# 1. This interface uses pseudo-random mode by default, through post-processing, the
 randomness will be better and easier to pass trng-related authentication
 ...
 */
    ...
    /**
     * @brief       TRNG power-on self-test disable (enable trng_skip_startup)
     * @return      none
     */
    void trng_skip_startup_enable(void);

    /**
     * @brief       TRNG power-on self-test enable (disable trng_skip_startup)
     * @return      none
     */
    void trng_skip_startup_disable(void);
