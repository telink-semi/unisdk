 ...
 * @param[in]  prikey              - private key, 32 bytes, little-endian
 * @param[in]  pubkey              - public key, 32 bytes, little-endian, if no pubkey, please set
 it to be NULL
 ...
 * @note
  @verbatim
      ...
      -# 5. if mode is Ed25519_PH, ctx length is from 0 to 255, default length is 0, thus ctx could
 be empty
  @endverbatim
 */
    unsigned int ed25519_sign(Ed25519_MODE mode, unsigned char prikey[32], unsigned char pubkey[32],
                              unsigned char* ctx, unsigned char ctxByteLen, unsigned char* M,
 ...
 * @note
  @verbatim
      ...
      -# 4. if mode is Ed25519_PH, ctx length is from 0 to 255, default length is 0, thus ctx could
 be empty
  @endverbatim
 */
    unsigned int ed25519_verify(Ed25519_MODE mode, unsigned char pubkey[32], unsigned char* ctx,
                                unsigned char ctxByteLen, unsigned char* M, unsigned int MByteLen,
                                unsigned char RS[64]);
