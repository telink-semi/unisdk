...
void dma_reverse_word_array(unsigned int *in, unsigned int *out, unsigned int wordlen, unsigned int reverse_word);
     ...
     * @return      the number of valid bits in the array, or 0 if the array is empty or all
     * elements are zero
     */
     ...
     */
    int uint32_BigNumCmp(volatile unsigned int* a, unsigned int aWordLen, volatile unsigned int* b,
 ...
 */
     ...
     * @return      ret_zero(k is zero)   ret_big(k is greater/bigger than or equal to n)
     * ret_success(k is in [1, n-1])
     */
    unsigned int uint32_integer_check(unsigned int* k, unsigned int* n, unsigned int wordLen,
                                      unsigned int ret_zero, unsigned int ret_big,
