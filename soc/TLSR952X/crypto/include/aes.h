 ...
 *
 *  - Use the function aes_set_em_base_addr() can modify the aes module em_base_address, but it's
 * not recommended to call when using a BT-related SDK.
 *
 *
 */
#ifndef _AES_H_
...
#include "registers/tlk_aes_reg.h"
 ...
 *********************************************************************************************************************/
/**
 * @brief     This function serves to aes finite state machine reset(the configuration register is
 * still there and does not need to be reconfigured).
 * @return    none.
 */
 ...
 * @return    none.
 * @note      The default timeout (g_aes_error_timeout_us) is the larger value.If the timeout
 * exceeds the feed dog time and triggers a watchdog restart, g_aes_error_timeout_us can be changed
 * to a smaller value via this interface, depending on the application. g_aes_error_timeout_us the
 * minimum time must meet the following conditions:
 *            1. at least 100us;
 ...
 * @return    none.
 * @note      This function can be rewritten according to the application scenario,can read the
 * parameters of the interface to obtain details about the timeout reason(aes_api_error_code_e),
 *            aes_hw_fsm_reset() must be called.
 */
__attribute__((weak)) void aes_timeout_handler(unsigned int aes_error_timeout_code);

/**
 * @brief      This function servers to perform aes_128 encryption for 16-Byte input data with
 ...
 *             DRV_API_TIMEOUT: timeout exit(g_aes_error_timeout_us refer to the note for
 * aes_set_error_timeout,the solution processing is already done in aes_timeout_handler, so just
 * re-invoke the interface);
 */
int aes_encrypt(unsigned char* key, unsigned char* plaintext, unsigned char* result);

/**
 * @brief      This function servers to perform aes_128 encryption for 16-Byte input data with
 * specific 16-Byte key when BT is connected.
 * @param[in]  key       - the key of encrypt, big--endian.
 * @param[in]  plaintext - the plaintext of encrypt, big--endian.
 * @param[out] result    - the result of encrypt, big--endian.
 * @return     DRV_API_TIMEOUT: timeout exit(g_aes_error_timeout_us refer to the note for
 * aes_set_error_timeout,the solution processing is already done in aes_timeout_handler, so just
 * re-invoke the interface);
...
/**
 * @brief      This function servers to perform aes_128 decryption for 16-Byte input data with
 * specific 16-Byte key.
 ...
 * @param[in] addr - The range of em base address that can be set is the address space of DLM and
 * ILM, which can view the Memory Map of datasheets. The current driver default setting is
 * em_base_addr = 0xc0000000, if you call this function to modify the em base address, you need to
 * ensure that the _attribute_aes_data_sec_ section in the link file (AES-related functions will use
 * this section) is set in the following address range: [em_base_addr,em_base_addr+64KB] (chip
 * design requirements)
 * @return    none.
 * @attention If you are using a BT-related SDK, you must follow the planning of BT's sdk to handle
 * this address and not call this function
 */
 ...
 * @return    none.
 * @note      The AES module register must be used by word and the key and data lengths must be 16
 * bytes.
 */
void aes_set_key_data(unsigned char* key, unsigned char* data);

/**
 * @brief      This function refer to encrypt/decrypt to get result. AES module register must be
 * used by word.
