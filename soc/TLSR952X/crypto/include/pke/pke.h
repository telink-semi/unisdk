...
#define MIN_RSA_BIT_LEN  (512)
    ...
    // define KDF
    typedef void* (*KDF_FUNC)(const void* input, unsigned int byteLen, unsigned char* key,
     ...
     */
    unsigned int pke_modinv(const unsigned int* modulus, volatile const unsigned int* a,
                            unsigned int* ainv, unsigned int modWordLen, unsigned int aWordLen);
     ...
     */
    unsigned int pke_modadd(const unsigned int* modulus, volatile const unsigned int* a,
                            volatile const unsigned int* b, unsigned int* out,
     ...
     *              2. wordLen must not be bigger than OPERAND_MAX_WORD_LEN.
     */
    unsigned int pke_modsub(const unsigned int* modulus, const unsigned int* a,
                            const unsigned int* b, unsigned int* out, unsigned int wordLen);
     ...
     *              2. wordLen must not be bigger than OPERAND_MAX_WORD_LEN.
     */
    unsigned int pke_add(volatile const unsigned int* a, volatile const unsigned int* b,
     ...
     *              2. wordLen must not be bigger than OPERAND_MAX_WORD_LEN.
     */
    unsigned int pke_sub(volatile const unsigned int* a, const unsigned int* b,
                         volatile unsigned int* out, unsigned int wordLen);
     ...
     * @param[out]  out     - out = a*b.
     * @return      PKE_SUCCESS(success), other(error).
     * @caution     1. please make sure out buffer word length is bigger than
     * (2*max_bit_len(a,b)+0x1F)>>5.
     *              2. please make sure ab_wordLen is not bigger than OPERAND_MAX_WORD_LEN/2.
     */
    unsigned int pke_mul(const unsigned int* a, const unsigned int* b, volatile unsigned int* out,
     ...
     *              2. wordLen must not be bigger than OPERAND_MAX_WORD_LEN.
     */
    void pke_load_pre_calc_mont(volatile const unsigned int* H, volatile const unsigned int* n1,
                                unsigned int wordLen);
     ...
     */
    unsigned int pke_modmul_internal(volatile const unsigned int* modulus,
                                     volatile const unsigned int* a, volatile const unsigned int* b,
                                     volatile unsigned int* out, unsigned int wordLen);
     ...
     *              3. wordLen must not be bigger than OPERAND_MAX_WORD_LEN.
     */
    unsigned int pke_modmul(const unsigned int* modulus, const unsigned int* a,
                            const unsigned int* b, unsigned int* out, unsigned int wordLen);

    /**
     * @brief       mod exponent, this could be used for rsa
     ...
     * @param[out]  out         - out = a^(b) mod modulus.
     * @return      PKE_SUCCESS(success), other(error)
     * @caution     1. before calling this function, please make sure R^2 mod modulus, the
     ...
     */
    unsigned int pke_modexp(volatile const unsigned int* modulus,
                            volatile const unsigned int* exponent,
                            volatile const unsigned int* base, volatile unsigned int* out,
     ...
     *              2. please make sure aWordLen <= 2*OPERAND_MAX_WORD_LEN, bWordLen <=
     * OPERAND_MAX_WORD_LEN.
     *              3. real bit length of a can not be bigger than 2*(real bit length of b).
     */
    unsigned int pke_mod(unsigned int* a, unsigned int aWordLen, const unsigned int* b,
                         volatile const unsigned int* b_h, volatile const unsigned int* b_n1,
     ...
     */
    unsigned int eccp_pointMul(eccp_curve_t* curve, unsigned int* k, const unsigned int* Px,
                               const unsigned int* Py, volatile unsigned int* Qx,
                               volatile unsigned int* Qy);
...
// #define ECCP_POINT_DOUBLE   //recommended not to define
    ...
    /**
     * @brief       get ECCP public key from private key(the key pair could be used in
     * SM2/ECDSA/ECDH).
     * @param[in]   curve   - eccp_curve_t curve struct pointer.
     ...
     * @return      PKE_SUCCESS(success), other(error).
     * @caution     1. please make sure input point P is on the curve.
     *              2. even if the input point P is valid, the output may be infinite point, in this
     * case return error.
     *              3. please make sure the curve is c25519.
     */
    unsigned int x25519_pointMul(mont_curve_t* curve, unsigned int* k, const unsigned int* Pu,
                                 unsigned int* Qu);
     ...
     * @return      PKE_SUCCESS(success), other(error)
     * @caution     1. please make sure input point P is on the curve.
     *              2. even if the input point P is valid, the output may be neutral point (0, 1),
     * it is valid.
