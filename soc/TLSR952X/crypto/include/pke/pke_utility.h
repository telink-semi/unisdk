...
#define GET_BYTE_LEN(bitLen) (((bitLen) + 7) / 8)
     ...
     * @return      none.
     */
    void uint32_copy(volatile unsigned int* dst, volatile const unsigned int* src,
     ...
     */
    void reverse_byte_array(volatile const unsigned char* in, unsigned char* out,
     ...
     * @return      none.
     * @caution     in DAM mode, the memory may be accessed by words, not by bytes, this function is
     * designed for the case
     */
    // void dma_reverse_word_array(unsigned int *in, unsigned int *out, unsigned int wordlen,
    // unsigned int reverse_word);
     ...
     */
    int uint32_BigNumCmp(volatile const unsigned int* a, unsigned int aWordLen,
                         volatile const unsigned int* b, unsigned int bWordLen);
