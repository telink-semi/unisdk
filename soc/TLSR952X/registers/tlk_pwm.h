...
typedef enum tlk_pwm_en0_e {
    TLK_PWM_EN0_PWM0_EN = TLK_BIT(0),
} tlk_pwm_en0_e;

typedef enum tlk_pwm_mode_e {
    TLK_PWM_MODE_CRUN          = TLK_BIT(0),
    TLK_PWM_MODE_CATCH         = TLK_BIT(1),
    TLK_PWM_MODE_FIFIO_MODE_EN = TLK_BIT(2),
} tlk_pwm_mode_e;
...
typedef enum tlk_pwm_mask_e {
    TLK_PWM_MASK_MASK_PWM  = TLK_BIT(0),
    TLK_PWM_MASK_MASK_FIFO = TLK_BIT(1),
} tlk_pwm_mask_e;

typedef enum tlk_pwm_irq_status_e {
    TLK_PWM_IRQ_STATUS_PWM_IRQ       = TLK_BIT(0),
    TLK_PWM_IRQ_STATUS_FIFO_DONE_IRQ = TLK_BIT(1),
} tlk_pwm_irq_status_e;
...
typedef struct __attribute__((packed)) {
    ...
    union {
        ...
        struct {
            enum tlk_pwm_mask_e __attribute__((packed)) mask : 8;
    ...
    union {
        ...
        struct {
            enum tlk_pwm_irq_status_e __attribute__((packed)) irq_status : 8;
