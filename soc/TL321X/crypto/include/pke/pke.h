...
#define PKE_ERROR           (9U)
     ...
     */
    unsigned int ed25519_decode_point(unsigned char in_y[32], unsigned char out_x[32],
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int pke_modinv(unsigned int* modulus, unsigned int* a, unsigned int* ainv,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int pke_mul_internal(unsigned int* a, unsigned int* b, unsigned int* out,
                                  unsigned int a_wordLen, unsigned int b_wordLen,
                                  unsigned int out_wordLen);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int pke_mul(unsigned int* a, unsigned int* b, unsigned int* out,
                         unsigned int ab_wordLen);

#ifndef PKE_CONFIG_ALL_MODEXP_PRE_CALC_WITH_MGMR_MICROCODE
    /**
 * @brief       calc H(R^2 mod modulus) and n0'( - modulus ^(-1) mod 2^w ) for modMul,modExp, and
 pointMul. etc
 ...
 *        and n0 only need one word.
  ...
  @endverbatim
 */
    unsigned int pke_pre_calc_mont_without_mgmr_microcode(unsigned int* modulus,
                                                          unsigned int bitLen, unsigned int* H,
                                                          unsigned int* n0);
#endif
    /**
 * @brief       calc H(R^2 mod modulus) and n0'( - modulus ^(-1) mod 2^w ) for modMul,modExp, and
 ...
 *        and n0 only need one word.
  ...
  @endverbatim
 */
    unsigned int pke_pre_calc_mont(unsigned int* modulus, unsigned int bitLen, unsigned int* H,
                                   unsigned int* n0);
 ...
 * @note
  @verbatim
      -# 1.modulus must be odd.
      -# 2.please make sure word length of buffer H is equal to wordLen(word length of modulus),and
 n0 only need one word.
      -# 3.bitLen must not be bigger than OPERAND_MAX_BIT_LEN
   @endverbatim
 */
    unsigned int pke_pre_calc_mont_for_modexp(unsigned int* modulus, unsigned int bitLen,
                                              unsigned int* H, unsigned int* n0);
 ...
 * @note
  @verbatim
   ...
   @endverbatim
 */
    unsigned int pke_load_modulus_and_pre_monts(unsigned int* modulus, unsigned int* modulus_h,
                                                unsigned int* modulus_n0, unsigned int bitLen);

    /**
 * @brief       set modulus and pre-calculated mont parameters H(R^2 mod modulus) and n0'(- modulus
 ^(-1) mod 2^w) for hardware operation
 * @param[in]   modulus             - modulus.
 ...
 * @note
  @verbatim
   ...
   @endverbatim
 */
    unsigned int pke_set_modulus_and_pre_monts(unsigned int* modulus, unsigned int* modulus_h,
                                               unsigned int* modulus_n0, unsigned int bitLen);
    /**
 * @brief       set modulus and pre-calculated mont parameters H(R^2 mod modulus) and n0'(- modulus
 ^(-1) mod 2^w) for hardware operation.
 ...
 * @note
 ...
 arguments of modulus are located in the right address.
  @endverbatim
 */
    unsigned int pke_modmul_internal(unsigned int* a, unsigned int* b, unsigned int* out,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int pke_modmul(unsigned int* modulus, unsigned int* a, unsigned int* b,
                            unsigned int* out, unsigned int wordLen);
 ...
 * @note
  @verbatim
      -# before calling this function, please make sure the pre-calculated mont arguments of modulus
 are located in the right address
 ...
 * @param[in]   mod_wordLen          - word length of modulus and base number.
 * @param[in]   exp_wordLen          - word length of exponent.
 * @return      PKE_SUCCESS(input is valid, allow to calculate),PKE_FINISHED(mod exponent
 finished),other(error)
 * @note
  @verbatim
      -# 1.modulus must be odd
  ...
  @endverbatim
 */
    unsigned int pke_modexp_check_input(unsigned int* modulus, unsigned int* exponent,
                                        unsigned int* base, unsigned int* out,
                                        unsigned int mod_wordLen, unsigned int exp_wordLen);

    /**
 * @brief       od exponent(for high level use, operands are all U8 big-endian big number), this
 could be used for rsa encrypting,decrypting,signing,verifying.
 * @param[in]   modulus              - modulus.
 * @param[in]   exponent             - exponent.
 * @param[in]   base                 - base number.
 * @param[out]  out                  - out = base^(exponent) mod modulus.
 ...
 * @param[in]   calc_pre_monts       - if it is 0, no need to calculate the pre-calculated mont
 arguments of modulus, otherwise calculate.
 * @return      PKE_SUCCESS(input is valid, allow to calculate),PKE_FINISHED(mod exponent
 finished),other(error)
 * @note
  @verbatim
      -# 1.modulus must be odd
      -# 2.please make sure exp_wordLen <= mod_wordLen <= OPERAND_MAX_WORD_LEN
      -# 3.this is for high level application or protocol to use RSA mod exponent directly. all
 ...
 */
    unsigned int pke_modexp_U8(unsigned char* modulus, unsigned char* exponent, unsigned char* base,
                               unsigned char* out, unsigned int mod_bitLen, unsigned int exp_bitLen,
                               unsigned int calc_pre_monts);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int pke_mod(unsigned int* a, unsigned int aWordLen, unsigned int* b, unsigned int* b_h,
                         unsigned int* b_n0, unsigned int bWordLen, unsigned int* c);
    /**
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int eccp_pointMul(eccp_curve_t* curve, unsigned int* k, unsigned int* Px,
                               unsigned int* Py, unsigned int* Qx, unsigned int* Qy);
 ...
 * @note
  @verbatim
      ...
      -# 2.please make sure input point P is on the curve.
      -# 3.even if the input point P1 and P2 are valid, it will return error in the following 2
 cases. (1). P1 = P2. return PKE_NO_MODINV. (2). P1 = -P2. return PKE_NO_MODINV. actually the output
 point is neutral point(point at infinity)
  @endverbatim
 */
    unsigned int eccp_pointAdd(eccp_curve_t* curve, unsigned int* P1x, unsigned int* P1y,
                               unsigned int* P2x, unsigned int* P2y, unsigned int* Qx,
                               unsigned int* Qy);
 ...
 * @note
  @verbatim
      ...
      -# 3.if P1 = -P2, it will return PKE_NO_MODINV. actually the output point is neutral
 point(point at infinity)
  @endverbatim
 */
    unsigned int eccp_pointAdd_safe(eccp_curve_t* curve, unsigned int* P1x, unsigned int* P1y,
                                    unsigned int* P2x, unsigned int* P2y, unsigned int* Qx,
                                    unsigned int* Qy);

// #define ECCP_POINT_DOUBLE   //not recommended to define
#ifdef ECCP_POINT_DOUBLE
    unsigned int eccp_pointDouble(eccp_curve_t* curve, unsigned int* Px, unsigned int* Py,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int eccp_get_pubkey_from_prikey(eccp_curve_t* curve, unsigned char* priKey,
 ...
 * @note
  @verbatim
      ...
      -# 2.even if the input point P is valid, the output may be infinite point, in this case return
 error.
      -# 3.please make sure the curve is c25519
  @endverbatim
 */
    unsigned int x25519_pointMul(mont_curve_t* curve, unsigned int* k, unsigned int* Pu,
                                 unsigned int* Qu);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int ed25519_pointMul(edward_curve_t* curve, unsigned int* k, unsigned int* Px,
                                  unsigned int* Py, unsigned int* Qx, unsigned int* Qy);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int ed25519_pointAdd(edward_curve_t* curve, unsigned int* P1x, unsigned int* P1y,
                                  unsigned int* P2x, unsigned int* P2y, unsigned int* Qx,
                                  unsigned int* Qy);
