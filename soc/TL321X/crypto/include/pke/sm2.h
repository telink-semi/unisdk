 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int sm2_getZ(unsigned char* ID, unsigned int byteLenofID, unsigned char pubKey[65],
                          unsigned char Z[32]);
     ...
     * @return      0:success     other:error
     */
    unsigned int sm2_getE(unsigned char* M, unsigned int byteLen, unsigned char Z[32],
                          unsigned char E[32]);
 ...
 */
    ...
    /**
 * @brief       step 3 of getting SM2 E value(stepwise style), message update done, get the
 digest(SM2 E value)
 * @param[in]   ctx           - input, HASH_CTX context pointer
 ...
 *                               if you do not have this integer, please set this parameter to be
 NULL,
 ...
 * @note
  @verbatim
      -# 1.if you do not have rand_k, please set the parameter to be NULL, it will be generated
 inside.
  @endverbatim
 */
    unsigned int sm2_sign(unsigned char E[32], unsigned char rand_k[32], unsigned char priKey[32],
     ...
     * @return      SM2_SUCCESS(success)     other:error
     */
    unsigned int sm2_verify(unsigned char E[32], unsigned char pubKey[65],
                            unsigned char signature[64]);
 ...
 * @note
  @verbatim
      ...
      -# 2.if you do not have rand_k, please set the parameter to be NULL, it will be generated
 inside.
      -# 3.please make sure pubKey is valid
  @endverbatim
 */
    unsigned int sm2_encrypt(unsigned char* M, unsigned int MByteLen, unsigned char rand_k[32],
                             unsigned char pubKey[65], sm2_cipher_order_e order, unsigned char* C,
                             unsigned int* CByteLen);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int sm2_exchangekey(sm2_exchange_role_e role, unsigned char* dA, unsigned char* PB,
                                 unsigned char* rA, unsigned char* RA, unsigned char* RB,
                                 unsigned char* ZA, unsigned char* ZB, unsigned int kByteLen,
...
#define SM2_ERROR_S   (0xCBAD735EU)
    ...
    unsigned int sm2_sign_s(unsigned char E[32], unsigned char rand_k[32], unsigned char priKey[32],
                            unsigned char signature[64]);

    unsigned int sm2_verify_s(unsigned char E[32], unsigned char pubKey[65],
                              unsigned char signature[64]);

    unsigned int sm2_encrypt_s(unsigned char* M, unsigned int MByteLen, unsigned char rand_k[32],
                               unsigned char pubKey[65], sm2_cipher_order_e order, unsigned char* C,
                               unsigned int* CByteLen);
    ...
    unsigned int sm2_exchangekey_s(sm2_exchange_role_e role, unsigned char* dA, unsigned char* PB,
                                   unsigned char* rA, unsigned char* RA, unsigned char* RB,
                                   unsigned char* ZA, unsigned char* ZB, unsigned int kByteLen,
                                   unsigned char* KA, unsigned char* S1, unsigned char* SA);
