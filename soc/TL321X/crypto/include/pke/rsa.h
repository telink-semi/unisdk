 ...
 * @note
  @verbatim
      -# 1.a, n, and out have the same word length:((nBitLen+31)>>5); and e word length is
 (eBitLen+31)>>5.
  @endverbatim
 */
    unsigned int RSA_ModExp(unsigned int* a, unsigned int* e, unsigned int* n, unsigned int* out,
 ...
 * @param[in]   p            - unsigned int big integer p, prime number, one part of private key
 (p,q,dp,dq,u).
 * @param[in]   q            - unsigned int big integer q, prime number, one part of private key
 (p,q,dp,dq,u).
 * @param[in]   dp           - unsigned int big integer dp = e^(-1) mod (p-1), one part of private
 key (p,q,dp,dq,u).
 * @param[in]   dq           - unsigned int big integer dq = e^(-1) mod (q-1), one part of private
 key (p,q,dp,dq,u).
 * @param[in]   u            - unsigned int big integer u = q^(-1) mod p, one part of private key
 (p,q,dp,dq,u).
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int RSA_CRTModExp(unsigned int* a, unsigned int* p, unsigned int* q, unsigned int* dp,
                               unsigned int* dq, unsigned int* u, unsigned int* out,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int RSA_GetKey(unsigned int* e, unsigned int* d, unsigned int* n, unsigned int eBitLen,
                            unsigned int nBitLen);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int RSA_GetCRTKey(unsigned int* e, unsigned int* p, unsigned int* q, unsigned int* dp,
                               unsigned int* dq, unsigned int* u, unsigned int* n,
                               unsigned int eBitLen, unsigned int nBitLen);
...
#define RSA_ERROR_S   (0x45DF3DAEU)
    ...
    unsigned int RSA_ModExp_with_pub(unsigned int* a, unsigned int* e, unsigned int* d,
                                     unsigned int* n, unsigned int* out, unsigned int eBitLen,
                                     unsigned int nBitLen);

    unsigned int RSA_CRTModExp_with_pub(unsigned int* a, unsigned int* p, unsigned int* q,
                                        unsigned int* dp, unsigned int* dq, unsigned int* u,
                                        unsigned int* e, unsigned int* out, unsigned int eBitLen,
                                        unsigned int nBitLen);
 ...
 * @param[out]  out          - if in is NULL, this is mask directly, otherwise,this is (mask XOR
 in).
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int rsa_pkcs1_mgf1_with_xor_in(HASH_ALG hash_alg, unsigned char* seed,
                                            unsigned int seed_bytes, unsigned char* in,
 ...
 * @param[in]   msg_digest           - Hash(message), message is to be signed, here Hash is
 msg_hash_alg.
 ...
 * @note
  @verbatim
      ...
      -# 3.salt_bytes should be in [0, em_bytes-digest_bytes-2], em_bytes is (em_bits+7)/8, em_bits
 is (n_bits-1). it is recommended to use default value, digest length of hash algorithm msg_hash_alg
 or mgf_hash_alg.
  @endverbatim
 */
    unsigned int rsa_ssa_pss_sign_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg,
                                                unsigned char* salt, unsigned int salt_bytes,
                                                unsigned char* msg_digest, unsigned char* d,
                                                unsigned char* n, unsigned int n_bits,
 ...
 * @note
 ...
 or mgf_hash_alg.
  @endverbatim
 */
    unsigned int rsa_ssa_pss_sign(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg, unsigned char* salt,
                                  unsigned int salt_bytes, unsigned char* msg,
                                  unsigned int msg_bytes, unsigned char* d, unsigned char* n,
 ...
 * @param[in]   msg_digest           - Hash(message), message is to be signed, here Hash is
 msg_hash_alg.
 * @param[in]   d                    - RSA-CRT private key (p,q,dp,dq,u), every field is
 (n_bits/2+7)/8 bytes, big-endian.
 * @param[out]  signature            - RSA signature, (n_bits+7)/8 bytes, big-endian.
 * @return      0:success     other:error
 * @note
  @verbatim
 ...
 or mgf_hash_alg.
  @endverbatim
 */
    unsigned int rsa_ssa_pss_crt_sign_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg,
                                                    unsigned char* salt, unsigned int salt_bytes,
                                                    unsigned char*       msg_digest,
                                                    RSA_CRT_PRIVATE_KEY* d, unsigned int n_bits,
                                                    unsigned char* signature);
 ...
 * @note
 ...
 or mgf_hash_alg.
  @endverbatim
 */
    unsigned int rsa_ssa_pss_crt_sign(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg,
                                      unsigned char* salt, unsigned int salt_bytes,
                                      unsigned char* msg, unsigned int msg_bytes,
                                      RSA_CRT_PRIVATE_KEY* d, unsigned int n_bits,
                                      unsigned char* signature);
 ...
 * @param[in]   mgf_hash_alg         - specific hash algorithm for MGF1
 * @param[in]   salt_bytes           - byte length of salt
 * @param[in]   msg_digest           - Hash(message), message is to be verified, here Hash is
 msg_hash_alg.
 ...
 * @note
  @verbatim
      -# 1.it is recommended that msg_hash_alg and mgf_hash_alg are the same.
      -# 2.salt_bytes should be in [0, em_bytes-digest_bytes-2], em_bytes is (em_bits+7)/8, em_bits
 is (n_bits-1). it is recommended to use default value, digest length of hash algorithm msg_hash_alg
 or mgf_hash_alg. if salt_bytes is not known, please set it to -1
  @endverbatim
 */
    unsigned int rsa_ssa_pss_verify_by_msg_digest(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg,
                                                  int32_t salt_bytes, unsigned char* msg_digest,
                                                  unsigned char* e, unsigned int e_bits,
                                                  unsigned char* n, unsigned int n_bits,
                                                  unsigned char* signature);
 ...
 * @note
 ...
 or mgf_hash_alg. if salt_bytes is not known, please set it to -1
  @endverbatim
 */
    unsigned int rsa_ssa_pss_verify(HASH_ALG msg_hash_alg, HASH_ALG mgf_hash_alg,
                                    int32_t salt_bytes, unsigned char* msg, unsigned int msg_bytes,
                                    unsigned char* e, unsigned int e_bits, unsigned char* n,
                                    unsigned int n_bits, unsigned char* signature);
#endif
