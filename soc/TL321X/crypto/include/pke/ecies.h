...
#define ECIES_MAC_KEY_ANSI_X963_MIN_BYTE_LEN                                                       \
    (10) // 10 for ANSI-9.63-2001, and 14 for ANSI-9.63-2011-r2017
 ...
 * @note
  ...
  @endverbatim
 */
    void e_hmac_init(E_HMAC_CTX* mac_ctx, unsigned char* key, unsigned int key_bytes,
                     unsigned char* appendix, unsigned int appendix_bytes, HASH_ALG hash_alg);
    ...
    struct ECIES_STANDARD
    {
        ...
        unsigned int (*get_point_len_from_ciphertext)(eccp_curve_t* curve, unsigned char* cipher,
                                                      unsigned int  cipher_bytes,
                                                      unsigned int* point_bytes);
        unsigned int (*point_decompress)(eccp_curve_t* curve, unsigned char* encode,
                                         unsigned int* x, unsigned int* y);
        unsigned int (*point_compress)(eccp_curve_t* curve, unsigned char* x, unsigned char* y,
                                       EC_POINT_FORM point_form, unsigned char* result,
 ...
 * @param[in]   sender_tmp_pri_key    - sender's ephemeral private key, big-endian.
                                        if you do not have this, please set this parameter to be
 NULL, it will be generated inside.
 ...
 * @note
   @verbatim
      -# 1. if no shared_info1 needs to be provided, set the shared_info1 to NULL and the
 shared_info1_len to 0
      -# 2. if no shared_info2 needs to be provided, set the shared_info2 to NULL and the
 shared_info2_len to 0
      -# 3. if you do not have local_tmp_pri_key, please set the parameter to be NULL, it will be
 generated inside. this is recommended.
 ...
 */
    unsigned int
    ansi_x963_2001_ecies_encrypt(eccp_curve_t* curve, unsigned char* msg, unsigned int msg_bytes,
                                 unsigned char* shared_info1, unsigned int shared_info1_bytes,
                                 unsigned char* shared_info2, unsigned int shared_info2_bytes,
                                 unsigned char* sender_tmp_pri_key, unsigned char* receiver_pub_key,
                                 EC_POINT_FORM point_form, HASH_ALG kdf_hash_alg,
                                 HASH_ALG mac_hash_alg, unsigned int mac_k_bytes,
                                 unsigned char* cipher, unsigned int* cipher_bytes);
 ...
 * @note
 ...
 shared_info2_len to 0
   ...
   @endverbatim
 */
    unsigned int ansi_x963_2001_ecies_decrypt(
        eccp_curve_t* curve, unsigned char* cipher, unsigned int cipher_bytes,
        unsigned char* receiver_pri_key, unsigned char* shared_info1,
        unsigned int shared_info1_bytes, unsigned char* shared_info2,
        unsigned int shared_info2_bytes, HASH_ALG kdf_hash_alg, HASH_ALG mac_hash_alg,
        unsigned int mac_k_bytes, unsigned char* msg, unsigned int* msg_bytes);
