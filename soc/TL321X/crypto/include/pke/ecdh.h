...
#define ECDH_INTEGER_TOO_BIG (PKE_SUCCESS + 0x63U)
    ...
    unsigned int ecdh_compute_key(eccp_curve_t* curve, unsigned char* local_prikey,
...
#define ECDH_ERROR_S   (0xCBC192A3U)
    ...
    unsigned int ecdh_compute_key_s(eccp_curve_t* curve, unsigned char* local_prikey,
                                    unsigned char* peer_pubkey, unsigned char* key,
                                    unsigned int keyByteLen, KDF_FUNC kdf);
