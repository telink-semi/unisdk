 ...
 * @param[in]   p_h_buf      - a unsigned int buffer holds pre-calculated mont parameters H(R^2 mod
 p).
 * @param[in]   p_n0_buf     - a unsigned int buffer holds pre-calculated mont parameters
 n0'(-modoulus^(-1) mod 2^w).
 * @param[in]   q_buf        - a unsigned int buffer holds q, a prime factor of p-1, aka order of
 g..
 * @param[in]   q_bits       - bit length of q.
 * @param[in]   g_buf        - a unsigned int buffer holds g, a generator of the q-order subgroup of
 GF(p)*.
 ...
 * @note
  @verbatim
      ...
      -# 2.p_n0_buf holds n0'(-modoulus^(-1) mod 2^w), to accelerate DH calculation, it occupies
          (w+7)/8 bytes, here w is actually 32. if you do not have this, please set p_n0_buf to
 NULL.
  @endverbatim
 */
    unsigned int dh_param_pointer_init(DH_PARA* dh_para, unsigned int* p_buf, unsigned int p_bits,
                                       unsigned int* p_h_buf, unsigned int* p_n0_buf,
                                       unsigned int* q_buf, unsigned int q_bits,
                                       unsigned int* g_buf, unsigned int g_bits);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int dh_param_value_init(DH_PARA* dh_para, unsigned char* p, unsigned char* p_h,
                                     unsigned char* p_n0, unsigned char* q, unsigned char* g);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int dh_check_public_key(DH_PARA* dh_para, unsigned int* p_minus_2,
                                     unsigned int* pubkey);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int dh_generate_pubkey_from_prikey(DH_PARA* dh_para, unsigned char* prikey,
 ...
 * @note
   ...
   @endverbatim
 */
    unsigned int dh_compute_key(DH_PARA* dh_para, unsigned char* local_prikey,
                                unsigned char* peer_pubkey, unsigned char* key);
...
#define DH_ERROR_S   (0xFDC28CB1U)
    ...
    unsigned int dh_compute_key_s(DH_PARA* dh_para, unsigned char* local_prikey,
                                  unsigned char* peer_pubkey, unsigned char* key);
