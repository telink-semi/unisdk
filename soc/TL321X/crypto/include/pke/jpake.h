...
#define JPAKE_MAX_WORD_LEN (2048 / 32)
    ...
    {
        unsigned int*
            ga; // A = g^a mod p, less than p, so this occupies the same words as J-PAKE parameter p
        unsigned int*
            gv; // V = g^v mod p, less than p, so this occupies the same words as J-PAKE parameter p
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int jpake_round1_generate_xa_xb_and_local_two_zkps(
        JPAKE_PARA* jpake_para, JPAKE_USER_INFO* local_zkp_owner_info, unsigned int* xa,
        unsigned int* xb, JPAKE_ZKP* xa_zkp, JPAKE_ZKP* xb_zkp);

    /**
     * @brief       J-PAKE round 1, verify ZKP(xc), ZKP(xd) of peer user
     * @param[in]   jpake_para               - JPAKE_PARA struct pointer
     * @param[in]   peer_zkp_owner_info      - ID and other info of peer user
     * @param[in]   xc_zkp                   - ZKP(xc) of peer user(ZKP(x1) of Alice, or ZKP(x3) of
     * Bob)
     * @param[in]   xd_zkp                   - ZKP(xd) of peer user(ZKP(x2) of Alice, or ZKP(x4) of
     * Bob)
     * @return      0:success     other:error
     */
    unsigned int jpake_round1_verify_peer_two_zkps(JPAKE_PARA*      jpake_para,
                                                   JPAKE_USER_INFO* peer_zkp_owner_info,
     ...
     * @param[in]   xb                       - private xb of local user(x2 of Alice, or x4 of Bob),
     * less than q
     * @param[in]   gxa                      - public g^xa of local user(g^x1 of Alice, or g^x3 of
     * Bob), less than p
     * @param[in]   gxc                      - public g^xc of peer user(g^x3 of Bob, or g^x1 of
     * Alice), less than p
     * @param[in]   gxd                      - public g^xd of peer user(g^x4 of Bob, or g^x2 of
     * Alice), less than p
     * @param[in]   s                        - shared secret of two sides
     * @param[in]   sWordLen                 - word length of s
     * @param[out]  local_zkp_xb_s           - ZKP(xb*s) of local user(g is (g^xa)(g^xc)(g^xd) mod
     * p)
     * @return      0:success     other:error
     */
    unsigned int jpake_round2_generate_local_zkp(JPAKE_PARA*       jpake_para,
                                                 JPAKE_USER_INFO*  local_zkp_owner_info,
                                                 JPAKE_ROUND2_CTX* round2_ctx, unsigned int* xb,
                                                 unsigned int* gxa, unsigned int* gxc,
                                                 unsigned int* gxd, unsigned int* s,
                                                 unsigned int sWordLen, JPAKE_ZKP* local_zkp_xb_s);

    /**
 * @brief       J-PAKE round 2, verify peer ZKP(xb*s) (ZKP(x2*s) of Alice, or ZKP(x4*s) of Bob), and
 compute key
 * @param[in]   jpake_para               - JPAKE_PARA struct pointer
 * @param[in]   peer_zkp_owner_info      - ID and other info of peer user
 * @param[out]  round2_ctx               - JPAKE_ROUND2_CTX struct pointer of local user
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int jpake_round2_verify_peer_zkp_and_compute_key(
        JPAKE_PARA* jpake_para, JPAKE_USER_INFO* peer_zkp_owner_info, JPAKE_ROUND2_CTX* round2_ctx,
        unsigned int* xb, unsigned int* gxb, unsigned int* gxd, JPAKE_ZKP* peer_zkp_xb_s,
        unsigned int* key);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int jpake_round3_hash_hash_key(JPAKE_PARA* jpake_para, unsigned char* h_key,
                                            unsigned char* h_h_key);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int jpake_round3_hash_key(JPAKE_PARA* jpake_para, unsigned int* key,
                                       unsigned char* h_key);
