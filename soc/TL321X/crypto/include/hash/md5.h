 ...
 * @param[in]   remainder_bytes          - byte length of the remainder message, must be in [0,
 BLOCK_BYTE_LEN-1],
 *                                         here BLOCK_BYTE_LEN is block byte length of md5, it
 is 64.
 * @param[out]  digest                   - md5 digest, 16 bytes.
 * @return      0:success     other:error
 * @note
  ...
  @endverbatim
 */
    unsigned int md5_dma_final(MD5_DMA_CTX* ctx, unsigned int* remainder_msg,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int md5_dma(unsigned int* msg, unsigned int msg_bytes, unsigned int* digest,
     ...
     * block length.
     */
    unsigned int md5_dma_node_steps(HASH_DMA_NODE* node, unsigned int node_num,
