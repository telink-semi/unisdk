...
extern "C"
{
     ...
     */
    unsigned int hmac_sha512_256_init(HMAC_SHA512_256_CTX* ctx, unsigned char* key,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_sha512_256_update(HMAC_SHA512_256_CTX* ctx, unsigned char* msg,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_sha512_256(unsigned char* key, unsigned short sp_key_idx,
 ...
 *        address and byte length.
  @endverbatim
 */
    unsigned int hmac_sha512_256_node_steps(unsigned char* key, unsigned short sp_key_idx,
     ...
     * @return      0:success     other:error
     */
    unsigned int hmac_sha512_256_dma_init(HMAC_SHA512_256_DMA_CTX* ctx, unsigned char* key,
                                          unsigned short sp_key_idx, unsigned int key_bytes,
 ...
 * @param[in]   ctx                 - HMAC_SHA512_256_CTX context pointer.
 * @param[in]   msg                 - key.
 * @param[in]   msg_bytes           - byte length of the input message, must be a multiple of block
 byte length.
 * @return      0:success     other:error
 * @note
  @verbatim
  ...
  @endverbatim
 */
    unsigned int hmac_sha512_256_dma_update_blocks(HMAC_SHA512_256_DMA_CTX* ctx, unsigned int* msg,
                                                   unsigned int msg_bytes);
    /**
 ...
 * @param[in]   remainder_bytes     - byte length of the last message, must be in [0,
 BLOCK_BYTE_LEN-1],
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_sha512_256_dma_final(HMAC_SHA512_256_DMA_CTX* ctx,
     ...
     * @return      0:success     other:error
     */
    unsigned int hmac_sha512_256_dma(unsigned char* key, unsigned short sp_key_idx,
...
-# 2. if the whole message consists of some segments, every segment is a node, a node includes
    address and byte length.
-# 3. for every node or segment except the last, its message length must be a multiple of block
length.
@endverbatim
*/
#ifdef SUPPORT_HASH_DMA_NODE
    unsigned int hmac_sha512_256_dma_node_steps(unsigned char* key, unsigned short sp_key_idx,
