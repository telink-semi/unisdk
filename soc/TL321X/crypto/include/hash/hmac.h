 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_init(HMAC_CTX* ctx, HASH_ALG hash_alg, unsigned char* key,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac(HASH_ALG hash_alg, unsigned char* key, unsigned short sp_key_idx,
     ...
     *     -# 3. if the whole message consists of some segments, every segment is a node, a node
     * includes address and byte length.
     */
    unsigned int hmac_node_steps(HASH_ALG hash_alg, unsigned char* key, unsigned short sp_key_idx,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_dma_init(HMAC_DMA_CTX* ctx, HASH_ALG hash_alg, unsigned char* key,
                               unsigned short sp_key_idx, unsigned int key_bytes,
 ...
 * @param[in]   ctx            - HMAC_DMA_CTX context pointer.
 * @param[in]   msg            - message.
 * @param[in]   msg_bytes      - word length of the input message, must be a multiple of block word
 length of HASH.
 * @return      0:success     other:error
 * @note
  @verbatim
  ...
  @endverbatim
 */
    unsigned int hmac_dma_update_blocks(HMAC_DMA_CTX* ctx, unsigned int* msg,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_dma_final(HMAC_DMA_CTX* ctx, unsigned int* remainder_msg,
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hmac_dma(HASH_ALG hash_alg, unsigned char* key, unsigned short sp_key_idx,
 ...
 * @note
  @verbatim
      ...
      -# 4. if the whole message consists of some segments, every segment is a node, a node includes
            address and byte length.
      -# 5. for every node or segment except the last, its message length must be a multiple of
 block length.
  @endverbatim
 */
    unsigned int hmac_dma_node_steps(HASH_ALG hash_alg, unsigned char* key,
                                     unsigned short sp_key_idx, unsigned int key_bytes,
                                     HASH_DMA_NODE* node, unsigned int node_num, unsigned int* mac,
                                     HASH_CALLBACK callback);
