...
#include "hash_basic.h"
    ...
    typedef struct
    {
        ...
        unsigned int iterator[HASH_ITERATOR_MAX_WORD_LEN]; // keep current hash iterator value for
                                                           // multiple thread
#endif
 ...
 */
     ...
     */
    unsigned int hash_total_byte_len_add_uint32(unsigned int* a, unsigned int a_words,
                                                unsigned int b);
 ...
 * @note
  @verbatim
      ...
      -# 2.  at present this function does not support the case that byte length of msg is a
 multiple of hash block length. actually msg_bytes here must be less than the hash block byte
 length, namely, this function is just for the remainder message, and will do padding, finally get
        digest.
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hash_init_with_iv_and_updated_length(HASH_CTX* ctx, HASH_ALG hash_alg,
                                                      unsigned int* iv, unsigned int byte_length_h,
                                                      unsigned int byte_length_l);
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hash(HASH_ALG hash_alg, unsigned char* msg, unsigned int msg_bytes,
 ...
 * @return      0:success     other:error
   @verbatim
      -# 1.  please make sure the digest buffer is sufficient.
      -# 2.  if the whole message consists of some segments, every segment is a node, a node
 includes address and byte length.
   @endverbatim
 */
    unsigned int hash_node_steps(HASH_ALG hash_alg, HASH_NODE* node, unsigned int node_num,
                                 unsigned char* digest);
#endif
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hash_dma_init_with_iv_and_updated_length(HASH_DMA_CTX* ctx, HASH_ALG hash_alg,
                                                          unsigned int* iv,
                                                          unsigned int  byte_length_h,
                                                          unsigned int  byte_length_l,
                                                          HASH_CALLBACK callback);
 ...
 * @param[in]   msg_bytes           - word length of the input message, must be a multiple of hash
 block word length.
 * @return      0:success     other:error
 * @note
  @verbatim
  ...
  @endverbatim
 */
    unsigned int hash_dma_update_blocks(HASH_DMA_CTX* ctx, unsigned int* msg,
                                        unsigned int msg_bytes);
    /**
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hash_dma_final(HASH_DMA_CTX* ctx, unsigned int* remainder_msg,
                                unsigned int remainder_bytes, unsigned int* digest);
    /**
 * @brief       dma hash digest calculate
 * @param[in]   hash_alg       - specific hash algorithm.
 * @param[in]   msg            - message.
 ...
 * @note
  ...
  @endverbatim
 */
    unsigned int hash_dma(HASH_ALG hash_alg, unsigned int* msg, unsigned int msg_bytes,
                          unsigned int* digest, HASH_CALLBACK callback);
#ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    /**
 * @brief       dma hash update some message blocks
 * @param[in]   ctx                - HASH_DMA_CTX context pointer.
 * @param[in]   msg                - message blocks.
 * @param[in]   msg_words          - word length of the input message, must be a multiple of hash
 block word length.
 * @return      0:success     other:error
 * @note
  @verbatim
...
#ifdef SUPPORT_HASH_DMA_NODE
    unsigned int hash_dma_node_steps(HASH_ALG hash_alg, HASH_DMA_NODE* node, unsigned int node_num,
                                     unsigned int digest_h, unsigned int digest_l,
                                     HASH_CALLBACK callback);
#endif
#else
 ...
 * @note
  @verbatim
      ...
      -# 2.  if the whole message consists of some segments, every segment is a node, a node
 includes address and byte length.
      -# 3.  for every node or segment except the last, its message length must be a multiple of
 block length.
  @endverbatim
 */
    unsigned int hash_dma_node_steps(HASH_ALG hash_alg, HASH_DMA_NODE* node, unsigned int node_num,
