...
#endif
#if (COMPATIBLE_WITH_TL321X_AND_TL323X ==                                                          \
     0) // warning: hardware does not support 3DES directly, here the 4 MACRO should be defined or
        // not defined simultaneously
...
#define SKE_LP_LAST_DATA_OFFSET                  (16)
    ...
    typedef struct
    {
        ...
        unsigned int ske_c_len_l; /* Offset: 0x068 (R/W) CCM/GCM/XTS mode plaintext/ciphertext
                                     length low Register */
        unsigned int ske_c_len_h; /* Offset: 0x06C (R/W) CCM/GCM/XTS mode plaintext/ciphertext
                                     length high Register */
    ...
    */
     ...
     * @note
      @verbatim
          -# 1.must be called after ske_lp_set_crypto() and ske_lp_set_alg(), and the key is set
     already.
      @endverbatim
     */
...
/**
 * @brief       ske_lp AES128_ECB encrypting or decrypting one block(16bytes) (CPU style, one-off
 style).
 ...
 */
     ...
     * @note
      @verbatim
          ...
          -# 2.it could be without output, namely, out can be NULL, out_words can be 0(for input
     AAD, or CBC_MAC/CMAC mode).
      @endverbatim
     */
    unsigned int ske_lp_dma_operate(SKE_CTX* ctx, unsigned int* in, unsigned int* out,
                                    unsigned int in_words, unsigned int out_words,
     ...
     * @note
      ...
      @endverbatim
     */
    unsigned int ske_lp_update_blocks_no_output(SKE_CTX* ctx, unsigned char* in,
                                                unsigned int bytes);
     ...
     * @note
      ...
      @endverbatim
     */
    unsigned int ske_lp_update_blocks_internal(SKE_CTX* ctx, unsigned char* in, unsigned char* out,
                                               unsigned int bytes);
    ...
    unsigned int tdes_ecb_update_one_block(unsigned int is_EEE, unsigned int key[6],
                                           SKE_CRYPTO crypto, unsigned int in[2],
                                           unsigned int out[2]);
#endif

    /*
     * @brief       ske_lp AES128_ECB encrypting or decrypting one block(16bytes) (CPU style,
     one-off style)
     * @param[in]   key                  - key
     ...
     * @note
      ...
      @endverbatim
     */
    unsigned int ske_lp_aes128_ecb_one_block(SKE_CRYPTO crypto, unsigned int* key, unsigned int* in,
                                             unsigned int* out);
