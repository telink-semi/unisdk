 ...
 * @note
  @verbatim
      ...
      -# 4.if key is from user input, please make sure the argument key is not NULL(now sp_key_idx
 is useless), otherwise, key is from secure port, and (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX].
  @endverbatim
 */
    void ske_lp_set_key(SKE_ALG alg, const unsigned char* key, unsigned short key_bytes,
                        unsigned short key_idx);

    unsigned int ske_lp_update_blocks_internal_(SKE_CTX* ctx, unsigned char* in, unsigned char* out,
                                                unsigned int bytes);

    unsigned int ske_lp_update_blocks_no_output_(SKE_CTX* ctx, unsigned char* in,
                                                 unsigned int bytes);
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
  @endverbatim
 */
    unsigned int ske_lp_init_internal(SKE_CTX* ctx, SKE_ALG alg, SKE_MODE mode, SKE_CRYPTO crypto,
                                      const unsigned char* key, unsigned short sp_key_idx,
                                      unsigned char* iv, unsigned int dma_en);
 ...
 * @note
  @verbatim
      ...
      -# 2.this function is designed for ECB/CBC/CFB/OFB/CTR/XTS modes, and input/output unit must
 be a block.
      -# 3.if key is from user input, please make sure the argument key is not NULL(now sp_key_idx
 is useless), otherwise, key is from secure port, and (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX].
  @endverbatim
 */
    unsigned int ske_lp_init(SKE_ALG alg, SKE_MODE mode, SKE_CRYPTO crypto, unsigned char* key,
 ...
 * @note
  @verbatim
      -# 1.this function is designed for ECB/CBC/CFB/OFB/CTR modes, and input/output unit must be a
 block.
 ...
 * @note
  @verbatim
      -# 1.if mode is ECB, then there is no iv, in this case iv could be NULL.
      -# 2.this function is designed for ECB/CBC/CFB/OFB/CTR/XTS modes, and input/output unit is a
 block.
      -# 3.if key is from user input, please make sure the argument key is not NULL(now sp_key_idx
 is useless), otherwise, key is from secure port, and (sp_key_idx & 0x7FFF) must be in
 [1,SKE_MAX_KEY_IDX].
  ...
  @endverbatim
 */
    unsigned int ske_lp_crypto(SKE_ALG alg, SKE_MODE mode, SKE_CRYPTO crypto, unsigned char* key,
                               unsigned short sp_key_idx, unsigned char* iv, unsigned char* in,
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
  @endverbatim
 */
    unsigned int ske_lp_dma_init(SKE_ALG alg, SKE_MODE mode, SKE_CRYPTO crypto, unsigned char* key,
                                 unsigned short sp_key_idx, unsigned char* iv);
 ...
 * @param[in]   in               - plaintext or ciphertext.
 * @param[out]  out              - ciphertext or plaintext.
 * @param[in]   words            - word length of input or output, must be multiples of block
 length.
 ...
 * @note
  @verbatim
      -# 1.this function is designed for ECB/CBC/CFB/OFB/CTR/XTS modes, and input/output unit is a
 block.
      -# 2.to save memory, in and out could be the same buffer, in this case, the output will
        cover the input.
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_update_blocks(unsigned int* in, unsigned int* out, unsigned int words,
 ...
 * @note
 ...
 [1,SKE_MAX_KEY_IDX].
  ...
  @endverbatim
 */
    unsigned int ske_lp_dma_crypto(SKE_ALG alg, SKE_MODE mode, SKE_CRYPTO crypto,
                                   unsigned char* key, unsigned short sp_key_idx, unsigned char* iv,
                                   unsigned int* in, unsigned int* out, unsigned int words,
                                   SKE_CALLBACK callback);
#endif
