...
typedef enum
{
    ...
    FLD_BB_TIMER_IRQ_CLR_CAL      = TLK_BIT(1), //calibration latch result update irq (clear 32k cal irq 09[1] )
...
enum
{
    FLD_BB_TIMER_SYS_TIMER_ALIGN_COMP = TLK_BIT_RNG(0, 4), //Compensation of system timer and system timer bb counters. default:0x10
...
enum
{
    FLD_BB_TIMER_SYS_TIMER_ALIGN_CNT    = TLK_BIT_RNG(0, 1), //system timer and system timer bb clock count alignment compensation. default:0x1
    FLD_BB_TIMER_SYS_32KTIMER_ALIGN_CNT = TLK_BIT_RNG(2, 3), //system timer and system timer bb 32k clock count alignment compensation. default:0x1
};
...
enum
{
    FLD_RF_R_STIMER_REVERT_EN = TLK_BIT(0),  //Set to 1, stimer controls the function and ststamp through the system timer;
                                         //Otherwise system timer bb is used
    FLD_RF_R_ST_HADM_REVERT_EN = TLK_BIT(1), //Set to 1, hadm ststamp through the system timer;Otherwise system timer bb is used
};
...
typedef enum
{
    FLD_RF_IRQ_RX                = TLK_BIT(0),  /*Auto/Manual: an interrupt is generated after each packet is received.*/
    FLD_RF_IRQ_TX                = TLK_BIT(1),  /*Auto/Manual: an interrupt is generated after each packet is sent*/
    FLD_RF_IRQ_RX_TIMEOUT        = TLK_BIT(2),  /*Auto: If the packet is still not received within the time set from trigger to timeout,
    ...
    FLD_RF_IRQ_RX_FIFO_FULL      = TLK_BIT(3),  /*Auto/Manual:An interrupt is generated when the data size of the total received packets is exceeded to the allocated storage range of Rx SRAM
                                                         TL321X chip unavailable, it will not work after configuration.(add by chenxi.wang, confirmed by jingyong 202402029)*/
    FLD_RF_IRQ_RX_CRC_2          = TLK_BIT(4),  /*Auto:BTX,BRX,PTX,PRX will generate interrupt if CRC error is detected twice in a row during packet receiving (continuous packet receiving).*/
    FLD_RF_IRQ_CMD_DONE          = TLK_BIT(5),  /*Auto:After completing a normal packet receiving or sending operation,
                                                  the state machine returns to the IDLE state normally and generates an interrupt.*/
    FLD_RF_IRQ_FSM_TIMEOUT       = TLK_BIT(6),  /*Auto:It contains the state machine used for switching from receive to send, specifying the time of the entire state machine, and determining
                                                  whether the specified time is exceeded in the TX_WAIT state*/
    FLD_RF_IRQ_TX_RETRYCNT       = TLK_BIT(7),  /*Auto:Generate an interrupt when the number of ptx retry exceeds the set r_max_retry_cnt.*/
    FLD_RF_IRQ_TX_DS             = TLK_BIT(8),  /*Auto:Length of payload (sent by PTX, PRX) ! = 0, generate tx_ds interrupt.*/
    FLD_RF_IRQ_RX_DR             = TLK_BIT(9),  /*Auto:PRX, PTX, SRX received packets detect the packet payload length ! = 0, generate rx_dr interrupt*/
    FLD_RF_IRQ_FIRST_TIMEOUT     = TLK_BIT(10), /*Auto:BRX, PRX, SRX, SRT first RX timeout, when the first packet receive timeout it will generate an interrupt*/
    FLD_RF_IRQ_INVALID_PID       = TLK_BIT(11), /*Auto:PTX or PRX receives invalid pid and generates interrupt*/
    FLD_RF_IRQ_STX_TIMEOUT       = TLK_BIT(12), /*Auto:The STX state does not wait for tx_done in the specified time, thus timeout, generating an interrupt.*/
    FLD_RF_IRQ_WIFI_DENY         = TLK_BIT(13), /*Auto:After the Bluetooth chip receives the wifi_deny signal from the WiFi chip when it is connected to the WiFi chip,
                                                   the Bluetooth chip generates an interrupt*/
    FLD_RF_IRQ_SUPP_OF           = TLK_BIT(14), /*Auto/Manual:The interrupt is mainly used for AOA and AOD. If iq sampling frequency is too high the hardware FIFO will overflow error,
    ...
    FLD_RF_IRQ_TLK_RXCRYPT_ERR   = TLK_BIT(17), /*Auto/Manual:Indicates real time decryption error, happens when CCM decryption is too slow compared to receiving bitstream by Packet Controller. */
    FLD_RF_IRQ_TLK_TXCRYPT_ERR   = TLK_BIT(18), /*Auto/Manual:Indicates Real Time encryption error, happens when CCM encryption is too slow compared to transmitting requests by Packet Controller. */
    FLD_RF_IRQ_HIT_SYNC          = TLK_BIT(19), /*Auto/Manual:An interrupt is generated when sync up of one packet is detected in Rx mode*/
    FLD_RF_IRQ_HEADER_DONE       = TLK_BIT(20), /*Auto/Manual:Interrupt is generated when the Header Field of one packet has been received completely.*/
    FLD_RF_IRQ_PKT_UNMATCH       = TLK_BIT(21), /*Auto/Manual:An interrupt is generated when the certain bits of the received packet are not matched to the predefined ones within the packet filter registers in Rx mode.*/
    FLD_RF_IRQ_PKT_MATCH         = TLK_BIT(22), /*Auto/Manual:An interrupt is generated when the certain bits of the received packet are matched to the predefined ones within the packet filter registers in Rx mode. */
    FLD_RF_IRQ_RX_ZB_FREQ_HOP    = TLK_BIT(23), /*Auto/Manual:When Zigbee performs dual-channel detection, it switches channels at preset intervals, and the interrupt generated at this time*/
    FLD_RF_IRQ_RX_FREQ_HOP_FIXED = TLK_BIT(24), /*Auto/Manual:When Zigbee performs dual-channel detection, the interrupt is generated when it stays in one of the channels to successfully receive packets and fixes the interrupt when it continues to receive over-the-air packets in this channel.*/
    FLD_RF_IRQ_TRX_FCAL_DONE     = TLK_BIT(25), /*After RF is turned on, the interrupt generated at the moment of fcal calibration is completed at tx or rx (including switching channels during dual-channel detection by zigbee).*/
    FLD_RF_IRQ_RADIO_DONE        = TLK_BIT(26), /*TODO:This function interface is not available at this time, and will be updated in subsequent releases.(unverified)*/
