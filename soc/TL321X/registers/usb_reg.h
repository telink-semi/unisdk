...
typedef enum
{
    FLD_USB_AUTO_HALT_CLR = TLK_BIT(0), /* enable auto clear halt feature(STALL) after either a
                                           SetConfiguration() or SetInterface() */
    FLD_USB_AUTO_HALT_STALL =
        TLK_BIT(1), /* enable auto decoding the halt feature(STALL) by hardware */
