...
#define write_reg8(addr, v)                                                                        \
    (*(volatile unsigned char*) (REG_RW_BASE_ADDR | (addr)) = (unsigned char) (v))
#define write_reg16(addr, v)                                                                       \
    (*(volatile unsigned short*) (REG_RW_BASE_ADDR | (addr)) = (unsigned short) (v))
#define write_reg32(addr, v)                                                                       \
    (*(volatile unsigned long*) (REG_RW_BASE_ADDR | (addr)) = (unsigned long) (v))
 ...
 * ILM area:0xc0000000~0xc0040000 TLK_BIT(31~30) is 3,TLK_BIT(21) is 0, TLK_BIT(20~18) do not care
 * TLK_BIT(17~0) 256K is address offset 256K is address offset DLM area:0xc0200000~0xc0240000
 * TLK_BIT(31~30) is 3,TLK_BIT(21) is 1, TLK_BIT(20~18) do not care  TLK_BIT(17~0) 256K is address
 * offset 256K is address offset TLK_BIT(19) is used to distinguish from IRAM to DRAM, TLK_BIT(21)
 * is used to distinguish from ILM to DLM. so we can write it as follow #define
 * convert_ram_addr_cpu2bus  (((((addr))&0x80000)? ((addr)| 0xc0200000) : ((addr)|0xc0000000)))
 * TLK_BIT(20~17) are invalid address line ,IRAM address is less than 0x80000, (address-0x80000)must
 * borrow from TLK_BIT(21) #define convert(addr) ((addr)-0x80000+0xc0200000) to simplify #define
 * convert(addr) ((addr)+0xc0180000)
 * */
// #define convert_ram_addr_cpu2bus(addr)   ((unsigned int)(addr)+0xc0180000)
// #else  //no optimize
// #define  convert_ram_addr_cpu2bus  (((((unsigned int)(addr)) >=0x80000)?(((unsigned
// int)(addr))-0x80000+0xc0200000) : (((unsigned int)(addr)) + 0xc0000000))) #endif
//  go further, if the source and destination  address is not in the sram(IRAM/DRAM)  interval, no
//  address translation
#define convert_ram_addr_cpu2bus(addr)                                                             \
    (unsigned int) (addr) < 0xc0000 ? ((unsigned int) (addr) + 0xc0180000) : (unsigned int) (addr)
// When using convert_ram_addr_cpu2bus(addr) formula to write to the register, TLK_BIT(20~18) may
// not be zero,  need to clear the corresponding bit to zero
#define convert_ram_addr_bus2cpu(addr)                                                             \
    (((((unsigned int) (addr)) >= 0xc0200000)                                                      \
          ? (((unsigned int) (addr)) + 0x80000 - 0xc0200000)                                       \
          : (((unsigned int) (addr) & (~0x1c0000)) - 0xc0000000)))
