...
enum
{
    FLD_I2C_WRITE_READ_BIT = TLK_BIT(0),
    FLD_I2C_ID             = TLK_BIT_RNG(1, 7),
};
 ...
 * BIT_RNG[3:5] Indicate what state the i2c is in when it acts as master, BIT_RNG[3:5] default value
 * is 0x06 it means master's state is IDLE. BIT_RNG[6:7] Indicate what state the i2c is in when it
 * acts as slave.
 */
...
enum
{
    FLD_I2C_MST_BUSY = TLK_BIT(0),
    FLD_I2C_SCS_N    = TLK_BIT(1),
    FLD_I2C_ACK_IN   = TLK_BIT(2),
    FLD_I2C_MST_P    = TLK_BIT_RNG(3, 5),
    FLD_I2C_SS       = TLK_BIT_RNG(6, 7),
};

/**
 * This shows the interrupt mask register of i2c
 * BIT[0] mask the slave parsing master cmd interrupt,when received the master read or write cmd,
 * will generate interrupt. BIT[1] when slave is wrong, reply nack to master,master detect to the
 * nack, will generate interrupt. BIT[2] rx interrupt enable.RX is related to rx_irq_trig_lev
 * function. fifo_data_cnt> = rx_irq_trig_lev generates an interrupt. BIT[3] tx interrupt
 * enable.Related to tx_irq_trig_lev function. fifo_data_cnt <= tx_irq_trig_lev, generate interrupt.
...
enum
{
    FLD_I2C_SLAVE_WR_IRQ   = TLK_BIT(0),
    FLD_I2C_MASTER_NAK_IRQ = TLK_BIT(1),
    FLD_I2C_RX_BUF_IRQ     = TLK_BIT(2),
    FLD_I2C_TX_BUF_IRQ     = TLK_BIT(3),
    FLD_I2C_RX_DONE_IRQ    = TLK_BIT(4),
    FLD_I2C_TX_DONE_IRQ    = TLK_BIT(5),
    FLD_I2C_RX_END_IRQ     = TLK_BIT(6),
    FLD_I2C_TX_END_IRQ     = TLK_BIT(7),
};
 ...
 * BIT[6] enable if BIT[6]=1,Its role is to convert the SDA from output to input, so that the master
 * can accept the data sent by the slave when it reads. BIT[7] enable ACK in read command.When the
 * master is reading, it needs to return to ack or nak. If it is in manual mode,when the master is
 * reading, you need to configure this bit to determine whether it will ack. BIT[7] = 0,return ack.
 * BIT[7] = 1,return nak.
 */
...
enum
{
    FLD_I2C_LS_ID    = TLK_BIT(0),
    FLD_I2C_LS_ADDR  = TLK_BIT(1),
    FLD_I2C_LS_DATAW = TLK_BIT(2),
    FLD_I2C_LS_DATAR = TLK_BIT(3),
    FLD_I2C_LS_START = TLK_BIT(4),
    FLD_I2C_LS_STOP  = TLK_BIT(5),
    FLD_I2C_LS_ID_R  = TLK_BIT(6),
    FLD_I2C_LS_ACK   = TLK_BIT(7),
};

/**
 * This is the register that configures the i2c trigger interrupt
 * BIT_RNG[0,3] to configure the interrupt trigger level of rx_status, for example
 * BIT_RNG[0:3]=0x04,when rx 4bytes,will trigger interrupt. BIT_RNG[4,7] to configure the interrupt
 * trigger level of tx_status, for example BIT_RNG[0:3]=0x04,when tx 4bytes,will trigger interrupt.
 */
#define reg_i2c_trig REG_ADDR8(REG_I2C_BASE + 0x05)

enum
{
    FLD_I2C_RX_IRQ_TRIG_LEV = TLK_BIT_RNG(0, 3),
    FLD_I2C_TX_IRQ_TRIG_LEV = TLK_BIT_RNG(4, 7),
};
 ...
 *         1:id or data stretch,when in the id or data phase, the slave tx_fifo is
 * empty,FLD_I2C_SS_SCL_IRQ set 1. BIT[6]: mask the slave stretch interrupt,when tx_fifo is empty or
...
typedef enum
{
    FLD_I2C_MASTER             = TLK_BIT(0),
    FLD_I2C_MASTER_STRETCH_EN  = TLK_BIT(1),
    FLD_MANUAL_TX_STOP_HIT     = TLK_BIT(2),
    FLD_MANUAL_RX_STOP_HIT     = TLK_BIT(3),
    FLD_I2C_MASTER_NAK_STOP_EN = TLK_BIT(4),
    FLD_I2C_TX_STRETCH_SEL     = TLK_BIT(5),
    FLD_I2C_MASK_STRETCH       = TLK_BIT(6),
} i2c_master_e;
...
enum
{
    FLD_I2C_SLAVE_STRETCH_EN = TLK_BIT(0),
    FLD_I2C_RNCK_EN          = TLK_BIT(2),
    FLD_MANUAL_SDA_DELAY     = TLK_BIT(3),
    FLD_NDMA_RXDONE_EN       = TLK_BIT(4),
    FLD_AUTO_RXCLR_EN        = TLK_BIT(5),
};
...
enum
{
    FLD_I2C_BUF3 = TLK_BIT_RNG(0, 7),
};

/**
 * This register is used to configure the number of bytes in the i2c buffer
 * BIT_RNG[0,3]  rx_bufcnt is equivalent to a pointer to fifo, one in data plus one, one out data
 * minus one. BIT_RNG[4,7]  tx_bufcnt is equivalent to a pointer to fifo, one in data plus one, one
 * out data minus one.
 */
...
enum
{
    FLD_I2C_RX_BUFCNT = TLK_BIT_RNG(0, 3),
    FLD_I2C_TX_BUFCNT = TLK_BIT_RNG(4, 7),
};

/**
 * This register used to configure the status of i2c.
 * TLK_BIT(0): W1C,clear the slave manual stretch.
 * TLK_BIT(1): W1C,slave manual stretch clk.
 * TLK_BIT(2): W1C,clear FLD_I2C_ACK_IN.
...
enum
{
    FLD_I2C_MANUAL_STRETCH_CLR    = TLK_BIT(0),
    FLD_I2C_R_MANUAL_STRETCH      = TLK_BIT(1),
    FLD_I2C_MANUAL_CLR_MASTER_ACK = TLK_BIT(2),
    FLD_I2C_RBCNT                 = TLK_BIT_RNG(0, 2),
    FLD_I2C_IRQ_O                 = TLK_BIT(3),
    FLD_I2C_WBCNT                 = TLK_BIT_RNG(4, 6),
    FLD_I2C_RX_DONE               = TLK_BIT(7),
};
...
enum
{
    FLD_I2C_SLAVE_READ             = TLK_BIT(0),
    FLD_I2C_SLAVE_STRETCH_INDICATE = TLK_BIT(1),
    FLD_I2C_SLAVE_TX_EMPTY         = TLK_BIT(2),
    FLD_I2C_SLAVE_RX_FULL          = TLK_BIT(3),
    FLD_I2C_SS_SCL_IRQ             = TLK_BIT(6),

};

// As a master, you need to configure this length for both sending and receiving, and the hardware
// needs to know what the length is.
...
enum
{
    FLD_ID_BUSY    = TLK_BIT(0),
    FLD_ADDR_BUSY  = TLK_BIT(1),
    FLD_DATAW_BUSY = TLK_BIT(2),
    FLD_DATAR_BUSY = TLK_BIT(3),
...
enum
{
    FLD_I2C1_M_WRITE_READ_BIT = TLK_BIT(0),
    FLD_I2C1_M_ID             = TLK_BIT_RNG(1, 7),
};
...
enum
{
    FLD_I2C1_M_MST_BUSY   = TLK_BIT(0),
    FLD_I2C1_M_MST_SCS_N  = TLK_BIT(1),
    FLD_I2C1_M_MAT_ACK_IN = TLK_BIT(2),
};
...
enum
{
    FLD_I2C1_M_MASTER           = TLK_BIT(1), // 1: master, 0: slave
    FLD_I2C1_M_R_CLK_STRETCH_EN = TLK_BIT(3),
};
...
enum
{
    FLD_I2C1_M_LS_ID    = TLK_BIT(0),
    FLD_I2C1_M_LS_ADDR  = TLK_BIT(1),
    FLD_I2C1_M_LS_DATAW = TLK_BIT(2),
    FLD_I2C1_M_LS_DATAR = TLK_BIT(3),
    FLD_I2C1_M_LS_START = TLK_BIT(4),
    FLD_I2C1_M_LS_STOP  = TLK_BIT(5),
    FLD_I2C1_M_LS_ID_R  = TLK_BIT(6),
    FLD_I2C1_M_LS_ACK   = TLK_BIT(7),
};
