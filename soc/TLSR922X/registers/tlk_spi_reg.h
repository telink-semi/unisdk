...
enum
{
    FLD_SPI_DUAL     = TLK_BIT(0),
    FLD_SPI_QUAD     = TLK_BIT(1),
 ...
 * sequence could be: 0x0:write and read at the same time(must enable CmdEn) 0x1:write only 0x2:read
 * only(must enable CmdEn) 0x3:write,read 0x4:read,write 0x5:write,dummy,read
...
enum
{
    ...
    FLD_SPI_DMATX_SOF_CLRTXF_EN = TLK_BIT(5),
    FLD_SPI_DMARX_EOF_CLRRXF_EN = TLK_BIT(6),
 ...
 * BIT[2]  stop xip.
 * BIT[3]  enable xip.
 * BIT[4]  dummy_cnt_add, dummy is always single wire mode, dummy number = {dummy_cnt_add,
...
enum
{
    FLD_SPI_XIP_PAGE_MODE_EN    = TLK_BIT(0),
    FLD_SPI_XIP_TIMEOUT_MODE_EN = TLK_BIT(1),
    FLD_SPI_XIP_STOP            = TLK_BIT(2),
    FLD_SPI_XIP_ENABLE          = TLK_BIT(3),
    FLD_SPI_DUMMY_CNT_ADD       = TLK_BIT(4),
};
 ...
 * phase(Dual/Quad),XIP ONLY BIT[7]  xip read spi command phase enable,XIP ONLY BIT[8:11]   xip read
 * dummy data cnt, dummy is always single wire mode, dummy number = dummy_cnt + 1.XIP only
...
typedef enum
{
    FLD_SPI_XIP_RD_DATA_IO_MODE = TLK_BIT_RNG(0, 1),
 ...
 * cmd format 0: single mode  1: the format of the cmd phase is the same as the data
 * phase(Dual/Quad),XIP ONLY BIT[7]  xip write spi command phase enable,XIP ONLY BIT[12:15]   xip
 * write transfer mode/lcd display transfer mode the transfer sequence could be: 0x0:write and read
 * at the same time(must enable CmdEn) 0x1:write only 0x2:read only(must enable CmdEn)
 *              0x3:write,read
 *              0x4:read,write
 *              0x5:write,dummy,read
 *              0x6:read,dummy,write(must enable CmdEn)
