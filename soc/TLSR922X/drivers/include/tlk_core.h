...
#define TLK_MCAUSE_INT   0x80000000UL
#define TLK_MCAUSE_CAUSE 0x7FFFFFFFUL


#define TLK_TRAP_U_ECALL 8
#define TLK_TRAP_M_ECALL 11


/* Machine CSR */
#define TLK_MSTATUS_MPP_MSK ((1ULL << (11)) | (1ULL << (12)))
/* Hart mode for machine/supervisor/user */
#define TLK_MSTATUS_MPP_USER (0ULL << (11))
#define TLK_MSTATUS_MPP_SV   (1ULL << (11))
#define TLK_MSTATUS_MPP_MC   ((1ULL << (11)) | (1ULL << (12)))

#ifdef STD_GCC
    #define TLK_DISABLE_BTB __asm__("csrci %0,8" ::"i"(mmisc_ctl))
    #define TLK_ENABLE_BTB  __asm__("csrsi %0,8" ::"i"(mmisc_ctl))
#else
    #define TLK_DISABLE_BTB __asm__("csrci mmisc_ctl,8")
    #define TLK_ENABLE_BTB  __asm__("csrsi mmisc_ctl,8")
#endif
...
typedef enum
{
    TLK_MHSP_CTL_OVF_EN   = TLK_BIT(0), /**< Enable bit for the stack overflow protection and recording mechanism. */
    TLK_MHSP_CTL_UDF_EN   = TLK_BIT(1), /**< Enable bit for the stack underflow protection mechanism. */
    TLK_MHSP_CTL_SCHM_SEL = TLK_BIT(2), /**< 0-Stack overflow/underflow,1-Top-of-stack recording. */
    TLK_MHSP_CTL_U_EN     = TLK_BIT(3), /**< Enables the SP protection and recording mechanism in User mode. */
    TLK_MHSP_CTL_S_EN     = TLK_BIT(4), /**< Enables the SP protection and recording mechanism in Supervisor mode. */
    TLK_MHSP_CTL_M_EN     = TLK_BIT(5), /**< Enables the SP protection and recording mechanism in Machine mode. */
} tlk_mhsp_ctl_e;
 ...
 *          - Other cases as described in tlk_core_preempt_pri_e below.
 */
typedef enum
{
    TLK_CORE_PREEMPT_PRI_MODE0 = TLK_FLD_MIE_MSIE | TLK_FLD_MIE_MTIE, /**< MTI and MSI cannot interrupt MEI, MSI and MTI can be nested within each other. */
    TLK_CORE_PREEMPT_PRI_MODE1 = TLK_FLD_MIE_MTIE,                /**< MTI cannot interrupt MEI, MSI and MTI can be nested within each other, MSI and MEI can be nested within each other. */
    TLK_CORE_PREEMPT_PRI_MODE2 = TLK_FLD_MIE_MSIE,                /**< MSI cannot interrupt MEI, MSI and MTI can be nested within each other, MTI and MEI can be nested within each other. */
    TLK_CORE_PREEMPT_PRI_MODE3 = TLK_BIT(1),                      /**< MEI, MSI and MTI can be nested within each other(MIE register bit1 is an invalid bit). */
} tlk_core_preempt_pri_e;

#define tlk_read_csr(reg)         ({ unsigned long __tmp; \
  __asm__ volatile ("csrr %0, %1" : "=r"(__tmp) : "i" (reg)); \
  __tmp; })

#define tlk_write_csr(reg, val)   ({ __asm__ volatile("csrw %0, %1" ::"i"(reg), "rK"(val)); })

#define tlk_swap_csr(reg, val)    ({ unsigned long __tmp; \
  __asm__ volatile ("csrrw %0, %1, %2" : "=r"(__tmp) : "i" (reg), "rK"(val)); \
  __tmp; })

#define tlk_set_csr(reg, bit)     ({ unsigned long __tmp; \
  __asm__ volatile ("csrrs %0, %1, %2" : "=r"(__tmp) : "i" (reg), "rK"(bit)); \
  __tmp; })

#define tlk_clear_csr(reg, bit)   ({ unsigned long __tmp; \
  __asm__ volatile ("csrrc %0, %1, %2" : "=r"(__tmp) : "i" (reg), "rK"(bit)); \
  __tmp; })

#define tlk_fence_iorw            __asm__ volatile("fence" : : : "memory")

#define tlk_core_get_current_sp() ({ unsigned long __tmp; \
...
#define tlk_core_save_nested_context()     \
    tlk_save_csr(NDS_MEPC)                 \
    tlk_save_csr(NDS_MIE)                  \
    tlk_save_csr(NDS_MSTATUS)              \
    tlk_save_csr(NDS_MXSTATUS)

/* Nested IRQ exit macro : Restore CSRs */
#define tlk_core_restore_nested_context()  \
    tlk_restore_csr(NDS_MIE)               \
    tlk_restore_csr(NDS_MSTATUS)           \
    tlk_restore_csr(NDS_MEPC)              \
 ...
 * @return     none
 */
_tlk_attribute_ram_code_sec_ static inline unsigned int tlk_core_interrupt_disable(void)
{
 ...
 */
_tlk_attribute_ram_code_sec_ static inline unsigned int tlk_core_restore_interrupt(unsigned int en)
{
    if (en) {
 ...
 * @return  none
 */
static _tlk_always_inline void tlk_core_mie_enable(tlk_mie_e mie_mask)
{
 ...
 */
static inline unsigned int tlk_core_mie_disable(tlk_mie_e mie_mask)
{
 ...
 * @note       tlk_core_mie_disable and tlk_core_mie_restore must be used in pairs.
 */
static inline void tlk_core_mie_restore(unsigned int mie_value)
{
 ...
 * @return  none
 */
static inline void tlk_core_interrupt_enable(void)
{
    tlk_set_csr(NDS_MSTATUS, TLK_FLD_MSTATUS_MIE); //global interrupts enable
    tlk_fence_iorw;                            /* Hardware may change this value, fence IO ensures that software changes are valid. */
    tlk_core_mie_enable(TLK_FLD_MIE_MEIE);         //machine interrupt enable selectively
}
 ...
 * @return     none
 */
static inline void tlk_core_set_mhsp_ctr(tlk_mhsp_ctl_e ctl)
{
 ...
 * @return     none
 */
static inline void tlk_core_set_msp_bound(unsigned int bound)
{
 ...
 * @return     none
 */
static inline void tlk_core_set_msp_base(unsigned int base)
{
 ...
 * @return     none
 */
static inline unsigned int tlk_core_get_msp_bound(void)
{
 ...
 * @return     none
 */
static inline unsigned int tlk_core_get_msp_base(void)
{
 ...
 * @return     none
 */
static inline unsigned int tlk_core_get_mcause(void)
{
 ...
 * @return     none
 */
static inline unsigned int tlk_core_get_mepc(void)
{
 ...
 *             - When global interrupts are enabled using the interface tlk_core_interrupt_enable() (mstatus.MIE is enabled)
 *               - Before entering WFI, make sure the following conditions are met:
 *                 -# Enable MEI, MSI or MTI using interface tlk_core_mie_enable(), so that can wake up mcu after core enters WFI.
 *                 -# If there is an interrupt that has already been triggered, the corresponding interrupt flag bit has been cleared.
 *                 -# Interrupt enable for wakeup source using interface tlk_plic_interrupt_enable(), interrupt disable for non-wakeup sources using interface tlk_plic_interrupt_disable().
 ...
 *             - When global interrupts are disabled using the interface tlk_core_interrupt_disable() (mstatus.MIE is disabled)
 *               - Before entering WFI, make sure the following conditions are met:
 *                 -# Enable MEI, MSI or MTI using interface tlk_core_mie_enable(), so that can wake up mcu after core enters WFI.
 *                 -# If there is an interrupt that has already been triggered, the corresponding interrupt flag bit has been cleared.
 *                 -# Interrupt enable for wakeup source using interface tlk_plic_interrupt_enable(), interrupt disable for non-wakeup sources using interface tlk_plic_interrupt_disable().
 *                 -# Clear all current requests from the PLIC using the tlk_plic_clr_all_request() interface.
 ...
 *                 -# Take stimer for example, using the interfaces tlk_stimer_get_irq_status() and tlk_stimer_clr_irq_status() to get and clear the corresponding interrupt flag bit according to the interrupt source.
 *                 -# Your application code.
 *                 -# Using interface tlk_plic_interrupt_complete() to notify PLIC that the corresponding interrupt processing is complete.
 */
static inline void tlk_core_entry_wfi_mode(void)
{
 ...
 * @return    current pc
 */
static inline unsigned int tlk_core_get_current_pc(void)
{
