...
#define TLK_PM_ANA_REG_WD_CLR_BUF0 0x35
#define TLK_PM_ANA_REG_POWER_ON_CLR_BUF0 0x3a
#define TLK_REBOOT_FLAG 1
...
{
    if (tlk_reboot_reason == TLK_SYS_REBOOT_REASON_DEEP_RETENTION) {
        return;
    }

    unsigned char wd_clr0      = tlk_analog_read_reg8(TLK_PM_ANA_REG_WD_CLR_BUF0);
    unsigned char poweron_clr0 = tlk_analog_read_reg8(TLK_PM_ANA_REG_POWER_ON_CLR_BUF0);

    if (wd_clr0 & TLK_POWERON_FLAG) {
        if (poweron_clr0 & TLK_REBOOT_FLAG) {
            tlk_reboot_reason = TLK_SYS_REBOOT_REASON_SW;

            IF_ENABLED(CONFIG_TLK_WDT, (
                extern unsigned char tlk_wd_get_status(void); 
            if (tlk_wd_get_status()) {
                tlk_reboot_reason = TLK_SYS_REBOOT_REASON_WDT;
            }
            ))

            if (clr_en == 1) {
                tlk_analog_write_reg8(TLK_PM_ANA_REG_WD_CLR_BUF0, wd_clr0 & (~TLK_POWERON_FLAG));
            }
        } else {
            tlk_reboot_reason = TLK_SYS_REBOOT_REASON_POR;

            IF_ENABLED(CONFIG_TLK_WDT, (
                extern unsigned char tlk_wd_32k_get_status(void);
            if (tlk_wd_32k_get_status()) {
                tlk_reboot_reason = TLK_SYS_REBOOT_REASON_WDT;
            }
            ))
            ...
            if (clr_en == 1) {
                tlk_analog_write_reg8(TLK_PM_ANA_REG_WD_CLR_BUF0, wd_clr0 & (~TLK_POWERON_FLAG));
                tlk_analog_write_reg8(TLK_PM_ANA_REG_POWER_ON_CLR_BUF0, (poweron_clr0 & (~TLK_BIT_RNG(3, 6))) | TLK_REBOOT_FLAG);
            }
        }
    } else {
        tlk_reboot_reason = TLK_SYS_REBOOT_REASON_DEEP_SLEEP;
    }
}
