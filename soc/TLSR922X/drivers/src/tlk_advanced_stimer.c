...
#include "properties/tlk_advanced_stimer.h"
 ...
 */
_tlk_attribute_ram_code_sec_ void tlk_stimer_set_32k_count_mode(tlk_advanced_system_timer_32k_cnt_mode_e mode)
{
...
{
    #if IS_ENABLED (CONFIG_TLK_SYSTEM_TIMER_ADVANCED_USE_32K_TICK_ANALOG_READOUT)
    /*
    ...
    */
        ...
        t0 = tlk_analog_read_reg32(UNISDK_ADVANCED_STIMER_AREG_32K_TICK);
        while (1) 
        {
            t1 = tlk_analog_read_reg32(UNISDK_ADVANCED_STIMER_AREG_32K_TICK);
            if ((t1 - t0) == 1) {
                return t1;
            } else if (t1 - t0) {
                t0 = t1;
    ...
    while((tlk_stimer_get_state() & FLD_SYSTEM_CLR_RD_DONE) != 0);   //wait rd_done = 0;
    tlk_stimer_set_32k_write(false);   //1:32k write mode; 0:32k read mode
    while((tlk_stimer_get_state() & FLD_SYSTEM_CLR_RD_DONE) == 0);   //wait rd_done = 1;
