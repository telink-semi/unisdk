...
static int storage_pages_modify(struct storage_device *dev,
        ...
        if (!same_data) {
            if (!no_erase) {
                ...
                if (page_offset + page_len < dev->page) {
                    result = dev->read(dev, addr + page_len,  &dev->buffer[page_offset + page_len],
                ...
                if (head_clean + tail_clean < dev->page) {
                    result = dev->write(dev, addr - page_offset + head_clean,
...
int storage_read(uintptr_t addr, void *buf, size_t len)
{
    ...
    do {
        ...
        struct storage_device *dev = storage_device_find(addr, len);
