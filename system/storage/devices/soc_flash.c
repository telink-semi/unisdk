...
_tlk_always_inline
static bool mmisc_ctl_brpe_clear_and_get(void)
{
    uint32_t old;
...
_tlk_always_inline
static void soc_flash_send_cmd(uintptr_t addr, uint32_t cmd)
{
...
_tlk_always_inline
static bool soc_flash_is_busy(uintptr_t addr, uint32_t cmd)
{
    tlk_mspi_rx_cnt(1);
    tlk_mspi_set_address(addr);
...
_tlk_always_inline
static void soc_flash_wait_done(uintptr_t addr, uint32_t cmd)
{
...
static void soc_flash_mspi_wr_ram(uint32_t cmd, uintptr_t addr,
                                  const void *src, void *dst, size_t data_len,
...
static int soc_flash_erase(struct storage_device *dev, uintptr_t addr, size_t len)
{
    (void)dev;
    (void)len;
    bool brpe = mmisc_ctl_brpe_clear_and_get();

    soc_flash_mspi_wr_ram(SOC_FLASH_SECT_ERASE_CMD, addr, NULL, NULL, 0, false,
                          TLK_MSPI_WRITE,
                          SOC_FLASH_WRITE_ENABLE_CMD, SOC_FLASH_READ_STATUS_CMD_LOWBYTE);
    mmisc_ctl_brpe_restore(brpe);
    return 0;
}

static int soc_flash_write(struct storage_device *dev, uintptr_t addr, const void *buf, size_t len)
{
    (void)dev;
    ...
    while (len) {
        ...
        soc_flash_mspi_wr_ram(SOC_FLASH_QUAD_PAGE_PROGRAM_CMD, addr, buf, NULL, wr_len, false,
                              TLK_MSPI_WRITE,
                              SOC_FLASH_WRITE_ENABLE_CMD, SOC_FLASH_READ_STATUS_CMD_LOWBYTE);
        mmisc_ctl_brpe_restore(brpe);
        len -= wr_len;
...
STORAGE_DEVICE_DEFINE(soc_flash_0,
                      SOC_FLASH_START_ADDR, SOC_FLASH_SIZE,
                      SOC_FLASH_PAGE_SIZE, SOC_FLASH_CLEAN_BYTE,
                      soc_flash_read, soc_flash_erase, soc_flash_write,
                      NULL);

__attribute__((used, noinline)) static void soc_flash_register(void)
{
    storage_device_add(&soc_flash_0);
}
