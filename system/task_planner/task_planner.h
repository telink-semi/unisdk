...
#define TASK_REGISTER(name, function)                                                              \
    ...
    static tTask name __attribute__((section(".tasks_array"), used)) = {                           \
        .fn     = function,                                                                        \
        .wake   = 0,                                                                               \
        .active = 0,                                                                               \
    };
...
#define TASK_DELAY(t, tick)                                                                        \
    do                                                                                             \
    {                                                                                              \
        (t)->wake = tlk_mtimer_get_mtime() + (uint64_t) (tick);                                    \
    } while (0)
 ...
 */
#define TASK_PAUSE(t) (t)->active = 0;
 ...
 */
#define TASK_RESUME(t) (t)->active = 1;
...
#define TASK_SPAWN(name, function)                                                                 \
    static tTask name __attribute__((section(".tasks_array.tasks"), used)) = {                     \
        .fn     = function,                                                                        \
        .wake   = 0,                                                                               \
        .active = 1,                                                                               \
    };
 ...
 *
 * @warning
 * - Expression is not parenthesized internally; wrap in parentheses when used in complex
 * expressions.
 ...
 * explicitly using ::tlk_task_sw_timer_start().
 *
...
#define TLK_SW_TIMER_REGISTER(name, callback, tmr_mode, shot_time)                                 \
    static tSwTimer name __attribute__((section(".tasks_array.timers"), used)) = {                 \
