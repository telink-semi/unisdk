...
{
    TLK_LOG_ERROR(assert_log, "ASSERT FAILED: (%s) at %s:%lu, %s",
                     expr ? expr : "?",
                     file ? file : "?",
                     (unsigned long)line,
                     func ? func : "?");
...
#if IS_ENABLED(CONFIG_TLK_DEBUG_ASSERT_MCU_MODE_REBOOT)
    TLK_LOG_ERROR(assert_log, "MCU action: reboot");
    
#elif IS_ENABLED(CONFIG_TLK_DEBUG_ASSERT_MCU_MODE_STALL)
    TLK_LOG_ERROR(assert_log, "MCU action: stall");
    while(1);
    
#elif IS_ENABLED(CONFIG_TLK_DEBUG_ASSERT_MCU_MODE_DEEP_SLEEP)
