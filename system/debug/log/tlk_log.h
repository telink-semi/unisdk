...
#define TLK_LOG_CREATE(logObject, log_module_name) \
    static const tlk_log_t logObject = { log_module_name }
#else
#define TLK_LOG_CREATE(logObject, log_module_name)
#endif
...
void tlk_log_queue(const tlk_log_t *log, tlk_log_level_t level, const char *fmt, ...);
    #define tlk_log(tlk_log_obj, logLevel, fmt, ...) tlk_log_queue(&(tlk_log_obj), (logLevel), (fmt), ##__VA_ARGS__)
 ...
 */
    ...
    #define tlk_log(tlk_log_obj, logLevel, fmt, ...) tlk_log_blocking(&(tlk_log_obj), (logLevel), (fmt), ##__VA_ARGS__)
#endif

#if IS_ENABLED(CONFIG_TLK_DEBUG_LOG_FROM_ISR)
/**
 * @brief       This function serves to set the log message to print queue from ISR without formatting.
 * @param[in]   log - the log object.
 * @param[in]   level - the log level.
...
#else
#define TLK_LOG_ISR(log, level, msg)  do{}while(0)
#endif
...
#else
#define TLK_LOG_DEBUG(log, fmt, ...) do{}while(0)
#define TLK_LOG_DEBUG_ISR(log, msg)  do{}while(0)
#endif
...
#else
#define TLK_LOG_INFO(log, fmt, ...) do{}while(0)
#define TLK_LOG_INFO_ISR(log, msg)  do{}while(0)
#endif
...
#else
#define TLK_LOG_WARN(log, fmt, ...) do{}while(0)
#define TLK_LOG_WARN_ISR(log, msg)  do{}while(0)
#endif
...
#else
#define TLK_LOG_ERROR(log, fmt, ...) do{}while(0)
#define TLK_LOG_ERROR_ISR(log, msg)  do{}while(0)
#endif
