...
#if IS_ENABLED (CONFIG_TLK_DEBUG_LOG_FROM_ISR)  
...
#if IS_ENABLED(CONFIG_TLK_DEBUG_LOG_PRIO_LOW) || IS_ENABLED(CONFIG_TLK_DEBUG_LOG_PRIO_HIGH)
...
static bool pop_event(tlk_log_event_t *out);

#elif IS_ENABLED(CONFIG_TLK_DEBUG_LOG_PRIO_IMMEDIATE)
...
static inline const char *tlk_lvl_str(tlk_log_level_t lvl)
{
    switch(lvl) {
    case TLK_LOG_LEVEL_ERROR: return COL_RED   "ERR" COL_RESET;
    case TLK_LOG_LEVEL_WARN:  return COL_YELLOW"WARN"COL_RESET;
    case TLK_LOG_LEVEL_INFO:  return COL_GREEN "INFO"COL_RESET;
    case TLK_LOG_LEVEL_DEBUG: return COL_CYAN  "DBG" COL_RESET;
    default:                  return "LOG";
    }
}

uint64_t log_get_us(void)
{
...
{
    tlk_tx_done = 1;
    #if IS_ENABLED(CONFIG_TLK_DEBUG_LOG_PRIO_HIGH) // apply for timer initiator
    static tlk_log_event_t e;
    if (pop_event(&e)) {
        tlk_uart_send_bytes(CONFIG_TLK_DEBUG_UART, (const uint8_t *)e.msg_buf, e.msg_len, tlk_tx_handler, TLK_USEC(5));
    }
    #endif
}

#if IS_ENABLED (CONFIG_TLK_DEBUG_LOG_PRIO_IMMEDIATE)

void tlk_log_blocking(const tlk_log_t *log,
                      tlk_log_level_t level,
                      const char *fmt, ...)
{
    uint64_t timestamp = log_get_us();

    /* 1) Prefix */
    int off = snprintf(tlk_log_buffer, sizeof(tlk_log_buffer),
                       "[%llu.%u.%03u]: [%s]: [%s]: ",
    ...
    /* 3) Blocking output */
    #if IS_ENABLED(CONFIG_TLK_DEBUG_LOG_INTERFACE_UART)
    if (size > 0) 
    {
        tlk_uart_send_bytes(CONFIG_TLK_DEBUG_UART, (uint8_t *)tlk_log_buffer, size, tlk_tx_handler, TLK_USEC(5));
        while (!tlk_tx_done) {}
        tlk_tx_done = 0;
    }

    #elif IS_ENABLED(CONFIG_TLK_DEBUG_LOG_INTERFACE_USB_CDC)
...
#if IS_ENABLED(CONFIG_TLK_DEBUG_LOG_PRIO_HIGH)

void tlk_log_timer_shot(void *arg)
{
    (void)arg;
    tlk_log_pump();
}
TLK_SW_TIMER_REGISTER(log_timer, tlk_log_timer_shot, SW_TIMER_MODE_PERIODIC, TASK_MS(CONFIG_TLK_DEBUG_LOG_PERIODIC_PUMP_PERIOD))

#endif

void tlk_log_init(void)
{
    #if IS_ENABLED(CONFIG_TLK_DEBUG_LOG_PRIO_HIGH)
    tlk_sw_timer_start(&log_timer);
    #endif
    s_wr = 0;
    s_rd = 0;
}

static bool push_event(const tlk_log_event_t *e)
{
...
void tlk_log_isr(const tlk_log_t *log, tlk_log_level_t level, const char *msg)
{
    ...
    (void)push_event(&e); /* drop if full */
}
#endif
...
void tlk_log_queue(const tlk_log_t *log, tlk_log_level_t level, const char *fmt, ...)
{
    ...
    if (tlk_plic_is_in_isr()) {
        tlk_log_isr(log, level, "tlk_log called from ISR (use tlk_log_isr)");
        return;
    }
#endif    
    ...
    int16_t off = snprintf(e.msg_buf, sizeof(e.msg_buf),
                   "[%llu.%u.%03u]: [%s]: [%s]: ",
                   (uint64_t)(e.ts_us/ 1000000),
...
void tlk_log_pump(void)
{
    ...
    if (tlk_tx_done)
    {
        while (pop_event(&e)) 
        {
            ...
            {
                static char tlk_log_buffer[CONFIG_TLK_DEBUG_LOG_BUFFER_SIZE];
                    int16_t off = snprintf(tlk_log_buffer, CONFIG_TLK_DEBUG_LOG_BUFFER_SIZE,
                    "[%llu.%u.%03u]: [IRQ]: [%s]: [%s]: %s",
                    (unsigned long long) (e.ts_us / 1000000),
                    (unsigned short)(e.ts_us / 1000u)% 1000u,
                    (unsigned short)(e.ts_us % 1000u),
                    e.module ? e.module : "?",
                    tlk_lvl_str(e.level), e.msg);
                uart_log_status = tlk_uart_send_bytes(CONFIG_TLK_DEBUG_UART, (const uint8_t *)tlk_log_buffer, off, tlk_tx_handler, TLK_USEC(5));
            }
            else
            {
                uart_log_status = tlk_uart_send_bytes(CONFIG_TLK_DEBUG_UART, (const uint8_t *)e.msg_buf, e.msg_len, tlk_tx_handler, TLK_USEC(5));
            }
...
TLK_REGISTER_PRE_INIT(tlk_log_init, TLK_INIT_LEVEL_APPLICATION, TLK_INIT_PRIORITY_NORMAL)

    #if IS_ENABLED (CONFIG_TLK_DEBUG_LOG_PRIO_LOW) && IS_ENABLED(CONFIG_TLK_TASK_PLANNER_MODE_LOOP)
    #include "common/include/tlk_loop.h"
    TLK_REGISTER_LOOP(tlk_log_pump)
    #endif
